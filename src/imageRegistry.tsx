...
export const IMAGE_FALLBACKS: Record<string, string> = {
  ...
  '/src/assets/images/pillar-thobes.jpg': '/images/thoves.1.jpeg',
  ...
  '/src/assets/images/thobes.jpg': '/images/thoves.1.jpeg',
  '/src/assets/images/thobes-white-luxury.jpg': '/images/thoves.1.jpeg',
  '/src/assets/images/thobes-heritage-modern.jpg': '/images/thoves.1.jpeg',
...
export function getFallbackImage(src?: string, category?: BusinessCategory): string {
  ...
  if (category === 'thobes') {
    ...
    if (isValidThobePattern) {
      // If it is one of the default un-uploaded ones, use the local master one
      if (src.includes('photo-1528459801416') || src.includes('photo-1620799140408') || src.includes('photo-1593030761757') || src.includes('src/assets/images/thobes')) {
        return '/images/thoves.1.jpeg';
      }
      return src;
    }
    ...
    // Absolutely no images from other categories are allowed under Thobes. Fall back to secure local Thobes.
    return '/images/thoves.1.jpeg';
  }
...
export function SafeImage({ src, alt, className, containerClassName, category, forceCover, ...props }: SafeImageProps) {
  ...
  const handleError = () => {
    ...
    setErrorCount(2);
    setCurrentSrc(category === 'thobes' ? '/images/thoves.1.jpeg' : ABSOLUTE_PLACEHOLDER);
  };
...
export const DEFAULT_BRAND_POOL: GlobalImage[] = [
  ...
  { id: 'bd9', url: '/src/assets/images/thobes-white-luxury.jpg', category: 'thobes', title: 'White Premium Luxury Thobe', source: 'brand default' },
...
export function getGlobalImagePool(categoryFilter?: BusinessCategory): GlobalImage[] {
  ...
  // Scan custom products (from database cache) set for images to maintain single-source integrity
  const productPool: GlobalImage[] = [];
  try {
    const rawCustom = localStorage.getItem('zoal_custom_products');
    if (rawCustom) {
      const parsedCustom: Product[] = JSON.parse(rawCustom);
...
export function useGlobalProducts(): Product[] {
  ...
  useEffect(() => {
    ...
    readProductsAndOverrides(); // Initial read
  ...
  const mergedProducts = React.useMemo(() => {
    const merged = customProducts.filter(p => !deletedStaticIds.includes(p.id));
    return merged.map(p => {
      let resolved = { ...p };
