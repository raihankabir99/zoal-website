...
export function mapCategoryName(name?: string): string {
  ...
  if (trimmed === 'Luxury Fashion' || trimmed === 'fashion' || trimmed === 'PREMIUM COLLECTIONS') return 'Fashion & Style';
  if (trimmed === 'Business & Strategy' || trimmed === 'business') return 'Business Insights';
  if (trimmed === 'Company News' || trimmed === 'company') return 'News & Updates';
  if (trimmed === 'coffee' || trimmed === 'Coffee Stories' || trimmed === 'Coffee & Drinks') return 'Coffee & Drinks';
  if (trimmed === 'bakery' || trimmed === 'Gourmet Bakery' || trimmed === 'Bakery Heritage') return 'Bakery Heritage';
  if (trimmed === 'market') return 'Market & Grocery';
  if (trimmed === 'thobes') return "Thobes & Men's Wear";
  return trimmed;
}
...
export const blogService = {
  async getPosts(params?: { category?: string; tag?: string; search?: string; status?: string }): Promise<BlogPost[]> {
    try {
      ...
      const rawPosts = data.posts && data.posts.length > 0 ? data.posts : FALLBACK_BLOG_POSTS;
      ...
      console.warn('Backend blog endpoint unavailable, using fallback blog posts:', e);
  ...
  async createPost(payload: Partial<BlogPost>): Promise<BlogPost> {
    try {
      ...
      console.warn('Backend createPost unavailable:', e);
      const newPost: BlogPost = {
        id: `post-${Date.now()}`,
        title: payload.title || 'Untitled Post',
        slug: payload.slug || 'untitled-post',
  ...
  async updatePost(id: string, payload: Partial<BlogPost>): Promise<BlogPost> {
    try {
      ...
      console.warn('Backend updatePost unavailable:', e);
      return { id, ...payload } as BlogPost;
    }
  },
  ...
  async deletePost(id: string): Promise<void> {
    try {
      ...
      console.warn('Backend deletePost unavailable:', e);
    }
  },
  ...
  async getCategories(): Promise<BlogCategory[]> {
    try {
      ...
      const data = await res.json();
      const rawCategories = Array.isArray(data) && data.length > 0 ? data : FALLBACK_CATEGORIES;
      return rawCategories.map(cat => ({
        ...cat,
        name: mapCategoryName(cat.name)
      }));
    } catch (e) {
      console.warn('Backend categories unavailable, using fallbacks:', e);
  ...
  async getTags(): Promise<BlogTag[]> {
    try {
      ...
      console.warn('Backend tags unavailable, using fallbacks:', e);
  ...
  async getAuthors(): Promise<BlogAuthor[]> {
    try {
      ...
      console.warn('Backend authors unavailable, using fallbacks:', e);
