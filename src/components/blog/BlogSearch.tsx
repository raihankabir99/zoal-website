...
export function BlogSearch({ onPostClick }: BlogSearchProps) {
  ...
  const localized = (obj: any, field: string) => {
    if (!obj) return '';
    if (i18n.language === 'ar') {
      if (obj.nameAr && field === 'name') return obj.nameAr;
      const arField = `${field}_ar`;
      if (obj[arField]) return obj[arField];
      if (obj.name_ar && field === 'name') return obj.name_ar;
    }
  ...
  return (
    <div className="max-w-7xl mx-auto px-0 sm:px-6 lg:px-8 pt-0 pb-12 sm:py-12 space-y-12">
      {/* Search Header */}
      <div className="space-y-6 px-2.5 sm:px-0">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/5 pb-6">
          <div className="flex items-center gap-6">
            <span className="text-[10px] text-zinc-500 font-mono uppercase tracking-[0.2em]">{t('blog.search.discoveries_found', { count: results.length })}</span>
            <button 
            ...
            >
              ...
              {showFilters ? t('blog.search.hide_filters') : t('blog.search.advanced_refinement')}
            </button>
          </div>

          <div className="flex items-center gap-4">
            <span className="text-[10px] text-zinc-600 uppercase font-mono tracking-widest">{t('blog.search.sort_by')}</span>
            <div className="flex gap-2">
              {[
                { id: 'newest', label: t('blog.search.sort_newest'), icon: Clock3 },
                { id: 'popular', label: t('blog.search.sort_popular'), icon: TrendingUp },
                { id: 'oldest', label: t('blog.search.sort_oldest'), icon: Calendar }
              ].map((s) => (
                <button
                  key={s.id}
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-8 border-b border-white/5">
              ...
              <div className="space-y-4">
                <h4 className="text-[10px] text-gold-pure font-mono uppercase tracking-widest">{t('blog.search.by_category')}</h4>
              ...
              {/* Tag Filter */}
              <div className="space-y-4">
                <h4 className="text-[10px] text-gold-pure font-mono uppercase tracking-widest">{t('blog.search.by_tag')}</h4>
                <div className="flex flex-wrap gap-2">
                  <button 
              ...
              {/* Author Filter */}
              <div className="space-y-4">
                <h4 className="text-[10px] text-gold-pure font-mono uppercase tracking-widest">{t('blog.search.by_author')}</h4>
                <div className="flex flex-wrap gap-2">
                  <button 
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 space-y-4">
          ...
          <span className="text-[10px] tracking-[0.4em] text-gold-pure uppercase font-mono animate-pulse">{t('blog.search.scanning')}</span>
        </div>
      ) : results.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 px-0 sm:px-0">
          {results.map((post) => (
            <motion.div
              key={post.id}
            ...
            >
              ...
              <div className="space-y-3 px-2.5 sm:px-0">
                <div className="flex items-center gap-3 text-[9px] text-zinc-500 font-mono uppercase tracking-widest">
                  ...
                  <span>{t('blog.by_author', { name: localized(post.zoal_blog_authors, 'name') || t('blog.editor') })} • ⏱ {t('blog.min_read', { count: post.reading_time || 5 })}</span>
                </div>
        ...
        <div className="text-center py-40 space-y-6 px-2.5 sm:px-0">
          ...
          <div className="space-y-2">
            <h3 className="text-2xl font-bold text-white font-display uppercase tracking-[0.2em]">{t('blog.search.no_results')}</h3>
            <p className="text-zinc-600 text-sm max-w-md mx-auto">{t('blog.search.no_results_desc')}</p>
          </div>
          <button 
