...
export function BlogGridPage({ type, id, onBack, onPostClick }: BlogGridPageProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setError('Content temporarily unavailable. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error) {
    return (
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-24 text-center space-y-6">
        <h2 className="text-2xl font-bold text-white font-display uppercase tracking-widest">{t('blog.unable_to_load')}</h2>
        <p className="text-zinc-400 text-sm">{error}</p>
        <button 
          onClick={onBack}
          className="bg-gold-pure text-black px-6 py-2 rounded-xs text-xs font-bold uppercase tracking-widest hover:bg-gold-light transition-all cursor-pointer inline-flex items-center gap-2"
        >
          <ArrowLeft className="w-3.5 h-3.5" />
          <span>{t('blog.back_to_journal')}</span>
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="w-full space-y-8 sm:space-y-16 pb-12">
      {/* Visual Subpage Hero Header */}
      <header className="relative h-[25vh] sm:h-[40vh] w-full overflow-hidden bg-zinc-950 border-b border-white/5">
        {/* Back Arrow Overlay */}
        ...
        {/* Overlay Title */}
        <div className="absolute inset-0 z-10 flex flex-col justify-end p-4 sm:p-8 md:p-12 max-w-7xl mx-auto w-full">
          <div className="space-y-1 sm:space-y-2 max-w-2xl px-3 sm:px-0">
            <span className="text-[8px] sm:text-[10px] tracking-[0.4em] text-gold-pure uppercase font-mono block">
              {type === 'author' ? t('blog.archive.dossier') : t('blog.archive.general')}
            </span>
            <h1 className="text-xl sm:text-4xl md:text-5xl font-bold text-white font-display uppercase tracking-wider leading-tight">
              {getPageTitle()}
            </h1>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-0 sm:px-6 lg:px-8 space-y-8 sm:space-y-16">
        {type === 'author' && !loading && (
          <div className="flex justify-end px-2.5 sm:px-0">
        ...
        {type === 'author' && activeTab === 'profile' && !loading && (
          <div className="space-y-16 px-2.5 sm:px-0">
          {/* Main Dossier Card */}
          <div className="bg-zinc-950 border border-white/5 p-4 sm:p-8 md:p-12 rounded-sm space-y-8 relative overflow-hidden">
          ...
          <div className="space-y-8 px-2.5 sm:px-0">
      ...
      {(type !== 'author' || activeTab === 'all') && (
        <div className="space-y-6 sm:space-y-12 px-0 sm:px-0">
          {/* Refined Description & Count Block */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6 sm:pb-8 px-2.5 sm:px-0">
            <p className="text-zinc-400 text-sm sm:text-base font-light leading-relaxed max-w-2xl">
              {getPageDescription()}
            </p>
            <div className="flex items-center gap-4 text-zinc-500 font-mono text-[10px] uppercase shrink-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
              {posts.map((post) => (
                ...
                >
                  ...
                  <div className="space-y-4 px-2.5 sm:px-0">
