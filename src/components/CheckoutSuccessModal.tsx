        ...
        >
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gold-pure/10 border border-gold-pure/25 mb-4 animate-pulse">
            ...
            <span className="text-[9px] sm:text-[10px] uppercase tracking-[0.3em] font-display font-semibold text-gold-pure">
              ✅ {t('checkout.status.confirmed', { defaultValue: 'Order Confirmed' })}
            </span>
          </div>

          {/* Main Titles */}
          <h2 className="text-lg sm:text-xl font-display font-semibold text-white uppercase tracking-wider mb-2">
            {t('order_success.order_placed_successfully', { defaultValue: 'Order Placed Successfully' })}
          </h2>
          
          <p className="text-zinc-400 text-xs leading-relaxed max-w-sm mx-auto mb-6">
            {t('order_success.thank_you_order', { name: order?.customerName ? `, ${order.customerName}` : '', defaultValue: `Thank you for your order${order?.customerName ? `, ${order.customerName}` : ''}` })}.<br />
            {t('order_success.order_received_processing', { defaultValue: 'Your order has been received successfully and is now being processed. You will receive an order confirmation email shortly.' })}
          </p>
          ...
          {order && (
            <div className="bg-black/80 border border-white/5 rounded-xs p-3.5 mb-6 text-left font-mono text-[11px] text-zinc-400 space-y-2">
              <div className="flex justify-between border-b border-white/5 pb-1.5 mb-1.5">
                <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-sans">{t('order_success.order_ref', { defaultValue: 'Order Ref' })}:</span>
                <strong className="text-gold-pure font-bold">{order.id}</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-[9px] text-zinc-600 uppercase font-sans">{t('order_success.products', { defaultValue: 'Products' })}:</span>
                <span className="text-zinc-300 truncate max-w-[200px] text-right">
                  {order.items.map(item => item.name).join(', ')}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-[9px] text-zinc-600 uppercase font-sans">{t('order_success.total', { defaultValue: 'Total' })}:</span>
                <span className="text-zinc-200 font-bold">{order.total.toFixed(2)} SAR</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[9px] text-zinc-600 uppercase font-sans">{t('order_success.email_logged', { defaultValue: 'Email Logged' })}:</span>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            >
              ...
              <span>{t('order_success.continue_shopping', { defaultValue: 'Continue Shopping' })}</span>
            </button>
            ...
            >
              ...
              <span>{t('order_success.view_my_orders', { defaultValue: 'View My Orders' })}</span>
            </button>
            ...
            >
              {t('order_success.close', { defaultValue: 'Close' })}
            </button>
          </div>
