...
export default function EnterpriseCrm({ currentUser, orders, addLog }: EnterpriseCrmProps) {
  ...
  // --- PostgreSQL / Supabase Real Database CRM State ---
  ...
  const fetchCrmCustomers = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        setCustomers([]);
        setAuthError('CRM authentication required. Please log in with an administrative account.');
      ...
      if (res.ok) {
        ...
        setCustomers([]);
        setAuthError(errData.message || 'Unauthorized access to CRM records. High-level administrative credentials required.');
      }
    } catch (err: any) {
      console.error('Error fetching CRM customers from server:', err);
    } finally {
      setIsLoadingCrm(false);
    }
  }, [searchQuery, filterStatus, filterSegment, filterVipStatus, currentPage]);
  ...
  const handleCreateCustomer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(`Validation Warning: ${errData.message || errData.error}`);
        return;
      }
      ...
      if (data.customer) {
        ...
        addLog(`Manually Created Client Profile: ${addForm.name}`, data.customer.id);
        addAdminNotification('New Customer Registered', `Verified elite account created for Customer ${addForm.name}.`, 'success', 'new_customer');
      ...
      alert('Error communicating with customer creation backend server.');
    }
  };
  ...
  const handleDeleteCustomer = async (id: string, name: string) => {
    if (isStaff) {
      alert('Security Refusal: Staff accounts are strictly forbidden from deleting Customer records.');
      return;
    }
    if (window.confirm(`Are you absolutely sure you want to erase/deactivate ${name}'s CRM profile?`)) {
      try {
        const token = localStorage.getItem('zoal_auth_token') || sessionStorage.getItem('zoal_auth_token');
        if (!token) {
          alert('CRM authentication required. Please log in with an administrative account.');
        ...
        if (res.ok) {
          ...
          addLog(`Erased/Deactivated Customer CRM Account`, name);
          if (selectedCustomerId === id) setSelectedCustomerId(null);
          alert('Customer Profile successfully updated/deactivated.');
        } else {
          const errData = await res.json();
          alert(`Error: ${errData.message || errData.error}`);
        }
      } catch (err) {
            ...
            activityTimeline: [
              {
                ...
                description: `Support Internal Note added by ${currentUser?.name || 'Staff'}: [${newNoteType}] ${newNoteContent.slice(0, 30)}...`,
  ...
  const handleGrantBirthdayReward = () => {
          ...
          activityTimeline: [
            {
              ...
              description: `Granted active Birthday Reward credit to Customer. Credited +200 bonus loyalty points.`,
              time: new Date().toLocaleString()
            },
            ...c.activityTimeline
          ]
    ...
    addAdminNotification('Birthday Reward Granted', `Customer ${selectedCustomer?.name} was granted a birthday reward.`, 'success', 'reward_granted');
  ...
  const handleRedeemLoyaltyReward = (rewardId: string, pointsCost: number) => {
    ...
    if (currentPoints < pointsCost) {
      alert(`Validation Refusal: Customer only has ${currentPoints} points. This reward requires ${pointsCost} points.`);
      return;
    }
    ...
    addAdminNotification('Loyalty Reward Redeemed', `Customer ${selectedCustomer?.name} redeemed ${pointsCost} points.`, 'success', 'reward_redeemed');
  ...
  const handleBulkAction = (actionType: 'status' | 'tag' | 'marketing' | 'archive' | 'notify' | 'export', payload?: any) => {
    if (selectedCustomerIds.length === 0) {
      alert('Validation Warning: Please select at least one Customer using checkboxes.');
      return;
    }
    ...
    if (actionType === 'status') {
            ...
            activityTimeline: [
              {
                ...
                description: `Account status bulk-updated to: ${nextStatus} by ${currentUser?.name || 'Administrator'}`,
                time: new Date().toLocaleString()
              },
              ...c.activityTimeline
            ]
      ...
      addLog(`Bulk Updated Status of ${selectedCustomerIds.length} Customers to ${nextStatus}`);
      addAdminNotification('Bulk Status Update', `Bulk update: ${selectedCustomerIds.length} Customers updated to ${nextStatus}.`, 'info', 'bulk_update');
      alert(`Success: Selected ${selectedCustomerIds.length} Customer profiles have been updated to status: ${nextStatus}.`);
            ...
            activityTimeline: [
              {
                id: `act-${Date.now()}`,
                event: 'Profile Update' as const,
                description: `Direct CRM tags bulk-applied: [${newTags.join(', ')}] by ${currentUser?.name || 'Support Representative'}`,
                time: new Date().toLocaleString()
              },
              ...c.activityTimeline
            ]
      ...
      addLog(`Bulk Tagged ${selectedCustomerIds.length} Customers with: ${tagStr}`);
      addAdminNotification('Bulk Tag Update', `Bulk update: ${selectedCustomerIds.length} Customers tagged with: ${newTags.join(', ')}.`, 'info', 'bulk_update');
      alert(`Success: Selected ${selectedCustomerIds.length} Customers have been tagged with: ${newTags.join(', ')}.`);
      setSelectedCustomerIds([]);
      setBulkTagInput('');
    }
      ...
      addLog(`Bulk configured marketing reach of ${selectedCustomerIds.length} Customers`);
      addAdminNotification('Bulk Marketing Update', `Bulk update: Marketing ${channel.toUpperCase()} set to ${optIn ? 'Opt-In' : 'Opt-Out'} for ${selectedCustomerIds.length} Customers.`, 'info', 'bulk_update');
      alert(`Success: Set marketing ${channel.toUpperCase()} to ${optIn ? 'Opt-In' : 'Opt-Out'} for ${selectedCustomerIds.length} Customers.`);
      setSelectedCustomerIds([]);
    }
            ...
            activityTimeline: [
              {
                id: `act-${Date.now()}`,
                event: 'Profile Update' as const,
                description: `CRM Customer profile archived to clear management clutter.`,
                time: new Date().toLocaleString()
              },
              ...c.activityTimeline
            ]
      ...
      addLog(`Bulk Archived ${selectedCustomerIds.length} Customer Profiles`);
      addAdminNotification('Bulk Archiving', `Bulk update: ${selectedCustomerIds.length} Customers archived.`, 'warning', 'bulk_update');
      alert(`Success: Archived ${selectedCustomerIds.length} Customer accounts. They are now hidden from active listings.`);
      setSelectedCustomerIds([]);
    }
      ...
      addLog(`Bulk Sent Direct Broadcast to ${selectedCustomerIds.length} Customers via ${notifyChannel}`);
      addAdminNotification('Bulk Notification', `Bulk broadcast sent to ${selectedCustomerIds.length} Customers via ${notifyChannel}.`, 'info', 'bulk_update');
      alert(`Success: Direct Broadcast has been sent to ${selectedCustomerIds.length} Customers via ${notifyChannel}.`);
      setSelectedCustomerIds([]);
      ...
      link.setAttribute('download', `AL_ZOAL_CRM_Selected_Customers_${selectedCustomerIds.length}_${new Date().toISOString().slice(0, 10)}.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      
      addLog(`Exported ${selectedCustomerIds.length} selected Customer profiles`);
      setSelectedCustomerIds([]);
    }
  };

  // D. Export CSV Customer Registrations
  const handleExportCsv = () => {
    ...
    const link = document.createElement('a');
    link.setAttribute('href', csvContent);
    link.setAttribute('download', `AL_ZOAL_CRM_Customers_Export_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  return (
    <div className="space-y-6 text-left animate-fade-in font-sans relative z-10" id="enterprise-crm-management">
      
      {/* AUTH ERROR BANNER FOR FAIL-CLOSED SECURITY */}
      {authError && (
        <div className="bg-red-950/40 border border-red-500/30 p-4 rounded-xs flex items-center gap-3 text-red-300 text-xs font-mono">
          <ShieldAlert className="w-5 h-5 text-red-400 shrink-0" />
          <div>
            <p className="font-bold uppercase tracking-wider">Authentication Required</p>
            <p className="text-[11px] text-red-400/80 mt-0.5">{authError}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-white/5 pb-5">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[9px] tracking-[0.4em] text-gold-pure uppercase font-mono block">AL ZOAL SUPPORT PLATFORM</span>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div className="bg-zinc-950/80 border border-white/5 p-4 rounded-xs text-left relative overflow-hidden group hover:border-gold-pure/20 duration-300">
              ...
              <p className="text-[8.5px] tracking-widest text-zinc-500 uppercase font-mono mb-1">TOTAL RECORDS</p>
              <h3 className="text-xl font-bold font-mono text-white tracking-tight">{crmMetrics.total}</h3>
              <p className="text-[8px] text-zinc-500 mt-1 font-mono">Customer Accounts</p>
            </div>
            ...
            <div className="bg-zinc-950/80 border border-white/5 p-4 rounded-xs text-left relative overflow-hidden group hover:border-gold-pure/20 duration-300">
              ...
              <p className="text-[8.5px] tracking-widest text-emerald-500 uppercase font-mono mb-1">ACTIVE CUSTOMERS</p>
            ...
            <div className="bg-zinc-950/80 border border-white/5 p-4 rounded-xs text-left relative overflow-hidden group hover:border-gold-pure/20 duration-300">
              ...
              <p className="text-[8px] text-zinc-500 mt-1 font-mono">Average Revenue / Customer</p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[8.5px] text-zinc-500 font-mono">Cumulative Customer Signups</span>
              </div>
            ...
            {/* Chart 2: Top Revenue Contributing Customers */}
            <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[9.5px] tracking-widest text-gold-pure uppercase font-mono block">TOP REVENUE CONTRIBUTORS</span>
                <span className="text-[8.5px] text-zinc-500 font-mono">Top VIP Customers Spend (SAR)</span>
              </div>
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="bg-zinc-950 border border-white/5 p-4 rounded-xs space-y-3">
            <div className="flex flex-col lg:flex-row gap-3">
              ...
              <div className="relative flex-grow">
                ...
                <input
                  type="text"
                  placeholder="Search Customer by Name, Email address, ID code or Phone..."
                  value={searchQuery}
                  ...
                  {filteredCustomers.length === 0 && (
                    <tr>
                      <td colSpan={8} className="p-8 text-center text-zinc-500 italic">
                        No Customers found matching current filter metrics.
                      </td>
                    </tr>
                  )}
                </tbody>
              ...
              <div className="sticky bottom-6 left-0 right-0 bg-zinc-950/95 border border-[#D4AF37]/40 p-4 rounded-xs shadow-[0_15px_40px_rgba(0,0,0,0.8)] backdrop-blur-md flex flex-col lg:flex-row justify-between items-center gap-4 z-40 animate-fade-in-up mt-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs text-white font-mono">
                    Bulk Selection: <span className="text-gold-pure font-bold font-mono">{selectedCustomerIds.length}</span> Customers active
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-500 font-mono mt-0.5">
                      Customer List • Registered {selectedCustomer.registrationDate}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (nextStatus === 'VIP') {
                          addAdminNotification('VIP Customer Created', `Customer ${selectedCustomer.name} was promoted to VIP status.`, 'success', 'new_vip_customer');
                        } else if (nextStatus === 'Blocked') {
                          addAdminNotification('Customer Blocked', `Security Event: Customer ${selectedCustomer.name} has been blocked.`, 'error', 'blocked_customer');
                        }
                      ...
                      onChange={(e) => {
                        ...
                        if (token) {
                          fetch(`/api/admin/customers/${selectedCustomer.id}`, {
                            method: 'PATCH',
                            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                            body: JSON.stringify({ segment: nextSegment, manualSegment: true })
                          }).catch(console.error);
                        }
                        if (nextSegment === 'VIP Customer') {
                          addAdminNotification('VIP Customer Created', `Customer ${selectedCustomer.name} was manually added to VIP Customer segment.`, 'success', 'new_vip_customer');
                        }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                  ...
                  <div className="bg-black/50 border border-white/5 p-4 rounded-xs space-y-3 font-mono text-[9.5px]">
                    <span className="text-[8.5px] uppercase tracking-widest text-gold-pure block font-bold border-b border-white/5 pb-1">Customer List Matrix</span>
                ...
                <div className="lg:col-span-2 space-y-6 text-left">
                  ...
                  {activeDrawerTab === 'orders' && (
                    <div className="space-y-6 animate-fade-in">
                      ...
                      {/* TRANSACTION HISTORY */}
                      <div className="bg-black/50 border border-white/5 p-4 rounded-xs space-y-3 font-mono text-[9.5px]">
                        <span className="text-[8.5px] uppercase tracking-widest text-gold-pure block font-bold border-b border-white/5 pb-1">PRESTIGE ORDER RECORDS</span>
                        ...
                        <div className="divide-y divide-white/5 max-h-48 overflow-y-auto pr-1">
                          ...
                          {selectedCustomer.orderHistory.length === 0 && (
                            <p className="text-zinc-600 italic text-center font-mono py-6 text-[9.5px]">No purchase record found for this Customer email account.</p>
                          )}
                        </div>

                        {/* Interactive Event Simulation Console */}
                        <div className="border-t border-white/5 pt-3 mt-2">
                          <span className="text-[7.5px] uppercase tracking-widest text-zinc-500 block mb-2">SUPPORT ALERT SIMULATION PANELS</span>
                          <div className="grid grid-cols-2 gap-2">
                            <button
                        ...
                        <div className="bg-black/50 border border-white/5 p-4 rounded-xs space-y-2.5 font-mono text-[10px]">
                          ...
                          <div className="space-y-2">
                            ...
                            {selectedCustomer.savedCart.length === 0 && (
                              <p className="text-zinc-600 italic">Shopping Bag is empty.</p>
                            )}
                          </div>
                        </div>
                      ...
                      {/* CUSTOMER NOTES AND INTERNAL SUPPORT LOGS */}
                      <div className="bg-black/50 border border-white/5 p-4 rounded-xs space-y-3">
                        <span className="text-[8.5px] font-mono uppercase tracking-widest text-gold-pure block font-bold border-b border-white/5 pb-1">
                          INTERNAL SUPPORT MEMORANDUM (HIDDEN FROM CUSTOMER)
                        </span>
                        ...
                        <div className="space-y-2.5 max-h-48 overflow-y-auto pr-1">
                          ...
                          {selectedCustomer.notes.length === 0 && (
                            <p className="text-zinc-600 italic text-center font-mono py-4 text-[9.5px]">No private memos recorded for this Customer profile.</p>
                          )}
                        </div>
                    ...
                    <div className="space-y-6 animate-fade-in">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs text-left">
                          ...
                          <span className="text-lg font-bold text-gold-pure font-sans block mt-1 tabular-nums-fix">{formatCurrency(selectedCustomer.totalSpending)}</span>
                        ...
                        <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs text-left">
                          ...
                          <span className="text-lg font-bold text-white font-sans block mt-1 tabular-nums-fix">
                        ...
                        <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs text-left">
                          ...
                          <span className="text-lg font-bold text-white font-sans block mt-1 tabular-nums-fix">28.4 Days</span>
                    ...
                    <div className="space-y-6 animate-fade-in">
                      ...
                      <div className="relative overflow-hidden p-5 bg-gradient-to-br from-zinc-900 to-black border border-[#D4AF37]/30 rounded-xs text-left">
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="text-[7.5px] font-mono tracking-widest uppercase text-zinc-500">ZOAL PORTAL LOYALTY CLUB</span>
                            <h4 className="text-xl font-bold font-display text-white mt-1 uppercase tracking-widest">
                              {selectedCustomer.totalSpending > 5000 ? 'Gold Elite Member' : selectedCustomer.totalSpending > 2000 ? 'Silver Tier Member' : 'Bronze Classic'}
                            </h4>
                          </div>
                        ...
                        <div className="bg-black/50 border border-white/5 p-4 rounded-xs space-y-3 font-mono">
                          ...
                          <div className="space-y-3 text-[10px]">
                            <div className="flex justify-between items-center bg-zinc-950 p-3 rounded-xs border border-white/5">
                              <div>
                                <span className="font-bold text-white block">Customer Birthday</span>
                  ...
                  {activeDrawerTab === 'communications' && (
                    <div className="space-y-6 animate-fade-in">
                      ...
                      <div className="bg-black/50 border border-white/5 p-4 rounded-xs space-y-3 font-mono">
                        <span className="text-[8.5px] uppercase tracking-widest text-gold-pure block font-bold border-b border-white/5 pb-1">CRM OUTBOUND SUPPORT OUTREACH</span>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1">
                              ...
                              <select name="channel" className="w-full bg-black border border-white/10 text-white p-1.5 text-xs rounded-xs outline-none focus:border-gold-pure">
                                ...
                                <option value="SMS">Telecommunications SMS Panel</option>
                                <option value="WhatsApp">WhatsApp Support Node</option>
                              </select>
                            </div>
                            <div className="space-y-1">
                              ...
                              <select name="template" className="w-full bg-black border border-white/10 text-white p-1.5 text-xs rounded-xs outline-none focus:border-gold-pure">
                                <option value="General Followup">General Support Followup</option>
                  ...
                  {activeDrawerTab === 'reviews' && (
                    <div className="space-y-6 animate-fade-in">
                      ...
                      <div className="bg-black/50 border border-white/5 p-4 rounded-xs space-y-4 font-mono text-[9.5px]">
                        <span className="text-[8.5px] uppercase tracking-widest text-gold-pure block font-bold border-b border-white/5 pb-1">CUSTOMER REVIEWS & FEEDBACK FEED</span>
                  ...
                  <div className="bg-black/50 border border-white/5 p-4 rounded-xs space-y-3 font-mono text-[9.5px]">
                    <span className="text-[8.5px] uppercase tracking-widest text-gold-pure block font-bold border-b border-white/5 pb-1">CUSTOMER AUDIT TIMELINE TRACKER</span>
      ...
      {/* 4. MODAL FOR REGISTERING A CUSTOMER ACCOUNT */}
      <AnimatePresence>
          ...
          <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-zinc-950 border border-white/10 rounded-xs max-w-md w-full p-6 space-y-5 relative text-left">
              ...
              <div>
                ...
                <h3 className="text-lg font-bold tracking-widest font-display text-white uppercase">Register Prestige Customer</h3>
              </div>

              <form onSubmit={handleCreateCustomer} className="space-y-4 text-xs font-mono">
                
                <div className="space-y-1">
                  <label className="text-zinc-500 uppercase tracking-widest block text-[8.5px]">Customer Full Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="email"
                    required
                    placeholder="e.g. customer@royal.sa"
                    value={addForm.email}
