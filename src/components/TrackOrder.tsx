...
export default function TrackOrder({ orders, setCurrentPage, isEmbedded = false }: TrackOrderProps) {
  ...
  const tr = {
    ...
    subtitle: isAr 
      ...
      : 'Enter your order number below to view the latest delivery updates and shipment status.',
    orderNum: isAr ? 'رقم الطلب' : 'Order Number',
    orderNumPlaceholder: isAr ? 'مثال: ZL-9871' : 'Order ID (e.g. ZL-9871)',
    email: isAr ? 'البريد الإلكتروني' : 'Email Address',
    emailPlaceholder: isAr ? 'أدخل البريد الإلكتروني' : 'Email Address',
    btnTrack: isAr ? 'تتبع طلبك الآن' : 'Track Package',
    btnTracking: isAr ? 'جاري البحث في السجلات...' : 'Searching archives...',
    quickDemo: isAr ? 'العروض التجريبية السريعة' : (import.meta.env.PROD ? 'Recent Orders' : 'Quick Demo'),
    ...
    emailSent: isAr ? 'تم إرسال بريد التأكيد الإلكتروني' : 'Fulfillment Email Shipped',
    ...
    timeline: [
      ...
      { key: 'shipped', label: isAr ? 'تم الشحن' : 'Shipped', desc: isAr ? 'تم تسليم الشحنة لشركة التوصيل الفاخر' : 'Shipped with premium logistic courier' },
    ...
    invoiceSubtitle: isAr ? 'سجل المعاملة المالية الرسمي لعملاء زول المميزين' : 'Official certified financial transaction receipt',
    ...
    itemCol: isAr ? 'المنتج الفاخر / الخيار' : 'Premium Item / Selected Option',
    ...
    // Support Modal
    supportTitle: isAr ? 'مجلس دعم عملاء زول النخبة' : 'ZOAL Support Panel',
    supportSubtitle: isAr ? 'تواصل مباشرة مع سفير خدمة العملاء لمساعدتك فوراً' : 'Connect instantly with our support team',
    whatsappSupport: isAr ? 'تواصل عبر واتساب النخبة' : 'Priority WhatsApp Support',
    callSupport: isAr ? 'اتصال مباشر بالسفير المالي' : 'Direct Call Hotlines',
    emailSupport: isAr ? 'راسل البريد الرسمي الفوري' : 'Business Priority Email',
    ...
    supportSuccess: isAr ? 'تم إرسال استفسارك بنجاح. سيتواصل معك أحد سفرائنا في أقل من ١٥ دقيقة.' : 'Your request was successfully prioritized. A support specialist will connect in under 15 minutes.'
  };
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      const mockInvoice = {
        ...
        vat: searchedOrder.vat || (searchedOrder.total - (searchedOrder.total / 1.15)),
  ...
  const computedCourierName = useMemo(() => {
    ...
    if (searchedOrder.shipping === 0) {
      return isAr ? 'أسطول زول للمرسول الخاص' : 'ZOAL VIP Delivery Transport';
    }
                  ...
                  orders.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      <p className="text-xs text-zinc-400">
                        {isAr ? 'اختر أحد طلباتك للتتبع السريع:' : 'Select one of your recent orders to track immediately:'}
                      </p>
