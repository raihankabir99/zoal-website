...
export default function TrackOrder({ orders, setCurrentPage, isEmbedded = false }: TrackOrderProps) {
  ...
  const tr = {
    title: t('track_order.title', { defaultValue: 'Track Your Order' }),
    subtitle: t('track_order.subtitle', { defaultValue: 'Enter your order details below to check live updates and status.' }),
    orderNum: t('track_order.orderNum', { defaultValue: 'Order ID' }),
    orderNumPlaceholder: t('track_order.orderNumPlaceholder', { defaultValue: 'e.g. ZL-9871' }),
    email: t('track_order.email', { defaultValue: 'Email Address' }),
    emailPlaceholder: t('track_order.emailPlaceholder', { defaultValue: 'Enter your email' }),
    btnTrack: t('track_order.btnTrack', { defaultValue: 'TRACK NOW' }),
    btnTracking: t('track_order.btnTracking', { defaultValue: 'Searching archives...' }),
    quickDemo: t('track_order.quickDemo', { defaultValue: import.meta.env.PROD ? 'Recent Orders' : 'Quick Demo' }),
    orSelectDemo: t('track_order.orSelectDemo', { defaultValue: 'Select a demo order to track immediately:' }),
    notFound: t('track_order.notFound', { defaultValue: 'We could not locate an order matching those details. Please check your credentials and try again.' }),
    orderStatusHeader: t('track_order.orderStatusHeader', { defaultValue: 'Fulfillment & Real-Time Tracking' }),
    backToStore: t('track_order.backToStore', { defaultValue: 'Back to Store' }),
    trackAnother: t('track_order.trackAnother', { defaultValue: 'Track Another Order' }),
    downloadInvoice: t('track_order.downloadInvoice', { defaultValue: 'Download Invoice' }),
    contactSupport: t('track_order.contactSupport', { defaultValue: 'Contact Support' }),
    
    // Order fields
    orderId: t('track_order.orderId', { defaultValue: 'Order Identifier' }),
    orderDate: t('track_order.orderDate', { defaultValue: 'Order Date' }),
    customerName: t('track_order.customerName', { defaultValue: 'Authorized Customer' }),
    paymentStatus: t('track_order.paymentStatus', { defaultValue: 'Payment Status' }),
    shippingStatus: t('track_order.shippingStatus', { defaultValue: 'Shipping Status' }),
    courierName: t('track_order.courierName', { defaultValue: 'Courier Partner' }),
    trackingNumber: t('track_order.trackingNumber', { defaultValue: 'Tracking Number' }),
    estDelivery: t('track_order.estDelivery', { defaultValue: 'Estimated Delivery' }),
    shippingAddress: t('track_order.shippingAddress', { defaultValue: 'Shipping Address' }),
    orderedProducts: t('track_order.orderedProducts', { defaultValue: 'Ordered Treasures' }),
    qty: t('track_order.qty', { defaultValue: 'Qty:' }),
    option: t('track_order.option', { defaultValue: 'Option:' }),
    totalAmount: t('track_order.totalAmount', { defaultValue: 'Total Certified Amount' }),
    subtotal: t('track_order.subtotal', { defaultValue: 'Subtotal' }),
    shippingFee: t('track_order.shippingFee', { defaultValue: 'Premium Shipping' }),
    discount: t('track_order.discount', { defaultValue: 'Applied Privileges' }),
    
    // Notification indicators
    emailSent: t('track_order.emailSent', { defaultValue: 'Fulfillment Email Shipped' }),
    invoiceSent: t('track_order.invoiceSent', { defaultValue: 'Tax Invoice Sent to Email' }),
    emailSentDesc: t('track_order.emailSentDesc', { defaultValue: 'Live tracking notifications are active for this order' }),
    invoiceSentDesc: t('track_order.invoiceSentDesc', { defaultValue: 'A formal certified digital invoice was emailed' }),

    // Timeline stages
    timeline: [
      { key: 'confirmed', label: t('track_order.timeline_confirmed_label', { defaultValue: isAr ? 'تأكيد الطلب' : 'Order Confirmed' }), desc: t('track_order.timeline_confirmed_desc', { defaultValue: isAr ? 'تم استلام وتوثيق طلبك بنجاح' : 'Order received and payment certified' }) },
      { key: 'preparing', label: t('track_order.timeline_preparing_label', { defaultValue: isAr ? 'قيد التحضير' : 'Preparing' }), desc: t('track_order.timeline_preparing_desc', { defaultValue: isAr ? 'نقوم بتحضير وتجهيز منتجاتك الفاخرة' : 'Assembling and fresh roasting products' }) },
      { key: 'packed', label: t('track_order.timeline_packed_label', { defaultValue: isAr ? 'تم التغليف' : 'Packed' }), desc: t('track_order.timeline_packed_desc', { defaultValue: isAr ? 'تم تغليف طلبك في صندوق زول المخملي' : 'Secured and packed in our velvet giftbox' }) },
      { key: 'shipped', label: t('track_order.timeline_shipped_label', { defaultValue: isAr ? 'تم الشحن' : 'Shipped' }), desc: t('track_order.timeline_shipped_desc', { defaultValue: isAr ? 'تم تسليم الشحنة لشركة التوصيل الفاخر' : 'Shipped with premium logistic courier' }) },
      { key: 'out_delivery', label: t('track_order.timeline_out_delivery_label', { defaultValue: isAr ? 'خارج للتوصيل' : 'Out for Delivery' }), desc: t('track_order.timeline_out_delivery_desc', { defaultValue: isAr ? 'المندوب قيد توصيل طلبك الآن لعنوانك' : 'Local courier is en route to your address' }) },
      { key: 'delivered', label: t('track_order.timeline_delivered_label', { defaultValue: isAr ? 'تم التوصيل' : 'Delivered' }), desc: t('track_order.timeline_delivered_desc', { defaultValue: isAr ? 'تم توصيل الروائع وتسليمها بسلام' : 'Fulfillment completed and package received' }) }
    ],

    // Invoice modal
    invoiceTitle: t('track_order.invoiceTitle', { defaultValue: 'ZOAL Certified Tax Invoice' }),
    invoiceSubtitle: t('track_order.invoiceSubtitle', { defaultValue: 'Official certified financial transaction receipt' }),
    vatNo: t('track_order.vatNo', { defaultValue: 'ZOAL VAT Registration: 310293847500003' }),
    invoiceNo: t('track_order.invoiceNo', { defaultValue: 'Invoice No:' }),
    billedTo: t('track_order.billedTo', { defaultValue: 'Billed To:' }),
    paymentMethod: t('track_order.paymentMethod', { defaultValue: 'Certified Payment:' }),
    itemCol: t('track_order.itemCol', { defaultValue: 'Premium Item / Selected Option' }),
    priceCol: t('track_order.priceCol', { defaultValue: 'Unit Price' }),
    qtyCol: t('track_order.qtyCol', { defaultValue: 'Quantity' }),
    totalCol: t('track_order.totalCol', { defaultValue: 'Total Amount' }),
    printInvoice: t('track_order.printInvoice', { defaultValue: 'Print Invoice' }),
    closeInvoice: t('track_order.closeInvoice', { defaultValue: 'Close Invoice' }),
    vatIncluded: t('track_order.vatIncluded', { defaultValue: 'Prices include 15% VAT where applicable' }),

    // Support Modal
    supportTitle: t('track_order.supportTitle', { defaultValue: 'ZOAL Support Panel' }),
    supportSubtitle: t('track_order.supportSubtitle', { defaultValue: 'Connect instantly with our support team' }),
    whatsappSupport: t('track_order.whatsappSupport', { defaultValue: 'Priority WhatsApp Support' }),
    callSupport: t('track_order.callSupport', { defaultValue: 'Direct Call Hotlines' }),
    emailSupport: t('track_order.emailSupport', { defaultValue: 'Business Priority Email' }),
    sendSupportMsg: t('track_order.sendSupportMsg', { defaultValue: 'Send Fast Support Message' }),
    supportFormName: t('track_order.supportFormName', { defaultValue: 'Full Name' }),
    supportFormSubject: t('track_order.supportFormSubject', { defaultValue: 'Subject / Question' }),
    supportFormMessage: t('track_order.supportFormMessage', { defaultValue: 'Message Details' }),
    supportFormSubmit: t('track_order.supportFormSubmit', { defaultValue: 'Submit Priority Request' }),
    supportSuccess: t('track_order.supportSuccess', { defaultValue: 'Your request was successfully prioritized. A support specialist will connect in under 15 minutes.' })
  };
  ...
  const handleTrackSubmit = (e?: React.FormEvent) => {
    ...
    if (!orderNumberInput.trim() || !emailInput.trim()) {
      setErrorMessage(t('track_order.errors.required_fields', { defaultValue: isAr ? 'يرجى ملء جميع الحقول المطلوبة للتتبع.' : 'Please enter both your order number and email address.' }));
      return;
    }
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      const mockInvoice = {
        invoiceNumber: `INV-${searchedOrder.id}`,
        invoiceDate: searchedOrder.date || new Date().toLocaleDateString(),
        merchantName: 'AL ZOAL Group',
        merchantVat: '310239485700003',
        orderId: searchedOrder.id,
        paymentId: `pay_${searchedOrder.id}`,
        gateway: searchedOrder.paymentMethod || 'Mada Card',
        transactionId: `txn_${searchedOrder.id}`,
        subtotal: searchedOrder.subtotal || searchedOrder.total / 1.15,
        vat: (searchedOrder as any).vat || (searchedOrder.total - (searchedOrder.total / 1.15)),
        discount: searchedOrder.discount || 0,
        delivery: searchedOrder.shipping || 0,
        total: searchedOrder.total,
        items: searchedOrder.items || []
      };
  ...
  const computedCourierName = useMemo(() => {
    ...
    if (searchedOrder.shipping === 0) {
      return t('track_order.courier.vip_courier', { defaultValue: isAr ? 'أسطول زول للمرسول الخاص' : 'ZOAL VIP Delivery Transport' });
    }
    return 'Aramex Priority Logistics';
  }, [searchedOrder, isAr]);
  ...
  return (
    ...
    >
          ...
          <div className="text-center space-y-4 mb-12 sm:mb-16">
            ...
            >
              ...
              <span className="text-[10px] font-mono tracking-[0.3em] font-bold text-gold-pure uppercase leading-none">
                {t('track_order.luxury_assurance', { defaultValue: isAr ? 'روائع الخياطة والضيافة الموثقة' : 'ZOAL LUXURY ASSURANCE' })}
              </span>
            </motion.div>
        ...
        <AnimatePresence mode="wait">
          {!searchedOrder ? (
            ...
            >
              ...
              {/* DIRECT QUICK SELECTION MATRIX FOR DEMOS OR RECENT ORDERS */}
              <div className="pt-6 border-t border-white/5 space-y-4">
                ...
                {import.meta.env.PROD ? (
                  orders.length === 0 ? (
                    /* Empty State */
                    <div className="p-8 border border-dashed border-white/10 rounded-xs text-center space-y-3 bg-black/40">
                      <ShoppingBag className="w-8 h-8 text-zinc-500 mx-auto animate-pulse" />
                      <div className="space-y-1">
                        <h4 className="text-white font-semibold text-sm uppercase font-display tracking-wider">{isAr ? 'لا توجد طلبات حديثة بعد' : 'No Recent Orders Yet'}</h4>
                        <p className="text-zinc-400 text-xs font-sans max-w-sm mx-auto">
                          {isAr ? 'ستظهر مشترياتك النشطة وحالة التوصيل هنا.' : 'Your active purchases and delivery status will appear here.'}
                        </p>
                      </div>
                      <button
                        onClick={() => setCurrentPage('store')}
                        className="px-6 py-2.5 bg-[#D4AF37] hover:bg-white text-black text-xs font-bold uppercase tracking-widest rounded-xs transition-colors cursor-pointer inline-block mt-2"
                      >
                        Start Shopping
                      </button>
                    </div>
                    ...
                    <div className="space-y-3">
                      <p className="text-xs text-zinc-400">
                        {t('track_order.select_recent', { defaultValue: isAr ? 'اختر أحد طلباتك للتتبع السريع:' : 'Select one of your recent orders to track immediately:' })}
                      </p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {orders.slice(0, 4).map((order) => (
                          <button
                            key={order.id}
                            onClick={() => selectDemoOrder(order.id, order.email)}
                          ...
                          >
                            <div className="space-y-1">
                              ...
                              <p className="text-[10px] text-zinc-500 font-sans tracking-tight tabular-nums-fix">
                                {order.date} • {formatCurrency(order.total)} SAR
                              </p>
                            </div>
                ...
                {searchedOrder.status === 'Cancelled' ? (
                  <div className="p-6 bg-red-950/20 border border-red-500/20 rounded-xs text-center space-y-3">
                    ...
                    <h3 className="text-base font-display text-white uppercase tracking-wider">
                      {t('track_order.cancelled_title', { defaultValue: isAr ? 'تم إلغاء هذا الطلب' : 'This Order has been Cancelled' })}
                    </h3>
                    <p className="text-xs text-zinc-400 max-w-md mx-auto">
                      {t('track_order.cancelled_desc', { defaultValue: isAr ? 'نأسف لإبلاغك بأن هذه المعاملة قد تم إلغاؤها. يرجى الاتصال بخدمة العملاء المميزة للحصول على المساعدة الفورية.' : 'We apologize, but this luxury purchase was cancelled. Please connect with our VIP Support Desk below for instant resolution.' })}
                    </p>
                  </div>
                ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-7 p-6 sm:p-8 rounded-sm bg-zinc-950/60 backdrop-blur-xl border border-white/10 space-y-6">
                  <h3 className="text-xs uppercase font-mono tracking-widest text-gold-pure border-b border-white/5 pb-3 font-semibold">
                    {t('track_order.fulfillment_records', { defaultValue: isAr ? 'تفاصيل ومعايير التنفيذ واللوجستيات' : 'Fulfillment Records & Logistics' })}
                  </h3>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-xs">
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="font-mono text-white block flex items-center gap-1.5">
                        ...
                        {searchedOrder.paymentMethod === 'Pay at Delivery' 
                          ? t('track_order.payment_cod', { defaultValue: isAr ? 'نقدي عند الاستلام (معلق)' : 'COD / Pending Receipt' }) 
                          : t('track_order.payment_prepaid', { defaultValue: isAr ? 'مدفوع إلكترونياً (مؤمن)' : 'Prepaid & Secured' })}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1 sm:col-span-2">
                      ...
                      <span className="font-sans text-white block font-semibold">
                        {computedEstDelivery} <span className="text-[10px] text-zinc-400 font-light">({t('track_order.delivery_slot', { defaultValue: isAr ? 'بين ٨:٠٠ صباحاً إلى ٨:٠٠ مساءً' : '08:00 AM - 08:00 PM slot' })})</span>
                      </span>
                    </div>
                ...
                <div className="md:col-span-5 p-6 sm:p-8 rounded-sm bg-zinc-950/60 backdrop-blur-xl border border-white/10 space-y-6 flex flex-col justify-between">
                    ...
                    <div className="space-y-4 max-h-[220px] overflow-y-auto divide-y divide-white/5 pr-1">
                      {searchedOrder.items.map((item, idx) => (
                        <div key={`${item.productId}-${idx}`} className="pt-3 first:pt-0 flex justify-between gap-3 text-xs">
                          ...
                          <span className="font-sans text-zinc-300 text-right shrink-0 tabular-nums-fix">
                            {formatCurrency(item.price * item.quantity)} <span className="text-[9px] text-zinc-500">{t('common.currency_short', { defaultValue: isAr ? 'ر.س' : 'SAR' })}</span>
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="border-t border-white/10 pt-4 space-y-2.5 font-mono text-[11px] text-zinc-400">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white tabular-nums-fix">{formatCurrency(searchedOrder.subtotal)} {t('common.currency', { defaultValue: isAr ? 'ريال' : 'SAR' })}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{tr.shippingFee}</span>
                      <span className="text-gold-pure tabular-nums-fix">{searchedOrder.shipping === 0 ? t('track_order.shipping_free', { defaultValue: isAr ? 'مستحق مجاني' : 'VIP Free' }) : `${formatCurrency(searchedOrder.shipping)} ${t('common.currency', { defaultValue: isAr ? 'ريال' : 'SAR' })}`}</span>
                    </div>
                    {searchedOrder.discount > 0 && (
                      <div className="flex justify-between text-gold-pure">
                        <span>{tr.discount}</span>
                        <span className="tabular-nums-fix">-{formatCurrency(searchedOrder.discount)} {t('common.currency', { defaultValue: isAr ? 'ريال' : 'SAR' })}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-xs font-bold text-white pt-2 border-t border-white/5 font-display">
                      ...
                      <span className="text-gold-pure font-sans text-sm tabular-nums-fix">{formatCurrency(searchedOrder.total)} {t('common.currency', { defaultValue: isAr ? 'ريال' : 'SAR' })}</span>
                    </div>
                  </div>
                </div>
            ...
            >
              ...
              <div id="zoal-printable-invoice" className="space-y-6 text-xs text-zinc-300">
                ...
                <div className="flex justify-between items-start gap-4 border-b border-white/10 pb-5">
                  ...
                  <div className="text-right ltr:text-right rtl:text-left space-y-1 font-mono">
                    <span className="px-3 py-1 bg-gold-pure/10 text-gold-pure font-bold text-[9px] rounded-full uppercase tracking-wider inline-block">
                      {t('track_order.tax_invoice_label', { defaultValue: isAr ? 'فاتورة ضريبية مبسطة' : 'Simplified Tax Invoice' })}
                    </span>
                ...
                <div className="grid grid-cols-2 gap-4 border-b border-white/5 pb-4 font-sans text-[11px]">
                  ...
                  <div className="space-y-1 text-right ltr:text-right rtl:text-left">
                    ...
                    <span className="text-[10px] font-mono uppercase tracking-widest text-gold-pure block mt-3">{t('track_order.carrier_waybill_label', { defaultValue: isAr ? 'رقم التتبع المرفق:' : 'Carrier Waybill:' })}</span>
                  ...
                  <div className="divide-y divide-white/5">
                    {searchedOrder.items.map((item, idx) => (
                      <div key={idx} className="grid grid-cols-12 gap-2 py-3 text-[11px]">
                        ...
                        <div className="col-span-2 text-right font-sans text-zinc-400 tabular-nums-fix">{formatCurrency(item.price)} {t('common.currency', { defaultValue: isAr ? 'ريال' : 'SAR' })}</div>
                        <div className="col-span-2 text-center font-sans text-zinc-400 tabular-nums-fix">{item.quantity}</div>
                        <div className="col-span-2 text-right font-sans text-white font-semibold tabular-nums-fix">{formatCurrency(item.price * item.quantity)} {t('common.currency', { defaultValue: isAr ? 'ريال' : 'SAR' })}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="border-t border-white/10 pt-4 flex flex-col items-end">
                  <div className="w-full sm:w-64 space-y-2 font-mono text-[11px] text-zinc-400">
                    ...
                    <div className="flex justify-between text-xs font-bold text-white pt-2 border-t border-white/5">
                      ...
                      <span className="text-gold-pure tabular-nums-fix">{formatCurrency(searchedOrder.total)} {t('common.currency', { defaultValue: isAr ? 'ريال' : 'SAR' })}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-white/5 text-center text-[9px] text-zinc-500 space-y-1">
                  ...
                  <p>ZOAL Business Headquarters, Al-Hofuf Main Avenue, Eastern Province, KSA.</p>
                  <p className="font-mono text-[8px] uppercase tracking-wider text-gold-pure/60 mt-2">{t('track_order.thank_you_note', { defaultValue: isAr ? 'نشكركم على اختيار روائع زول' : 'Thank you for choosing ZOAL Curated Masterpieces' })}</p>
                </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
                ...
                <button
                  ...
                  className="px-4.5 py-2.5 bg-black hover:bg-zinc-800 text-[#D4AF37] border border-[#D4AF37]/30 text-[10px] font-mono uppercase tracking-widest rounded-xs transition-colors cursor-pointer select-none flex items-center gap-1.5"
                >
      ...
      {/* SUPPORT INSTANT SLIDEOUT MODAL */}
      <AnimatePresence>
        {showSupportModal && (
          <motion.div 
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              ...
              {/* Instant Support Links */}
              <div className="space-y-3 pt-2">
                {/* Premium Support WhatsApp */}
                <a 
                  ...
                  <form onSubmit={handleSupportSubmit} className="space-y-4 text-xs">
                    ...
                    <div className="space-y-1">
                      ...
                      <textarea 
                        required 
                        rows={3} 
                        placeholder={t('track_order.support_form_placeholder', { defaultValue: isAr ? 'اكتب تفاصيل استفسارك هنا...' : 'Describe how we can assist with your delivery...' })} 
