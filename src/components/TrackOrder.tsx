    ...
    const loadOrders = async () => {
      ...
      if (!token) {
        if (!cancelled) {
          ...
          setLoading(false);
          setError('Authentication is required to view order tracking.');
        }
        return;
      }
      ...
      try {
        ...
        if (!cancelled) {
          setAuthoritativeOrders(rows.map(mapApiOrder));
          setError('');
        }
      } catch (err: any) {
    ...
    <section className={`space-y-5 ${isEmbedded ? '' : 'max-w-4xl mx-auto'}`} aria-label="Customer order tracking">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h2 className="text-xl font-semibold text-white mt-1">Track Orders</h2>
          <p className="text-xs text-zinc-500 mt-1">Tracking data is loaded from your authenticated orders.</p>
        </div>
        <button type="button" onClick={() => setCurrentPage('dashboard')} className="inline-flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 text-zinc-300 rounded-sm text-xs">
      ...
      <form onSubmit={handleSearch} className="bg-zinc-950 border border-white/5 rounded-sm p-4 flex flex-col sm:flex-row gap-2">
        <input
          ...
          aria-label="Order ID"
          className="flex-1 bg-black/30 border border-white/10 rounded-sm px-3 py-2.5 text-sm text-white outline-none focus:border-[#D4AF37]/50"
        />
        <button type="submit" disabled={loading || searching} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-[#D4AF37] text-black rounded-sm text-xs font-semibold disabled:opacity-50">
          <Search className="w-3.5 h-3.5" /> {searching ? 'Searching…' : 'Track Order'}
        </button>
      </form>
      ...
      {!loading && !selectedOrder && !error && (
        <div className="bg-zinc-950 border border-white/5 rounded-sm p-5 space-y-2">
          <p className="text-[10px] uppercase tracking-wider text-zinc-500">Your orders</p>
          {orders.length === 0 ? (
            <p className="text-sm text-zinc-500 py-4">No orders are available for this account.</p>
          ) : orders.map((order) => (
            <button key={order.id} type="button" onClick={() => { setOrderId(order.id); setSelectedOrder(order); }} className="w-full text-left flex items-center justify-between gap-3 py-3 border-t border-white/5 hover:bg-white/[0.02]">
              <span className="text-xs text-white font-medium">{order.id}</span>
              <span className="inline-flex items-center gap-1.5 text-xs text-zinc-400">{statusIcon(order.status)} {order.status}</span>
            </button>
          ))}
        </div>
      )}
      ...
      {selectedOrder && (
        <div className="bg-zinc-950 border border-white/5 rounded-sm p-5 space-y-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[9px] uppercase tracking-wider text-zinc-500">Order</p>
              <h3 className="text-lg font-semibold text-white mt-1">{selectedOrder.id}</h3>
            </div>
            <button type="button" onClick={reset} className="px-3 py-2 bg-white/5 border border-white/10 text-zinc-300 rounded-sm text-xs">Track another</button>
          </div>
          ...
          <div className="border-t border-white/5 pt-4">
            <p className="text-[9px] uppercase tracking-wider text-zinc-500 mb-2">Delivery information</p>
            <p className="text-sm text-zinc-300">{selectedOrder.address || 'Shipping address unavailable'}</p>
          </div>

          <div className="border border-amber-500/20 bg-amber-500/5 rounded-sm p-4 text-xs text-amber-200">
            Tracking events are shown only when the backend provides authoritative status/tracking data. No estimated or simulated timeline is generated by this component.
          </div>
        </div>
      )}
    </section>
