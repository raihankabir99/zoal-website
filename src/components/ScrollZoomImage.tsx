...
import { getFallbackImage, ABSOLUTE_PLACEHOLDER, isValidUploadedImageUrl } from '../imageRegistry';
...
export default function ScrollZoomImage({
  ...
  const [currentSrc, setCurrentSrc] = useState<string>(isValidUploadedImageUrl(src) ? src : getFallbackImage(src, category));
  const [errorCount, setErrorCount] = useState<number>(0);

  if (src !== prevSrc || category !== prevCategory) {
    setPrevSrc(src);
    setPrevCategory(category);
    setCurrentSrc(isValidUploadedImageUrl(src) ? src : getFallbackImage(src, category));
    setErrorCount(0);
  }
  ...
  // Instant Paint Logic: Eager loading and sync decoding for priority images
