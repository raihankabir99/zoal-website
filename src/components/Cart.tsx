              ...
              >
                ...
                <div className="flex items-center space-x-4 rtl:space-x-reverse">
                  <div className="w-20 h-20 bg-zinc-950 rounded-xs overflow-hidden border border-white/5 shrink-0">
                    <SafeImage product={item.product} alt={item.product.category === 'thobes' ? "ZOAL THOBES & MEN'S WEAR" : (i18n.language === 'ar' ? (item.product.title_ar || item.product.nameAr || t(`products.${item.product.id}.name`, { defaultValue: item.product.name })) : (item.product.nameEn || item.product.name))} className={item.product.category === 'market' ? "w-full h-full object-contain" : "w-full h-full object-cover"} containerClassName="w-full h-full relative" category={item.product.category} />
                  </div>
                  <div>
                    ...
                    <h3 className="text-white text-xs font-display uppercase tracking-wider font-semibold mt-0.5">{i18n.language === 'ar' ? (item.product.title_ar || item.product.nameAr || t(`products.${item.product.id}.name`, { defaultValue: item.product.name })) : (item.product.nameEn || item.product.name)}</h3>
                    {item.selectedOption && (
                      <span className="text-[10px] text-zinc-500 block font-sans">{t('cart.option_prefix', { defaultValue: 'Option:' })} {item.selectedOption}</span>
                    )}
                    <span className="text-[10.5px] font-sans text-gold-pure block mt-1 tabular-nums-fix">{formatCurrency(item.product.price)} {t('app.sar')}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-8">
                  ...
                  <div className="flex items-center border border-white/10 rounded overflow-hidden bg-black">
                    ...
                    <span className="px-3 py-1 font-sans text-xs text-white select-none tabular-nums-fix">{item.quantity}</span>
                    <button
                      onClick={() => onUpdateQuantity(item.product.id, 1)}
                      className="px-2.5 py-1 text-zinc-500 hover:text-white cursor-pointer"
                    >
