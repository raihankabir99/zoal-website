...
export default function EnterpriseInventoryManagement({
  ...
  // Warehouses state list populated from Supabase API
  ...
  const [newBatchWarehouse, setNewBatchWarehouse] = useState('Branch B Hub');
  ...
  const handleExportCSV = () => {
    try {
      ...
      link.setAttribute('download', `AL_ZOAL_Inventory_List_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <div className="space-y-6 text-left animate-fade-in pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-4">
        <div>
          ...
          <h1 className="text-xl md:text-2xl font-bold tracking-widest font-display uppercase text-white flex items-center gap-2">
            <Package className="w-5 h-5 text-gold-pure" /> INVENTORY MANAGEMENT
          </h1>
        </div>
      ...
      <div className="flex border-b border-white/5 overflow-x-auto scrollbar-none gap-2 pb-px font-mono text-[10px]">
        {[
          ...
          { id: 'list', label: 'STOCK LIST', icon: ClipboardList },
          { id: 'adjustment', label: 'STOCK MOVEMENTS', icon: Sliders },
          ...
          { id: 'barcode', label: 'BARCODE & SCANNER', icon: QrCode }
        ].map((tab) => {
          const Icon = tab.icon;
      ...
      {loading ? (
        <div className="space-y-6">
          ...
          <div className="bg-zinc-950/40 border border-white/5 h-80 rounded-xs animate-pulse flex items-center justify-center">
            <span className="text-zinc-500 font-mono text-xs uppercase tracking-widest animate-pulse">Loading Vault Indices...</span>
          </div>
        </div>
      ) : (
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-zinc-950/60 border border-white/5 p-4 rounded-xs hover:border-gold-pure/30 transition-all">
                  ...
                  <span className="text-[9px] font-sans text-zinc-400 block mt-0.5">Active catalog</span>
                </div>
              ...
              <div className="bg-zinc-950 border border-white/5 p-4 rounded-xs space-y-4">
                <div className="flex flex-col lg:flex-row gap-4 justify-between items-stretch lg:items-center">
                  ...
                  <div className="flex items-center gap-2 bg-black border border-white/5 px-3 py-1.5 rounded-xs w-full lg:w-96">
                    ...
                    <input
                      type="text"
                      placeholder="Search stock list by product, SKU, barcode, location..."
                      value={searchQuery}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      className="py-1.5 px-3 bg-zinc-900 border border-white/10 hover:border-gold-pure text-white rounded-xs text-[9px] uppercase tracking-widest font-bold cursor-pointer transition-all flex items-center gap-1.5"
                    >
                      <Download className="w-3 h-3 text-gold-pure" /> Export List
                    </button>
              ...
              {showImportArea && (
                <div className="bg-zinc-950 border border-gold-pure/20 p-4 rounded-xs space-y-3">
                  <div className="flex justify-between items-center border-b border-white/5 pb-2">
                    <h3 className="text-xs uppercase font-mono text-gold-pure tracking-widest font-bold">BULK CSV IMPORT PANEL</h3>
              ...
              <div className="space-y-6">
                <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-3 font-mono text-[10px]">
                  ...
                  <div className="space-y-2.5 text-zinc-400">
                    ...
                    <p>• <strong className="text-white">RESERVATIONS:</strong> Handled in real time by client side integrations with active orders list.</p>
                  </div>
                </div>
          ...
          {activeTab === 'logs' && (
            ...
            >
              <div className="bg-zinc-950 border border-white/5 p-4 rounded-xs flex flex-col md:flex-row md:items-center justify-between gap-4 font-mono text-[10px]">
                <div>
                  <h3 className="text-xs uppercase text-gold-pure tracking-widest font-bold">FORENSIC INVENTORY AUDIT TRAIL</h3>
                  <p className="text-[9.5px] text-zinc-500 mt-1">Immutable list tracking every stock adjustment, shipment received, order reserved, or return recorded.</p>
                </div>
          ...
          {activeTab === 'barcode' && (
            ...
            >
              ...
              <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4 font-mono text-[10px]">
                <h3 className="text-xs uppercase text-gold-pure tracking-widest font-bold">THERMAL LABEL CREATOR</h3>
                ...
                {selectedProduct ? (
                  <div className="space-y-4">
                    ...
                    <div id="barcode-thermal-sticker" className="bg-white text-black p-4 rounded-xs border-2 border-black max-w-xs mx-auto space-y-3 text-center">
                      ...
                      <div className="text-[6.5px] text-zinc-500 border-t border-black/10 pt-1.5 uppercase font-sans tracking-wider">
                        Logistics Item Verification Label
                      </div>
                    </div>
                  ...
                  <div className="border border-dashed border-white/15 p-8 text-center text-zinc-500 space-y-2 rounded-xs">
                    ...
                    <p className="text-[9px] font-sans">Select a product from the Stock List tab or simulate a USB scan to generate a physical barcode label template.</p>
                  </div>
                )}
              </div>
            </motion.div>
      ...
      {showEditModal && selectedProduct && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div>
              <span className="text-[8px] tracking-widest text-gold-pure uppercase font-bold">STOCK OVERRIDES</span>
                ...
                <div className="space-y-1">
                  ...
                  <select
                    value={warehouses.some(w => editWarehouse.startsWith(w.name) || editWarehouse.startsWith(w.code)) 
                      ? (warehouses.find(w => editWarehouse.startsWith(w.name) || editWarehouse.startsWith(w.code))?.name || 'Branch B Hub')
                      : 'Branch B Hub'
                    }
                    onChange={(e) => {
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const whName = editWarehouse.split(' - ')[0] || 'Branch B Hub';
      ...
      {showBarcodePrintModal && selectedProduct && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div>
              <span className="text-[8px] tracking-widest text-gold-pure uppercase font-bold">BARCODE PRINT SPOOLER</span>
              <h3 className="text-sm font-bold text-white uppercase font-sans mt-0.5">{selectedProduct.name}</h3>
            </div>
...
function ReportsTab({ products, warehouses, transactions, batches }: ReportsTabProps) {
  ...
  const reportData = useMemo(() => {
    ...
    if (activeReport === 'damaged') {
      ...
      return [
        { id: 'DMG-102', productName: 'Imperial Dark Roast', sku: 'ALZ-COF-001', warehouse: 'Branch B Hub', qty: 4, reason: 'Moisture bag seal rupture', timestamp: '2026-07-10' },
        { id: 'DMG-104', productName: 'Saffron Cardamom Brioche', sku: 'ALZ-BAK-002', warehouse: 'Al Hofuf Gourmet Kitchen', qty: 2, reason: 'Transit container crushing', timestamp: '2026-07-15' }
      ];
    }
    ...
    <div className="space-y-6">
      ...
      {exporting && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex flex-col items-center justify-center p-6 text-center space-y-4 font-mono text-xs">
          ...
          <div className="space-y-1">
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest block">Cloud compiler active</span>
      ...
      <div className="flex flex-col lg:flex-row justify-between lg:items-center bg-zinc-950 border border-white/5 p-4 rounded-xs gap-4">
        <div>
          <h3 className="text-xs font-mono uppercase text-gold-pure tracking-widest font-bold">REPORT ENGINE</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 font-mono text-[10px]">
        {[
          { id: 'current_stock', label: 'Current Stock list', desc: 'Realtime SKU quantity levels' },
          ...
          { id: 'valuation', label: 'Capital Valuation', desc: 'Inventory equity list' },
      ...
      <div className="bg-zinc-950 border border-white/5 rounded-xs overflow-hidden text-[11px] font-mono">
        <div className="bg-white/[0.02] border-b border-white/5 py-2.5 px-4 flex justify-between items-center">
          <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">REALTIME STATEMENT PREVIEW</span>
