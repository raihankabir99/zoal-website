...
import { contactDetails, TermsSection } from '../data/termsData';
...
export default function TermsAndConditions() {
  ...
  // Fetch Published Terms Document from Legal API
  useEffect(() => {
    let isMounted = true;
    setIsLoading(true);
    setApiError(null);
    ...
    fetch('/api/legal/documents/terms')
      .then(async (res) => {
        if (!res.ok) {
          throw new Error(`Failed to load legal terms document (HTTP ${res.status})`);
        }
        return res.json();
      })
      .then((doc) => {
        ...
        if (!currentVersion || currentVersion.status !== 'Published') {
          setApiError(isAr ? 'وثيقة الشروط والأحكام غير متاحة حالياً.' : 'The Terms & Conditions document is currently unavailable.');
          setIsLoading(false);
          return;
        }

        setDocumentTitle(doc.title || 'Terms & Conditions');
        ...
        // Parse content
        let parsedSections: TermsSection[] = [];
          ...
          parsedSections = [
            {
              id: 'terms-body',
              title: {
                en: doc.title || 'Terms & Conditions',
                ar: doc.title || 'الشروط والأحكام'
              },
              icon: 'Scale',
              content: {
        ...
        setTermsSections(parsedSections);
      ...
      .catch((err) => {
        ...
        setApiError(isAr ? 'عذراً، حدث خطأ أثناء تحميل وثيقة الشروط والأحكام.' : 'Sorry, an error occurred while loading the Terms & Conditions document.');
        setIsLoading(false);
      });
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        {isLoading && (
          <div className="py-20 flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-xs font-mono tracking-widest uppercase text-zinc-400">
              {isAr ? 'جاري التحقق من الوثائق الرسمية...' : 'Verifying Official Legal Document...'}
            </p>
          </div>
        )}
        ...
        {!isLoading && !apiError && termsSections.length > 0 && (
          <>
