  ...
  return (
    <div className="space-y-6 text-left animate-fade-in font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
        <div>
          <span className="text-[9px] tracking-[0.4em] text-gold-pure uppercase font-mono block mb-1">ZOAL CONVERSIONS ENGINE</span>
          <h2 className="text-xl font-bold tracking-widest font-display uppercase text-white">VIP CAMPAIGNS & MARKETING</h2>
        </div>
      </div>
      ...
      {marketingSubTab === 'campaigns' && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-zinc-950 border border-white/5 p-4 rounded-xs">
              <span className="text-[8px] font-mono uppercase text-zinc-500 block">Total Issued Coupons</span>
            ...
            <div className="bg-zinc-950 border border-white/5 p-4 rounded-xs">
              <span className="text-[8px] font-mono uppercase text-zinc-500 block">Patron Subscribers</span>
          ...
          <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4">
            <div className="flex justify-between items-center border-b border-white/5 pb-2">
              <h3 className="text-xs uppercase font-mono text-gold-pure tracking-widest font-bold flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-gold-pure" /> ACTIVE DISCOUNT CAMPAIGNS
              </h3>
              <button 
              ...
              <table className="w-full text-left border-collapse text-[10px] font-mono">
                <thead>
                  <tr className="border-b border-white/5 text-zinc-500 text-[8px] uppercase tracking-wider">
                    ...
                    <th className="pb-2">Category Link</th>
          ...
          <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4">
            <div className="flex justify-between items-center border-b border-white/5 pb-2">
              <h3 className="text-xs uppercase font-mono text-gold-pure tracking-widest font-bold flex items-center gap-1.5">
                <Gift className="w-3.5 h-3.5 text-gold-pure" /> PROMO COUPONS
              </h3>
              <button 
                onClick={() => {
                  ...
                  addLog(`Created Promo Coupon: ${newC.code}`, "Coupon Registry");
                ...
                <div key={c.id} className="p-4 bg-black/40 border border-white/5 rounded-xs font-mono text-[10px] space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <span className="text-[8.5px] text-zinc-500 block">Voucher Code Level</span>
                    </div>
                  ...
                  <div className="space-y-1 text-zinc-400 text-[9px]">
                    <p>• Usage Count Index: {c.usedCount} of {c.limit} used</p>
                    <p>• Verification Expiry: {c.expiry}</p>
                  </div>
                  <button 
                    onClick={() => {
                      if (window.confirm(`Delete coupon "${c.code}"?`)) {
                        setCoupons(prev => prev.filter(x => x.id !== c.id));
                        addLog(`Erased Promo Coupon: ${c.code}`, "Coupon Registry");
                      }
                    }}
                          ...
                          <td className="py-2.5 text-right">
                            <button
                              onClick={() => {
                                ...
                                addLog(`Toggled homepage promotion status for product: ${p.name}`, "Product Registry");
                              }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4 flex flex-col justify-between">
              ...
              <div className="pt-4 border-t border-white/5 space-y-2">
                <span className="text-[8px] font-mono uppercase text-gold-pure tracking-widest block font-bold">Subscribers List Registry</span>
                      ...
                      <button 
                        onClick={() => {
                          if (window.confirm(`Unsubscribe "${sub.email}"?`)) {
                            ...
                            addLog(`Unsubscribed subscriber email: ${sub.email}`, "Patron Campaign");
                          }
                        }}
