                      ...
                      <td className="py-3 text-center">
                        <button
                          onClick={async () => {
                            ...
                            try {
                              const res = await fetch(`/api/marketing/campaigns/${camp.id}`, {
                                method: 'PUT',
                              ...
                              alert(`Failed to update campaign status: ${e.message}`);
                            }
                          }}
              ...
              <button 
                onClick={async () => {
                  ...
                  try {
                    ...
                    setCoupons(prev => [newC, ...prev.filter(x => x.id !== newC.id)]);
                  ...
                  <button 
                    onClick={async () => {
                      if (window.confirm(`Delete coupon "${c.code}"?`)) {
                        try {
                          const res = await fetch(`/api/marketing/coupons/${c.id}`, {
                            method: 'DELETE'
                          });
                          if (!res.ok) {
                            const err = await res.json().catch(() => ({}));
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await fetch(`/api/homepage-heroes/${b.id}`, {
                              ...
                              body: JSON.stringify({ active: nextStatus === 'active' })
                            });
                            if (!res.ok) {
                              const err = await res.json().catch(() => ({}));
                              throw new Error(err.error || `Server error ${res.status}`);
                            ...
                            addLog(`Updated Banner Slider "${b.title}" Status in database to ${nextStatus}`, "CMS Slider");
                          } catch (err: any) {
                            console.warn('Failed to toggle banner status in database:', err.message);
                            setBanners(prev => prev.map(x => x.id === b.id ? { ...x, status: nextStatus } : x));
                            addLog(`Updated Banner Slider "${b.title}" Status locally to ${nextStatus}`, "CMS Slider");
                          }
                        }}
                        ...
                        onClick={async () => {
                          if (window.confirm(`Delete slider banner "${b.title}"?`)) {
                            ...
                            try {
                              ...
                              addLog(`Deleted Banner Slider from database: ${b.title}`, "CMS Slider");
                            } catch (err: any) {
                              console.warn('Failed to delete banner from database:', err.message);
                              setBanners(prev => prev.filter(x => x.id !== b.id));
                              addLog(`Deleted Banner Slider locally: ${b.title}`, "CMS Slider");
                            }
                          }
                        }}
                        ...
                        <tr key={p.id} className="hover:bg-white/[0.02]">
                          <td className="py-2.5 flex items-center gap-2.5">
                            <div className="w-7 h-7 bg-zinc-900 border border-white/5 overflow-hidden rounded-xs flex-shrink-0">
                              <img 
                                src={p.images?.[0] || 'https://images.unsplash.com/photo-1583391733956-3750e0ff4e8b?auto=format&fit=crop&q=80&w=100'} 
                                alt={p.name} 
                                referrerPolicy="no-referrer"
          ...
          <div className="bg-zinc-950 border border-white/10 rounded-sm max-w-md w-full p-6 space-y-6 relative">
            ...
            <form onSubmit={async (e) => {
              ...
              try {
                ...
                setCampaigns(prev => [newCamp, ...prev.filter(x => x.id !== newCamp.id)]);
          ...
          <div className="bg-zinc-950 border border-white/10 rounded-sm max-w-md w-full p-6 space-y-6 relative">
            ...
            <form onSubmit={async (e) => {
              ...
              const payload = {
                hero_title: title,
                hero_title_ar: title,
                hero_subtitle: 'PROMOTION',
                hero_subtitle_ar: 'عرض ترويجي',
                hero_description: 'Exclusive custom banner created from management portal.',
                hero_description_ar: 'شعار مخصص تم إنشاؤه من لوحة التحكم الإدارية.',
              ...
              try {
                ...
                const newBan = {
                  id: newHero.id,
                  title: newHero.hero_title,
                  image: newHero.hero_image_desktop,
                  link: newHero.cta_link,
                  status: newHero.active ? 'active' : 'inactive'
                };
                setBanners(prev => [...prev, newBan]);
                addLog(`Uploaded Promotional Banner to database: ${title}`, "CMS Slider");
                setIsAddBannerOpen(false);
                alert(`Hero banner "${title}" successfully saved to database!`);
              } catch (err: any) {
                console.warn('Failed to save banner to database:', err.message);
                const newBan = {
                  id: `ban-${Date.now()}`,
                  title,
                  image: img,
                ...
                setBanners(prev => [...prev, newBan]);
                addLog(`Uploaded Promotional Banner locally: ${title}`, "CMS Slider");
                setIsAddBannerOpen(false);
                alert(`Hero banner "${title}" added locally.`);
              }
            }} className="space-y-4 text-[10px]">
              <div className="space-y-1">
