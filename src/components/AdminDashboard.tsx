...
import { saveProductToSupabase, deleteProductFromSupabase } from '../lib/productSync';
...
export default function AdminDashboard({
  ...
  useEffect(() => {
    fetch('/api/marketing')
      ...
      .catch(err => console.error('Failed to fetch marketing data:', err));
  }, []);
  ...
  const [notifications, setNotifications] = useState<any[]>(() => {
    ...
    return [
      { id: 'notif-1', title: 'New Order Placed', message: 'VIP Order #ZL-4491 placed by Customer Amna Al-Saeed for 350 SAR.', time: '5 mins ago', type: 'success', category: 'new_order', status: 'unread' },
      ...
      { id: 'notif-4', title: 'Refund Request Issued', message: 'Customer alzoal3003@gmail.com has requested a refund for Order #ZL-9543 (420 SAR).', time: '3 hours ago', type: 'warning', category: 'refund_request', status: 'unread' },
      { id: 'notif-5', title: 'New Customer Registered', message: 'Verified elite account created for Customer Khalid bin Al-Waleed.', time: '5 hours ago', type: 'info', category: 'new_customer', status: 'unread' },
  ...
  // Bulk Imports - Production implementation
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        const importedProducts = data.map((row, index) => ({
          id: `custom-prod-imported-${Date.now()}-${index}`,
          name: row.name || row.Name || 'Imported Product',
          nameEn: row.nameEn || row.NameEn || row.name || row.Name || 'Imported Product',
          ...
          category: (row.category || row.Category || 'market') as BusinessCategory,
          brand: row.brand || row.Brand || 'AL ZOAL Specialty Roasters',
          images: row.images ? row.images.split(',') : row.Images ? row.Images.split(',') : [],
          sku: row.sku || row.SKU || `ZL-SKU-${Date.now()}-${index}`,
          barcode: row.barcode || row.Barcode || `628${Date.now()}-${index}`,
        ...
        addLog('Product Imported', `Imported ${importedProducts.length} products from ${file.name}`);
        alert(`Import completed successfully! (${importedProducts.length} products imported)`);
      ...
      <main className="flex-grow flex flex-col overflow-x-hidden min-h-screen">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative group/notif-panel">
              ...
              <div className="absolute right-0 top-full mt-1.5 w-80 bg-zinc-950 border border-white/10 rounded-xs shadow-[0_12px_40px_rgba(0,0,0,0.95)] opacity-0 pointer-events-none group-hover/notif-panel:opacity-100 group-hover/notif-panel:pointer-events-auto transition-all duration-300 z-50 p-3 text-left">
                <div className="flex items-center justify-between border-b border-white/5 pb-2 mb-2">
                  <span className="text-[9px] uppercase tracking-widest font-bold text-gold-pure">Notifications</span>
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                ...
                <span className="text-[8px] font-mono text-gold-pure block tracking-widest uppercase mt-1">
                  {currentUser?.role === 'owner' ? 'Owner' : 'Administrator'}
                </span>
              </div>
        ...
        <div className="flex-grow p-6 space-y-8 overflow-y-auto">
            ...
            <div className="space-y-8 animate-fade-in">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
                <div>
                  ...
                  <h2 className="text-xl sm:text-2xl font-bold tracking-widest font-display uppercase text-white">OWNER / ADMIN DASHBOARD</h2>
                </div>
                ...
                <div className="flex gap-2 font-mono text-[9px]">
                  ...
                  <button
                    onClick={() => {
                      ...
                      setNotifications([
                        { id: `notif-${Date.now()}-1`, title: 'New Order Placed', message: 'VIP Order #ZL-4491 placed by Customer Amna Al-Saeed for 350 SAR.', time: 'Just now', type: 'success', category: 'new_order', status: 'unread' },
                        ...
                        { id: `notif-${Date.now()}-4`, title: 'Refund Request Issued', message: 'Customer alzoal3003@gmail.com has requested a refund for Order #ZL-9543 (420 SAR).', time: '1 hour ago', type: 'warning', category: 'refund_request', status: 'unread' },
                        { id: `notif-${Date.now()}-5`, title: 'New Customer Registered', message: 'Verified elite account created for Customer Khalid bin Al-Waleed.', time: '3 hours ago', type: 'info', category: 'new_customer', status: 'unread' },
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (picked === 'new_order') {
                          ...
                          message = "Customer has requested immediate refund for Order #ZL-4412 due to courier delays.";
                          type = "warning";
                        } else if (picked === 'new_customer') {
                          title = "New Customer Registered";
                          message = "Premium clientele record added for Ambassador Al-Sabah.";
                          type = "info";
                        } else if (picked === 'payment_failed') {
                          title = "Payment Failed";
                          message = "SADAD Panel rejected bank transfer transaction #TX-90231 (Authentication Error).";
                          type = "error";
                ...
                {((window as any)._settingsActiveSub || 'profile') === 'profile' && (
                  <div className="space-y-4 animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono">
                      ...
                      <div className="space-y-1">
                        <label className="text-[8.5px] uppercase text-zinc-500">Store Logo URL</label>
                        <input 
                          type="text" 
                      ...
                      <div className="space-y-1">
                        <label className="text-[8.5px] uppercase text-zinc-500">Favicon URL</label>
                        <input 
                          type="text" 
          ...
          {activeTab === 'profile' && (
            <div className="space-y-6 text-left animate-fade-in font-sans">
              <div className="border-b border-white/5 pb-4">
                <span className="text-[9px] tracking-[0.4em] text-gold-pure uppercase font-mono block mb-1">{(currentUser?.role === 'manager' || currentUser?.role === 'manager') ? 'MANAGER' : 'ADMIN'} PROFILE</span>
                        ...
                        {coupons.map((c: any) => (
                          <tr key={c.id} className="hover:bg-white/1">
                            ...
                            <td className="py-3 px-2">
                              <button
                                onClick={() => {
                                  if (window.confirm(`Revoke coupon code ${c.code}?`)) {
          ...
          {activeTab === 'shipping' && (
            <div className="space-y-6 text-left animate-fade-in font-sans">
              <div className="border-b border-white/5 pb-4">
                <span className="text-[9px] tracking-[0.4em] text-gold-pure uppercase font-mono block mb-1">LOGISTICS CHANNELS</span>
                <h2 className="text-xl font-bold tracking-widest font-display uppercase text-white">SOVEREIGN LOGISTICS & DISPATCH HUB</h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-zinc-950 border border-white/5 p-6 rounded-xs space-y-4">
                  <h3 className="text-white text-xs font-display uppercase tracking-widest border-b border-white/5 pb-2">Active Logistics Carriers</h3>
                  <div className="space-y-3 font-mono text-xs">
                    {[
                      { name: 'Aramex VIP Courier', cost: '35 SAR', speed: '24-48 Hours', zone: 'National wide' },
                      { name: 'SMSA Express Premium', cost: '40 SAR', speed: 'Next Day Delivery', zone: 'Major Saudi Cities' },
                      { name: 'Saudi Post (SPL) Standard', cost: '25 SAR', speed: '3-5 Working Days', zone: 'All Regions' }
                    ].map((carrier, i) => (
                      <div key={i} className="p-3 bg-black border border-white/5 rounded-xs space-y-2">
                        <div className="flex justify-between text-white font-bold">
                          <span>{carrier.name}</span>
                          <span className="text-gold-pure">{carrier.cost}</span>
                        </div>
                        <div className="flex justify-between text-[10px] text-zinc-500">
                          <span>Transit: {carrier.speed}</span>
                          <span>Range: {carrier.zone}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="bg-zinc-950 border border-white/5 p-6 rounded-xs col-span-2 space-y-4">
                  <h3 className="text-white text-xs font-display uppercase tracking-widest border-b border-white/5 pb-2">Parcel Transit Density Radar</h3>
                  <div className="grid grid-cols-3 gap-4 text-center font-mono">
                    <div className="p-4 bg-black border border-white/5 rounded-xs">
                      <span className="text-zinc-500 text-[10px] block">DISPATCHED COURIERS</span>
                      <span className="text-3xl text-white font-bold block mt-2">48</span>
                      <span className="text-[9px] text-gold-pure block mt-1">Active Transit</span>
                    </div>
                    <div className="p-4 bg-black border border-white/5 rounded-xs">
                      <span className="text-zinc-500 text-[10px] block">DELIVERED SUCCESS</span>
                      <span className="text-3xl text-white font-bold block mt-2">954</span>
                      <span className="text-[9px] text-emerald-400 block mt-1">Completed Yield</span>
                    </div>
                    <div className="p-4 bg-black border border-white/5 rounded-xs">
                      <span className="text-zinc-500 text-[10px] block">RETURN PROTOCOLS</span>
                      <span className="text-3xl text-white font-bold block mt-2">3</span>
                      <span className="text-[9px] text-rose-400 block mt-1">Returned Safe</span>
                    </div>
                  </div>
                  <div className="p-4 bg-black border border-white/5 rounded-xs text-xs text-zinc-400 space-y-2">
                    <h4 className="text-white font-bold uppercase text-[10px]">Unified Tracking Dispatcher</h4>
                    <p>All online checkouts generate direct webhook calls to SMSA and Aramex. Digital waybills are synchronized automatically, sending SMS alerts to the client with the tracking code.</p>
                  </div>
                </div>
              </div>
            </div>
