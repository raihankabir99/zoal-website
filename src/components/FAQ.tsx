...
const getCategoryIcon = (iconName: string) => {
  switch (iconName) {
    case 'ShoppingBag': return <ShoppingBag className="w-4 h-4 sm:w-5 h-5 shrink-0" />;
    case 'CreditCard': return <CreditCard className="w-4 h-4 sm:w-5 h-5 shrink-0" />;
    case 'Truck': return <Truck className="w-4 h-4 sm:w-5 h-5 shrink-0" />;
    case 'Coffee': return <Coffee className="w-4 h-4 sm:w-5 h-5 shrink-0" />;
    case 'RefreshCw': return <RefreshCw className="w-4 h-4 sm:w-5 h-5 shrink-0" />;
    default: return <HelpCircle className="w-4 h-4 sm:w-5 h-5 shrink-0" />;
  }
};
...
export default function FAQ({ setCurrentPage }: FAQProps) {
  ...
  return (
    <div dir={isAr ? 'rtl' : 'ltr'} className="bg-black text-white min-h-screen pt-20 sm:pt-32 pb-12 sm:pb-16 px-2 sm:px-6 lg:px-8 font-sans selection:bg-gold-pure selection:text-black overflow-hidden relative">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gold-pure/5 border border-gold-pure/15">
            ...
            <span className="text-[9px] uppercase tracking-[0.3em] font-display font-semibold text-gold-pure">
              {t('faq.badge', { defaultValue: isAr ? 'مركز المساعدة الفاخر' : 'Luxury Support Center' })}
            </span>
          </div>
        </motion.div>
        ...
        <div className="text-center mb-6 sm:mb-16">
          <motion.h1 
            ...
            className="text-2xl sm:text-4xl font-display font-medium text-white tracking-[0.2em] uppercase mb-2 sm:mb-4"
          >
            {t('faq.title', { defaultValue: isAr ? 'الأسئلة الشائعة' : 'Frequently Asked Questions' })}
          </motion.h1>
          ...
          >
            {t('faq.subtitle', { defaultValue: isAr ? 'اعثر على إجابات وافية لأكثر الأسئلة تكراراً حول طريقتنا الفاخرة في التسوق، المدفوعات الآمنة، سياسات التوصيل، الإرجاع، وتفاصيل حسابك في زول.' : 'Find answers to the most common questions about shopping, payments, delivery, returns, and your ZOAL account.' })}
          </motion.p>
        </div>
        ...
        <motion.div 
          ...
          className="mb-6 sm:mb-10 max-w-xl mx-auto"
        >
          <div className="relative group">
            <div className="absolute inset-y-0 start-4 flex items-center pointer-events-none text-zinc-500 group-focus-within:text-gold-pure transition-colors">
            ...
            <input
              ...
              placeholder={t('faq.search_placeholder', { defaultValue: isAr ? 'ابحث عن سؤالك...' : 'Search your question...' })}
              className="w-full ps-11 pe-11 py-3.5 bg-[#050505] border border-white/5 hover:border-white/10 focus:border-[#D4AF37]/50 focus:outline-none rounded-sm text-xs sm:text-sm text-white placeholder-zinc-500 font-sans tracking-wide transition-all duration-300 shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)]"
            />
            {searchQuery && (
              <button 
                onClick={() => setSearchQuery('')}
                className="absolute end-4 inset-y-0 text-zinc-500 hover:text-white text-xs tracking-widest uppercase transition-colors"
              >
                {t('faq.clear', { defaultValue: isAr ? 'مسح' : 'Clear' })}
              </button>
            )}
          </div>
        </motion.div>
        ...
        <motion.div 
          ...
          className="grid grid-cols-2 sm:flex sm:flex-wrap sm:items-center sm:justify-center gap-2 sm:gap-3 mb-6 sm:mb-10 border-b border-white/5 pb-6"
        >
          <button
            onClick={() => {
            ...
            className={`px-3 py-2 sm:px-4 sm:py-2 rounded-full text-[10px] uppercase tracking-wider font-mono transition-all duration-300 border cursor-pointer select-none flex items-center justify-center text-center w-full sm:w-auto ${
          ...
          >
            {t('faq.all', { defaultValue: isAr ? 'الكل' : 'All' })}
          </button>
          ...
          {faqCategories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-2 sm:px-4 sm:py-2 rounded-full text-[10px] uppercase tracking-wider font-mono transition-all duration-300 border flex items-center justify-center gap-1.5 cursor-pointer select-none w-full sm:w-auto ${
                  isActive 
                    ? 'bg-[#D4AF37] text-black border-[#D4AF37] font-semibold shadow-[0_4px_12px_rgba(212,175,55,0.2)]' 
                    : 'bg-zinc-950/40 text-zinc-400 border-white/5 hover:text-white hover:border-white/20'
                }`}
              >
                ...
                <span className="truncate">{isAr ? cat.name.ar : cat.name.en}</span>
              </button>
            );
          })}
        </motion.div>
        ...
        <div className="space-y-2 sm:space-y-4 mb-10 sm:mb-16 min-h-[200px]">
            ...
            {filteredFAQs.length > 0 ? (
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full p-3 sm:p-5 flex items-center justify-between text-start focus:outline-none focus:ring-1 focus:ring-[#D4AF37]/35 cursor-pointer select-none"
                      aria-expanded={isExpanded}
                      id={`faq-btn-${item.id}`}
                    >
                      <span className={`text-xs sm:text-sm font-display uppercase tracking-wider font-semibold transition-colors duration-300 text-start leading-tight ${
                    ...
                    <AnimatePresence initial={false}>
                      {isExpanded && (
                        ...
                        >
                          <div className="px-3 sm:px-5 pb-4 sm:pb-5 pt-1 text-zinc-400 text-xs sm:text-[13px] leading-relaxed border-t border-white/5 bg-zinc-950/20 font-sans">
              ...
              >
                ...
                <h3 className="text-zinc-300 font-display uppercase tracking-widest text-xs mb-1 font-semibold">
                  {t('faq.no_results', { defaultValue: isAr ? 'لا توجد نتائج مطابقة' : 'No matching questions found' })}
                </h3>
                <p className="text-zinc-500 text-[11px] max-w-xs mx-auto">
                  {t('faq.no_results_desc', { defaultValue: isAr ? 'حاول إعادة صياغة البحث أو تصفح الأقسام للعثور على الإجابة.' : 'Try clarifying your search terms or exploring the category filters above.' })}
                </p>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <motion.div 
          ...
          className="bg-gradient-to-b from-zinc-950/80 to-black border border-[#D4AF37]/25 px-4 py-6 sm:p-10 rounded-sm text-center relative overflow-hidden shadow-[0_15px_40px_rgba(0,0,0,0.8)] mb-8 sm:mb-12"
        >
          {/* Subtle gold frames */}
          ...
          <h3 className="text-sm sm:text-base font-display font-semibold uppercase tracking-widest text-white mb-2">
            {t('faq.still_need_help', { defaultValue: isAr ? 'هل ما زلت بحاجة إلى مساعدة؟' : 'Still need help?' })}
          </h3>
          <p className="text-zinc-400 text-xs max-w-xs sm:max-w-md mx-auto mb-6 sm:mb-8 leading-relaxed">
            {t('faq.still_need_help_desc', { defaultValue: isAr ? 'إذا لم تتمكن من العثور على إجابتك هنا، يسعدنا جداً تقديم مساعدة مخصصة عبر قنوات الدعم الرسمية لدينا.' : "If you couldn't find your answer, our support team is happy to assist you." })}
          </p>

          <div className="flex flex-col sm:grid sm:grid-cols-3 gap-3.5 max-w-2xl mx-auto">
            ...
            <button
              ...
              className="w-full py-3 px-4 bg-[#D4AF37] hover:bg-white text-black font-display font-semibold uppercase tracking-widest text-[10px] rounded-sm transition-all duration-300 active:scale-98 cursor-pointer flex items-center justify-center gap-1.5"
            >
              <span>{t('faq.contact_us', { defaultValue: isAr ? 'تواصل معنا' : 'Contact Us' })}</span>
              <ArrowRight className="w-3.5 h-3.5 shrink-0 rtl:rotate-180" />
            </button>

            {/* WhatsApp + Email Support 2-Column Grid on Mobile */}
            <div className="grid grid-cols-2 gap-3.5 sm:contents">
              ...
              >
                ...
                <span>{t('faq.email_support', { defaultValue: isAr ? 'الدعم البريدي' : 'Email Support' })}</span>
              </a>
            </div>
          </div>
        </motion.div>
        ...
        <div className="text-center pt-4 border-t border-white/5 mt-12">
          <p className="text-zinc-500 text-[10.5px] leading-relaxed max-w-md mx-auto">
            {t('faq.more_help_desc', { defaultValue: isAr ? 'هل تحتاج لمزيد من المساعدة؟ تواصل مع فريق العناية بالعملاء الفاخر للحصول على استشارات مخصصة لطلبك.' : 'Need more assistance? Contact our customer support team for personalized help.' })}
          </p>
          <div className="flex items-center justify-center gap-2.5 mt-3 text-zinc-400 font-mono text-[10px]">
            ...
            <span className="text-gold-pure uppercase tracking-widest">{t('faq.available_24_7', { defaultValue: isAr ? 'متاح 24/7' : 'Available 24/7' })}</span>
          </div>
        </div>
