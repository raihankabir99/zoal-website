...
export default function About() {
  ...
  const content = {
    ...
    title: isAr ? `تراث ${brandName}` : `THE ${brandName} HERITAGE`,
    subtitle: isAr 
      ? `حيث يلتقي التراث السوداني بالتميز السعودي الحديث` 
      : `Where Sudanese Heritage Meets Modern Saudi Excellence`,
    
    intro_1: isAr
      ? `تأسست ${brandName} في قلب مدينة الهفوف النابضة بالحياة بالمملكة العربية السعودية، برؤية تتجاوز مجرد تجارة التجزئة. نحن نؤمن بأن كل منتج يحمل قصة، وأن كل تقليد يستحق الحفاظ عليه، وأن كل عميل يستحق تجربة تحددها الأصالة والأناقة والضيافة الاستثنائية.`
      : `Nestled in the vibrant city of Al Hofuf, Saudi Arabia, ${brandName} was founded with a vision far greater than retail. We believe that every product carries a story, every tradition deserves to be preserved, and every customer deserves an experience defined by authenticity, elegance, and exceptional hospitality.`,
    
    intro_2: isAr
      ? `مستوحاة من التراث الثقافي الغني للسودان والضيافة الخالدة للمملكة العربية السعودية، تطورت ${brandName} لتصبح وجهة حياة فريدة من نوعها حيث يتعايش التراث والحرفية والمعيشة المعاصرة في انسجام تام. تم تصميم كل زيارة لتلهم وتصل وتحتفل بالثقافات من خلال منتجات منسقة بعناية وتجارب لا تُنسى.`
      : `Inspired by the rich cultural heritage of Sudan and the timeless hospitality of the Kingdom of Saudi Arabia, ${brandName} has evolved into a unique lifestyle destination where heritage, craftsmanship, and contemporary living exist in perfect harmony. Every visit is designed to inspire, connect, and celebrate cultures through thoughtfully curated products and unforgettable experiences.`,

    destinations_title: isAr ? 'وجهة التميز والفرادة' : 'A Destination of Distinction',
    destinations_desc: isAr
      ? `في ${brandName}، تم تصميم كل مساحة بعناية لتعكس شغفنا بالجودة والتقاليد والفخامة الحديثة.`
      : `At ${brandName}, every space has been carefully crafted to reflect our passion for quality, tradition, and modern luxury.`,
    ...
    more_desc1: isAr
      ? `إن ${brandName} أكثر من مجرد وجهة للتسوق — إنها تجربة ثقافية حيوية يتم فيها الحفاظ على التراث، وتواصل المجتمعات، والاحتفاء بالتقاليد بفخر.`
      : `${brandName} is more than a shopping destination—it is a vibrant cultural experience where heritage is preserved, communities are connected, and traditions are celebrated with pride.`,
    more_desc2: isAr
    ...
    more_desc3: isAr
      ? `سواء كنت تستمتع بكوب قهوة معد بشكل مثالي، أو تكتشف منتجات تخصصية أصيلة، أو تختار أزياء تقليدية أنيقة، أو تشارك لحظات مميزة مع العائلة والأصدقاء، فإن كل زيارة إلى ${brandName} مصممة لتترك أثراً دائماً.`
      : `Whether you're enjoying a perfectly crafted cup of coffee, discovering authentic specialty products, selecting elegant traditional attire, or sharing moments with family and friends, every visit to ${brandName} is designed to leave a lasting impression.`,
    ...
    values_title: isAr ? 'قيمنا الأساسية' : 'Our Values',
    closing_title: isAr ? 'بيان الختام والالتزام' : 'Closing Statement',
    closing_text: isAr
      ? `في ${brandName}، نؤمن بأن الفخامة الحقيقية لا تحددها الحصرية وحدها — بل تُخلق من خلال الأصالة والثقة والحرفية والعلاقات الإنسانية الهادفة. يعكس كل منتج نقدمه وكل تجربة نبتكرها شغفنا بالاحتفاء بالتراث السوداني مع احتضان مستقبل نمط الحياة السعودي الحديث. مرحباً بكم في ${brandName}، حيث تلتقي التقاليد بالأناقة الخالدة.`
      : `At ${brandName}, we believe true luxury is not defined by exclusivity alone—it is created through authenticity, trust, craftsmanship, and meaningful human connections. Every product we offer and every experience we create reflects our passion for celebrating Sudanese heritage while embracing the future of modern Saudi lifestyle. Welcome to ${brandName}, where tradition meets timeless elegance.`
  };
  ...
  const destinations = [
    ...
    },
    {
      ...
      desc: isAr
        ...
        : "Celebrate timeless elegance through premium thobes, Sudanese traditional garments, women's fashion, children's clothing, premium fabrics, hijabs, scarves, and professional tailoring services designed with exceptional attention to detail.",
      icon: Scissors
    }
  ];
        ...
        >
          ...
          <div className="w-24 h-24 sm:w-28 sm:h-28 mb-6 rounded-full overflow-hidden flex items-center justify-center shrink-0 shadow-[0_0_20px_rgba(212,175,55,0.15)] ring-1 ring-gold-pure/20 bg-black">
            <img
              ...
              alt={`${settings.businessName} Badge`}
