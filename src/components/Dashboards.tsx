...
export default function Dashboards({
  ...
  const memberSinceFormatter = React.useMemo(
    () =>
  ...
  const formatMemberSince = (dateVal: any) => {
    if (!dateVal) return '—';
    try {
  ...
  const sidebarGroups = useMemo(() => [
    {
      ...
      icon: User,
      items: [
        { id: 'overview', name: isAr ? 'ملخص الملف الشخصي' : 'Profile Summary', icon: BarChart3 },
        { id: 'profile', name: isAr ? 'ملفي الشخصي' : 'My Profile', icon: User },
        { id: 'notifications', name: isAr ? 'الإشعارات' : 'Notifications', icon: Bell },
        { id: 'settings', name: isAr ? 'إعدادات الحساب' : 'Account Settings', icon: Shield },
      ]
    },
    {
      title: isAr ? 'الطلبات والعمليات' : 'Orders',
      icon: ClipboardList,
      items: [
        { id: 'orders', name: isAr ? 'طلباتي' : 'My Orders', icon: ClipboardList },
        { id: 'track', name: isAr ? 'تتبع الطلبات' : 'Track Orders', icon: Truck },
        { id: 'invoices', name: isAr ? 'الفواتير والإيصالات' : 'Invoices & Receipts', icon: FileText },
      ]
    },
    {
      title: isAr ? 'التسوق والتوصيل' : 'Shopping & Delivery',
      icon: Bookmark,
      items: [
    ...
    orders.forEach(o => {
      if (!map.has(o.phone)) {
        map.set(o.phone, {
          name: o.customerName,
          phone: o.phone,
          email: `${(o.customerName || 'customer').toLowerCase().replace(/\s+/g, '')}@zoal-patron.sa`,
          address: o.address,
  ...
  return (
    <div className={`bg-black text-white min-h-screen mt-[10px] md:mt-0 pb-10 md:pb-20 ${activeDashboardTab === 'admin' ? 'pt-1.5 lg:pt-10' : 'pt-4 md:pt-10'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Dynamic Multi-Dashboard Tab Switcher (Apple/Rolex inspired) */}
        <div className={`hidden md:flex flex-col md:flex-row items-center justify-between border-b border-white/5 gap-1.5 md:gap-6 ${activeDashboardTab === 'customer' && customerSubTab === 'overview' && userRole !== 'admin' && userRole !== 'staff' ? 'hidden' : ''} ${
          activeDashboardTab === 'admin' 
            ? 'hidden lg:flex pb-1 lg:pb-6 mb-1 lg:mb-[37px]' 
            : 'pb-1 md:pb-6 mb-1.5 md:mb-10 lg:mb-[37px]'
        }`}>
          <div className="w-full md:w-auto">
            {activeDashboardTab === 'customer' ? (
              ...
              <div className="text-center md:text-left">
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-wider font-display uppercase text-white">
                  Staff Dashboard
                </h1>
              </div>
            )}
          ...
          {(userRole === 'admin' || userRole === 'staff' || userRole === 'owner') && (
            <div className={`flex bg-zinc-950 border border-white/5 rounded-sm w-full md:w-auto rtl:flex-row-reverse ${activeDashboardTab === 'admin' ? 'p-0.5 lg:p-1' : 'p-1'}`}>
              {activeDashboardTab !== 'customer' && (
                <button
                  onClick={() => setActiveDashboardTab('customer')}
                  className={`py-1 px-2 md:py-2.5 md:px-5 rounded-xs font-display uppercase cursor-pointer transition-all flex items-center justify-center gap-1 lg:gap-1.5 text-zinc-500 hover:text-white ${
                    activeDashboardTab === 'admin'
                      ? 'flex-1 min-w-0 lg:flex-initial lg:w-auto py-0.5 px-1 lg:py-2.5 lg:px-5 text-[9.5px] lg:text-xs tracking-wide lg:tracking-widest leading-tight lg:leading-normal'
                      : 'flex-grow md:flex-initial text-[10px] sm:text-xs tracking-widest'
                  }`}
                >
                  <User className="w-3 h-3 lg:w-3.5 lg:h-3.5" /> MY DASHBOARD
                </button>
              )}
              {(userRole === 'admin' || userRole === 'staff') && (
                <button
                  onClick={() => setActiveDashboardTab('admin')}
                  className={`rounded-xs font-display uppercase cursor-pointer transition-all flex items-center justify-center gap-1 lg:gap-1.5 ${
                    activeDashboardTab === 'admin' 
                      ? 'bg-gold-pure text-black font-semibold flex-1 min-w-0 lg:flex-initial lg:w-auto py-0.5 px-1 lg:py-2.5 lg:px-5 text-[9.5px] lg:text-xs tracking-wide lg:tracking-widest leading-tight lg:leading-normal' 
                      : 'text-zinc-500 hover:text-white flex-grow md:flex-initial py-1 px-2 md:py-2.5 md:px-5 text-[10px] sm:text-xs tracking-widest'
                  }`}
                >
                  <Shield className="w-3 h-3 lg:w-3.5 lg:h-3.5" /> {t('dashboard.staff_dashboard', { defaultValue: 'Staff Dashboard' })}
                </button>
              )}
              {(userRole === 'admin' || userRole === 'owner') && (
                <button
                  onClick={() => setCurrentPage('admin')}
                  className={`rounded-xs font-display uppercase cursor-pointer transition-all flex items-center justify-center gap-1 lg:gap-1.5 text-gold-pure hover:bg-gold-pure/10 border border-gold-pure/30 ${
                    activeDashboardTab === 'admin'
                      ? 'flex-1 min-w-0 lg:flex-initial lg:w-auto py-0.5 px-1 lg:py-2.5 lg:px-5 text-[9.5px] lg:text-xs tracking-wide lg:tracking-widest leading-tight lg:leading-normal'
                      : 'flex-grow md:flex-initial py-1 px-2 md:py-2.5 md:px-5 text-[10px] sm:text-xs tracking-widest'
                  }`}
                >
                  <Shield className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-gold-pure" /> OWNER / ADMIN DASHBOARD
                </button>
              )}
            </div>
          )}
        ...
        {activeDashboardTab === 'customer' && (
          <div className="flex flex-col">
            ...
            {/* Mobile Navigation Wrapper (Mobile Only, < 768px) */}
            <div className="md:hidden md:mt-0 flex flex-col mt-[-18px] mb-4">
              {/* Compact Single-Row Navigation Bar */}
              <div className="bg-zinc-950 border border-white/5 rounded-sm py-1.5 px-2.5 flex items-center justify-between gap-1.5 white-space:nowrap w-full">
                {/* ☰ Menu & Dashboard Title */}
                <div className="flex items-center gap-1.5 min-w-0">
                  <button 
                    onClick={() => { setSidebarOpen(true); }}
                    className="p-1 -ml-1 text-zinc-400 hover:text-[#D4AF37] transition-colors focus:outline-none shrink-0"
                    aria-label="Open Sidebar Menu"
                  >
                    <Menu className="w-4 h-4" />
                  </button>
                  <div className="flex items-center text-[10px] uppercase tracking-wider text-zinc-500 font-mono truncate">
                    <span className="text-[#D4AF37] font-semibold truncate max-w-[100px] sm:max-w-[130px] block">
                      {customerSubTab === 'overview' ? (isAr ? 'الملخص' : 'Dashboard') :
                       customerSubTab === 'orders' ? (selectedOrder ? (isAr ? 'تفاصيل الطلب' : 'Order Details') : (isAr ? 'طلباتي' : 'My Orders')) :
                       customerSubTab === 'track' ? (isAr ? 'تتبع الطلبات' : 'Track Orders') :
                       customerSubTab === 'wishlist' ? (isAr ? 'المفضلة' : 'Wishlist') :
                       customerSubTab === 'addresses' ? (isAr ? 'العناوين المحفوظة' : 'Saved Addresses') :
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <button
                    onClick={() => {
                      setCustomerSubTab('notifications');
                      setSelectedOrder(null);
                    }}
                    className="relative p-1 bg-black hover:bg-zinc-900 border border-white/5 hover:border-[#D4AF37]/30 text-zinc-400 hover:text-white rounded-xs transition-all cursor-pointer group shrink-0"
                    title={isAr ? 'الإشعارات' : 'Notifications'}
                  >
                    <Bell className="w-3 h-3 group-hover:animate-swing" />
                    {activeNotificationEngine.unreadCount > 0 && (
                      <span className="absolute -top-1 -right-1 min-w-[12px] h-3 px-0.5 bg-[#D4AF37] text-black font-mono font-bold text-[7px] flex items-center justify-center rounded-full">
                        {activeNotificationEngine.unreadCount}
                      </span>
                    )}
                  </button>
                  ...
                  {/* Profile Avatar */}
                  <div className="relative group/profile select-none shrink-0">
                    <div className="flex items-center p-0.5 bg-black hover:bg-zinc-900 border border-white/5 rounded-xs cursor-pointer duration-300">
                      <div className="w-5 h-5 rounded-full border border-[#D4AF37] overflow-hidden bg-zinc-900 flex items-center justify-center text-[8.5px] font-bold text-[#D4AF37] font-mono shrink-0">
            ...
            {/* Mobile Multi-Dashboard Tab Switcher (Mobile Only) */}
            {(userRole === 'admin' || userRole === 'staff' || userRole === 'owner') && !(activeDashboardTab === 'customer' && customerSubTab === 'overview' && userRole !== 'admin' && userRole !== 'staff') && (
              <div className="md:hidden flex bg-zinc-950 border border-white/5 rounded-sm p-1 mb-4 w-full gap-1.5">
                {(userRole === 'admin' || userRole === 'staff') && (
                  <button
                    onClick={() => setActiveDashboardTab('admin')}
                    className="flex-1 py-1.5 px-2 rounded-xs font-display uppercase cursor-pointer transition-all flex items-center justify-center gap-1 text-[10px] tracking-widest text-zinc-500 hover:text-white"
                  >
                    <Shield className="w-3 h-3" /> {t('dashboard.staff_dashboard', { defaultValue: 'Staff Dashboard' })}
                  </button>
                )}
                {(userRole === 'admin' || userRole === 'owner') && (
                  <button
                    onClick={() => setCurrentPage('admin')}
                    className="flex-1 py-1.5 px-2 rounded-xs font-display uppercase cursor-pointer transition-all flex items-center justify-center gap-1 text-[10px] tracking-widest text-gold-pure hover:bg-gold-pure/10 border border-gold-pure/30"
                  >
                    <Shield className="w-3 h-3 text-gold-pure" /> OWNER / ADMIN DASHBOARD
                  </button>
                )}
              </div>
            )}

            {/* Desktop/Tablet Navigation & Breadcrumb Bar (>= 768px) */}
            <div className="hidden md:flex bg-zinc-950 border border-white/5 rounded-sm p-4 flex-col md:flex-row md:items-center justify-between gap-4 md:-mt-[41px] md:mb-6">
              {/* Breadcrumbs */}
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      if (selectedOrder) {
                        setSelectedOrder(null);
                      } else if (customerSubTab !== 'overview') {
                  ...
                  >
                    ...
                    <span className="text-[9px] uppercase tracking-widest font-bold">{isAr ? 'رجوع' : 'Back'}</span>
                  </button>
              ...
              <div className="flex items-center justify-end gap-2.5 sm:gap-3.5">
                ...
                >
                  ...
                  {activeNotificationEngine.unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 min-w-[14px] h-3.5 px-0.5 bg-[#D4AF37] text-black font-mono font-bold text-[8px] flex items-center justify-center rounded-full">
                      {activeNotificationEngine.unreadCount}
                    </span>
                  )}
                </button>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <span className="block text-[8px] text-zinc-500 font-sans tracking-normal leading-tight">
                            {isAr ? 'الخروج بأمان من الحساب' : 'Sign out from your account'}
                          </span>
                        </div>
                      </button>
                    </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start relative">
              ...
              <div className="hidden lg:block w-64 shrink-0 bg-zinc-950 border border-white/5 rounded-sm p-4 space-y-4">
                ...
                {onLogout && (
                  ...
                  >
                    ...
                    <span>{isAr ? 'إنهاء الجلسة والخروج' : 'Log Out Session'}</span>
                  </button>
                )}
              </div>
              ...
              <div className="flex-1 w-full bg-zinc-950/30 border border-white/5 rounded-sm p-3.5 sm:p-6 min-h-[500px]">
                ...
                <AnimatePresence mode="wait">
                  ...
                  {customerSubTab === 'overview' && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      className="space-y-4 sm:space-y-3.5 text-left"
                    >
                      {/* Welcome Banner Card */}
                      <div className="relative overflow-hidden bg-gradient-to-br from-zinc-950 via-zinc-900 to-black border border-[#D4AF37]/20 rounded-xs p-1.5 xs:p-2 sm:p-6 md:py-3 md:px-4 shadow-2xl md:-mt-5 lg:-mt-5">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-[#D4AF37] opacity-5 blur-2xl rounded-full pointer-events-none"></div>
                        <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-5 relative z-10 text-center sm:text-left">
                          
                          {/* Avatar Display */}
                          <div className="hidden md:flex w-10 h-10 sm:w-16 sm:h-16 rounded-full border-2 border-[#D4AF37] overflow-hidden bg-zinc-950 items-center justify-center text-base sm:text-xl font-bold text-[#D4AF37] shadow-[0_0_15px_rgba(212,175,55,0.15)] shrink-0 relative group">
                            {currentUser?.name?.[0]?.toUpperCase() || 'U'}
                            <div className="absolute inset-0 bg-black/60 rounded-full opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity cursor-pointer" title={isAr ? 'تغيير الصورة' : 'Change Photo'}>
                              <Camera className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-[#D4AF37]" />
                            </div>
                          </div>

                          <div className="space-y-0.5 sm:space-y-1 w-full">
                            <h2 className="text-white font-display uppercase font-bold text-[11px] sm:text-lg tracking-tight sm:tracking-wider leading-tight mb-0.5 sm:mb-1">
                              <span className="md:hidden">
                                {isAr ? (
                                  <>أهلاً بك مجدداً، <span className="text-[#D4AF37]">{currentUser?.name || 'عميلنا العزيز'}</span></>
                                ) : (
                                  <>Welcome back, <span className="text-[#D4AF37]">{currentUser?.name || 'Customer'}</span></>
                                )}
                              </span>
                              <span className="hidden md:inline">
                                {isAr ? <>أهلاً بك مجدداً،<br /></> : 'Welcome back,' + '\n'}
                                <span className="text-[#D4AF37]">{currentUser?.name || (isAr ? 'عميلنا العزيز' : 'Customer')}</span>
                              </span>
                            </h2>
                            <p className="text-zinc-400 text-[8.5px] sm:text-xs leading-tight sm:leading-normal max-w-xl">
                              {isAr 
                                ? 'أدر طلباتك، وعناوين التوصيل، وقائمة المفضلة، وإعدادات الحساب من مكان واحد وبكل سهولة.' 
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-4">
                        {[
                          { id: 'orders', label: isAr ? 'طلباتي' : 'Orders', value: customerOrders.length, desc: isAr ? 'عرض سجل الطلبات بالكامل' : 'View order history', icon: ClipboardList, color: 'text-[#D4AF37]' },
                          { id: 'wishlist', label: isAr ? 'المفضلة' : t('nav.wishlist', { defaultValue: 'My Wishlist' }), value: wishlist.length, desc: isAr ? 'القطع المحفوظة لشرائها لاحقاً' : 'Your saved items', icon: Heart, color: 'text-rose-500' },
                          { id: 'reviews', label: isAr ? 'التقييمات' : 'Reviews', value: customerReviews.length, desc: isAr ? 'آرائي حول المنتجات' : 'My product feedback', icon: Star, color: 'text-orange-400' },
                          { id: 'notifications', label: isAr ? 'الإشعارات' : 'Notifications', value: 0, desc: isAr ? 'تنبيهات وتحديثات الحساب' : 'Account alerts', icon: Bell, color: 'text-blue-500' },
                          { id: 'addresses', label: isAr ? 'العناوين' : 'Addresses', value: (currentUser?.addresses || []).length, desc: isAr ? 'إدارة مواقع التوصيل المحددة' : 'Manage your locations', icon: MapPin, color: 'text-emerald-500' },
                        ].map((stat) => {
                          ...
                          return (
                            <button
                              ...
                              className="bg-[#060606]/95 border border-white/5 hover:border-[#D4AF37]/30 hover:bg-zinc-950/85 duration-300 p-2 sm:p-4 rounded-xs text-center relative group cursor-pointer flex flex-col justify-between h-[76px] sm:h-32 focus:outline-none"
                            >
                              <div>
                                <Icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 mx-auto mb-0.5 sm:mb-1 ${stat.color} group-hover:scale-115 duration-300`} />
                                <span className="text-[9.5px] sm:text-[11px] font-bold text-white block truncate leading-snug">{stat.label}</span>
                                <span className="text-[6.5px] sm:text-[7.5px] text-zinc-500 block leading-tight mt-0.5 max-w-[120px] mx-auto font-sans font-medium hidden sm:block">{stat.desc}</span>
                              </div>
                              <span className="text-base sm:text-xl font-mono text-[#D4AF37] font-extrabold block mt-0.5 sm:mt-1.5">{stat.value}</span>
                            </button>
                          );
                        })}
                      </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <div className="lg:col-span-8 bg-[#060606]/50 border border-white/5 rounded-xs p-5 space-y-4">
                          <div className="flex justify-between items-center border-b border-white/5 pb-2">
                            <h3 className="text-white text-[10px] uppercase font-display font-bold tracking-wider flex items-center gap-2">
                              <ClipboardList className="w-4 h-4 text-[#D4AF37]" /> {isAr ? 'الطلبات الأخيرة' : 'Recent Orders'}
                            </h3>
                            <button
                          ...
                          <div className="space-y-3">
                            {customerOrders.length === 0 ? (
                              ...
                              customerOrders.slice(0, 2).map((order) => (
                                <div key={order.id} className="p-3 bg-black border border-white/5 rounded-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-[#D4AF37]/20 duration-300" dir={isAr ? 'rtl' : 'ltr'}>
                                  <div>
                                    <span className="text-white font-mono text-[10px] font-semibold block">{order.id}</span>
                                    <span className="text-[9px] text-zinc-500 font-mono">{order.date} • {order.items.reduce((acc, item) => acc + item.quantity, 0)} {isAr ? 'قطع' : 'Items'}</span>
                                  </div>
                                  <div className="flex items-center gap-3 justify-between sm:justify-end">
                                    ...
                                    <span className={`px-2 py-0.5 border text-[7.5px] uppercase tracking-wider rounded-sm font-bold font-mono ${
                                      ...
                                      {order.status ? t(`checkout.status.${(order.status || '').toLowerCase().replace(/ /g, '_')}`, { defaultValue: order.status }) : ''}
                                    </span>
                                    <button
                                      ...
                                      title={isAr ? 'تفاصيل الطلب' : 'Review details'}
                                    >
                        ...
                        <div className="lg:col-span-4 bg-[#060606]/50 border border-white/5 rounded-xs p-5 space-y-4">
                          <h3 className="text-white text-[10px] uppercase font-display font-bold tracking-wider border-b border-white/5 pb-2 flex items-center gap-2">
                            <Sparkles className="w-4 h-4 text-[#D4AF37]" /> {isAr ? 'إجراءات سريعة' : 'Quick Actions'}
                          </h3>
                          <div className="grid grid-cols-1 gap-2.5">
                            {[
                              { label: isAr ? 'تسوق الآن' : 'Shop Now', action: () => setCurrentPage('store'), desc: isAr ? 'تصفح أحدث القطع والروائع الفاخرة' : 'Browse latest premium creations' },
                              { label: isAr ? 'تتبع الطلبات' : 'Track Orders', action: () => { setCustomerSubTab('track'); setSelectedOrder(null); }, desc: isAr ? 'عرض مسار التوصيل وحالة الشحن الفوري' : 'View dispatch map and delivery status' },
                              { label: isAr ? 'كتالوج المفضلة' : 'Wishlist Catalog', action: () => { setCustomerSubTab('wishlist'); setSelectedOrder(null); }, desc: isAr ? 'تفقد روائعك وقطعك المفضلة المحفوظة' : 'Inspect saved luxury creations' },
                              { label: isAr ? 'إضافة عنوان' : 'Add Address', action: () => { setCustomerSubTab('addresses'); setSelectedOrder(null); }, desc: isAr ? 'تهيئة وتعديل إحداثيات عناوين التوصيل' : 'Configure checkout delivery coordinates' },
                              { label: isAr ? 'كتابة تقييم' : 'Write Review', action: () => { setCustomerSubTab('reviews'); setSelectedOrder(null); }, desc: isAr ? 'شارك تجربتك الحصرية الفاخرة مع الآخرين' : 'Share your exclusive experience' }
                            ].map((act, i) => (
                              <button
                                key={i}
                                onClick={act.action}
                              ...
                              >
                                ...
                                <ChevronRight className={`w-3.5 h-3.5 text-zinc-600 group-hover:text-[#D4AF37] transition-all ${isAr ? 'group-hover:-translate-x-0.5 rotate-180' : 'group-hover:translate-x-0.5'}`} />
                              </button>
                            ))}
                          </div>
                        </div>
                                  ...
                                  return (
                                    <div key={idx} className="flex gap-4 items-center p-3 bg-black border border-white/5 rounded-xs">
                                      <SafeImage
                                        ...
                                        containerClassName="w-12 h-12 relative rounded-xs overflow-hidden shrink-0"
                                      />
                    ...
                    >
                      <div className="bg-[#060606]/40 border border-white/5 rounded-xs p-4 flex gap-4 items-center justify-between mb-2">
                        <div className="relative w-full max-w-sm">
                          ...
                          <input
                            type="text"
                            placeholder={isAr ? 'البحث في كتالوج القطع المفضلة المحفوظة...' : 'Search saved wishlist creations...'}
                        ...
                        <span className="text-zinc-500 text-[10px] font-mono whitespace-nowrap">{wishlist.length} {isAr ? 'قطع' : 'Items'}</span>
                      </div>

                      {wishlist.length === 0 ? (
                        <div className="p-12 text-center border border-dashed border-white/5 bg-zinc-950/25 rounded-sm">
                          <Bookmark className="w-8 h-8 text-zinc-500 mx-auto mb-3 opacity-20" />
                          <h4 className="text-white text-xs font-display uppercase tracking-wider">{isAr ? 'قائمة المفضلة فارغة حالياً' : t('wishlist.empty', { defaultValue: 'Your Wishlist is Empty' })}</h4>
                          <p className="text-zinc-500 text-[10px] mt-2 mb-4 max-w-[220px] mx-auto">{isAr ? 'احفظ القطع والروائع الفاخرة التي تنال إعجابك هنا للوصول إليها وشرائها لاحقاً.' : t('wishlist.empty_desc', { defaultValue: 'Save items you love here to easily find or purchase them later.' })}</p>
                          <button
                            onClick={() => setCurrentPage('store')}
                          ...
                          >
                            {isAr ? 'تصفح واستكشف المنتجات' : t('wishlist.browse', { defaultValue: 'Explore Products' })}
                          </button>
                        </div>
                      ) : (
                            ...
                            .map((product) => (
                              <div key={product.id} className="p-4 border border-white/5 bg-[#060606] rounded-xs flex gap-4 hover:border-[#D4AF37]/20 duration-300 items-center">
                                <SafeImage
                                  ...
                                  containerClassName="w-16 h-16 relative rounded-xs overflow-hidden shrink-0"
                                />
                                <div className="flex-1 min-w-0 text-left">
                    ...
                    >
                      ...
                      <div className="bg-[#060606]/60 border border-white/5 rounded-xs p-5 space-y-5">
                        <h3 className="text-white text-[11px] uppercase font-display font-bold tracking-widest border-b border-white/5 pb-2.5 flex items-center gap-2">
                          <User className="w-4.5 h-4.5 text-[#D4AF37]" /> {isAr ? 'المعلومات الشخصية والبيانات' : 'Personal Information'}
                        </h3>
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-5 border-b border-white/5 pb-5">
                          ...
                          <div className="space-y-1 text-center sm:text-left">
                            ...
                            <p className="text-[#D4AF37] font-mono text-[9px] uppercase tracking-widest">{isAr ? 'عضو منذ • ' : 'Member Since • '}{formatMemberSince((currentUser as any)?.created_at)}</p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-sans">
                          <div className="p-3 bg-black border border-white/5 rounded-xs space-y-1">
                            <span className="text-zinc-500 block uppercase font-mono text-[8px] tracking-widest">{isAr ? 'الاسم بالكامل' : 'Full Name'}</span>
                            <span className="text-white font-medium">{currentUser?.name}</span>
                          </div>
                          <div className="p-3 bg-black border border-white/5 rounded-xs space-y-1">
                            <span className="text-zinc-500 block uppercase font-mono text-[8px] tracking-widest">{isAr ? 'البريد الإلكتروني' : 'Email Address'}</span>
                            <span className="text-zinc-300 font-mono">{currentUser?.email}</span>
                          </div>
                          <div className="p-3 bg-black border border-white/5 rounded-xs space-y-1">
                            <span className="text-zinc-500 block uppercase font-mono text-[8px] tracking-widest">{isAr ? 'رقم الهاتف الجوال' : 'Phone Number'}</span>
                            <span className="text-zinc-300 font-mono">{currentUser?.phone || (isAr ? 'غير محدد' : 'Not Specified')}</span>
                          </div>
                          <div className="p-3 bg-black border border-white/5 rounded-xs space-y-1">
                            <span className="text-zinc-500 block uppercase font-mono text-[8px] tracking-widest">{isAr ? 'تاريخ الانضمام' : 'Member Since'}</span>
                            <span className="text-zinc-300 font-mono">{formatMemberSince((currentUser as any)?.created_at)}</span>
                          </div>
                          <div className="p-3 bg-black border border-white/5 rounded-xs space-y-1">
                            <span className="text-zinc-500 block uppercase font-mono text-[8px] tracking-widest">{isAr ? 'اللغة المفضلة' : 'Preferred Language'}</span>
                            <span className="text-zinc-300 font-mono">{isAr ? 'العربية' : 'English'}</span>
                          </div>
                          <div className="p-3 bg-black border border-white/5 rounded-xs space-y-1 sm:col-span-2 flex justify-between items-center" dir={isAr ? 'rtl' : 'ltr'}>
                            <div>
                              <span className="text-zinc-500 block uppercase font-mono text-[8px] tracking-widest">{isAr ? 'عنوان التوصيل الافتراضي' : 'Default Delivery Address'}</span>
                              <span className="text-zinc-400 text-xs">
                                {currentUser?.addresses?.find((a: any) => a.isDefault)?.addressLine || (isAr ? 'لا يوجد عنوان افتراضي محدد بعد' : 'No Default Address Selected')}
                              </span>
                            </div>
                            <button onClick={() => setCustomerSubTab('addresses')} className="text-[9.5px] text-[#D4AF37] hover:underline uppercase font-mono font-bold">{isAr ? 'إدارة العناوين' : 'Manage Addresses'}</button>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-white/5">
                          <button 
                            onClick={() => setCurrentPage('store')} 
                            className="flex items-center gap-4 p-4 bg-zinc-950/40 backdrop-blur-md border border-[#D4AF37]/10 hover:border-[#D4AF37]/40 hover:bg-[#D4AF37]/5 rounded-sm group transition-all duration-300 cursor-pointer relative overflow-hidden text-left rtl:text-right"
                            dir={isAr ? 'rtl' : 'ltr'}
                          >
                            ...
                            <div className="flex-1 z-10">
                              <div className="text-[11px] text-white uppercase tracking-widest font-bold font-display">{isAr ? 'تسوق الآن' : 'SHOP NOW'}</div>
                              <div className="text-[10px] text-zinc-500 font-sans tracking-wide mt-0.5 uppercase">{isAr ? 'تصفح أحدث التشكيلات الفاخرة والروائع' : 'Browse latest premium creations'}</div>
                            </div>
                            <ChevronRight className={`w-4 h-4 text-zinc-800 group-hover:text-[#D4AF37] transition-all duration-300 z-10 ${isAr ? 'group-hover:-translate-x-1 rotate-180' : 'group-hover:translate-x-1'}`} />
                          </button>
                          ...
                          >
                            ...
                            <div className="flex-1 z-10">
                              <div className="text-[11px] text-white uppercase tracking-widest font-bold font-display">{isAr ? 'تتبع الطلبات' : 'TRACK ORDERS'}</div>
                              <div className="text-[10px] text-zinc-500 font-sans tracking-wide mt-0.5 uppercase">{isAr ? 'تفقد مسارات الشحن المباشرة مع البريد السعودي' : 'View dispatch map and delivery status'}</div>
                            </div>
                            <ChevronRight className={`w-4 h-4 text-zinc-800 group-hover:text-[#D4AF37] transition-all duration-300 z-10 ${isAr ? 'group-hover:-translate-x-1 rotate-180' : 'group-hover:translate-x-1'}`} />
                          </button>
                          ...
                          >
                            ...
                            <div className="flex-1 z-10">
                              <div className="text-[11px] text-white uppercase tracking-widest font-bold font-display">{isAr ? 'كتالوج المفضلة' : 'WISHLIST CATALOG'}</div>
                              <div className="text-[10px] text-zinc-500 font-sans tracking-wide mt-0.5 uppercase">{isAr ? 'استعراض الروائع والقطع المحفوظة لديك' : 'Inspect saved luxury creations'}</div>
                            </div>
                            <ChevronRight className={`w-4 h-4 text-zinc-800 group-hover:text-[#D4AF37] transition-all duration-300 z-10 ${isAr ? 'group-hover:-translate-x-1 rotate-180' : 'group-hover:translate-x-1'}`} />
                          </button>
                          ...
                          >
                            ...
                            <div className="flex-1 z-10">
                              <div className="text-[11px] text-white uppercase tracking-widest font-bold font-display">{isAr ? 'تقييم المنتجات' : 'WRITE REVIEW'}</div>
                              <div className="text-[10px] text-zinc-500 font-sans tracking-wide mt-0.5 uppercase">{isAr ? 'شارك تجربتك الحصرية الفاخرة مع دار زول' : 'Share your exclusive experience'}</div>
                            </div>
                            <ChevronRight className={`w-4 h-4 text-zinc-800 group-hover:text-[#D4AF37] transition-all duration-300 z-10 ${isAr ? 'group-hover:-translate-x-1 rotate-180' : 'group-hover:translate-x-1'}`} />
                          </button>
                        </div>
                        ...
                        <div className="flex justify-end pt-2">
                          ...
                          >
                            {isAr ? 'تعديل بيانات الحساب' : 'Edit Profile'}
                          </button>
                        </div>
                      </div>
                    </motion.div>
                    ...
                    >
                      <div className="space-y-1">
                        <h3 className="text-white text-base font-display font-bold uppercase tracking-wider flex items-center gap-2">
                          <Bell className="w-5 h-5 text-[#D4AF37]" /> {isAr ? 'الإشعارات' : 'Notifications'}
                        </h3>
                        <p className="text-zinc-400 text-xs font-sans leading-relaxed">
                          {isAr 
                            ? 'تابع آخر مستجدات طلباتك، العروض الحصرية، والتنبيهات الهامة لحسابك.' 
                            : 'Stay updated with your order status, exclusive offers, and account alerts.'}
                        </p>
                      </div>

                      {activeNotificationEngine.notifications.length === 0 ? (
                        <div className="bg-[#060606]/80 border border-white/10 rounded-sm p-10 flex flex-col items-center justify-center text-center space-y-4 shadow-xl">
                          ...
                          <div className="space-y-2">
                            <h4 className="text-white text-sm font-display uppercase font-bold tracking-wider">
                              {isAr ? 'لا توجد إشعارات حالياً' : 'No notifications yet.'}
                            </h4>
                            <p className="text-zinc-500 text-xs font-sans max-w-xs mx-auto">
                              {isAr 
                                ? 'سنقوم بإخطارك بأحدث المستجدات حول طلباتك وتحديثات حسابك فور حدوثها.' 
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-center pb-2 border-b border-white/10">
                            <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-widest">
                              {isAr ? `${activeNotificationEngine.notifications.length} إشعار` : `${activeNotificationEngine.notifications.length} Notifications`}
                            </span>
                            <button
                              onClick={() => activeNotificationEngine.markAllAsRead()}
                              className="text-[9px] font-mono uppercase text-[#D4AF37] hover:text-white tracking-widest cursor-pointer"
                            >
                              {isAr ? 'تحديد الكل كمعروء' : 'Mark all as read'}
                            </button>
                          </div>
                          <div className="divide-y divide-white/5 bg-[#060606]/80 border border-white/10 rounded-sm">
                            {activeNotificationEngine.notifications.map((notif: any) => (
                              <div
                                key={notif.id}
                                onClick={() => activeNotificationEngine.markAsRead(notif.id)}
                                className={`p-4 flex items-start justify-between gap-4 cursor-pointer transition-colors ${
                                  !notif.read ? 'bg-[#D4AF37]/[0.03] border-l-2 border-[#D4AF37]' : 'opacity-60'
                                }`}
                              >
                                <div className="space-y-1 text-left">
                                  <div className="flex items-center gap-2">
                                    {!notif.read && <span className="w-2 h-2 rounded-full bg-[#D4AF37]" />}
                                    <h4 className="text-white font-semibold text-xs font-sans uppercase tracking-wide">{notif.title}</h4>
                                  </div>
                                  <p className="text-zinc-400 leading-relaxed font-sans text-xs">{notif.message}</p>
                                  <span className="text-zinc-500 text-[9px] font-mono block pt-1">
                                ...
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    activeNotificationEngine.deleteNotification(notif.id);
                                  }}
                                  className="text-zinc-600 hover:text-rose-400 p-1 text-[10px] font-mono"
                  ...
                  {customerSubTab === 'reviews' && (
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        <h3 className="text-white text-base font-display font-bold uppercase tracking-wider flex items-center gap-2">
                          <Star className="w-5 h-5 text-[#D4AF37]" /> {isAr ? 'تقييمات المنتجات' : 'Product Reviews'}
                        </h3>
                        <p className="text-zinc-400 text-xs font-sans leading-relaxed">
                          {isAr 
                            ? 'شاركنا تجربتك لمساعدة العملاء الآخرين في اتخاذ أفضل القرارات أثناء التسوق.' 
                      ...
                      <div className="bg-[#060606]/80 border border-white/10 rounded-sm p-5 space-y-4 shadow-xl">
                        <h4 className="text-white text-xs uppercase font-display font-bold tracking-wider">
                            ...
                            {customerOrders.flatMap(o => o.items).map((item, idx) => (
                              <div key={idx} className="bg-black border border-white/5 p-3 rounded-xs flex items-center gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <span className="text-[9px] text-zinc-500 font-mono block">
                                    {isAr ? 'منتج مؤكد من طلبك' : 'Verified Order Item'}
                                  </span>
                                  <button
                                    onClick={() => {
                                      const selProdEl = document.getElementById('reviewProductSelect') as HTMLSelectElement;
                                    ...
                                    className="mt-1 text-[9px] text-[#D4AF37] hover:underline font-mono uppercase font-semibold cursor-pointer"
                                  >
                                    {isAr ? 'تقييم المنتج' : 'Review Product'}
                                  </button>
                                </div>
                              </div>
                            ))}
                      ...
                      <div className="bg-[#060606]/80 border border-white/10 rounded-sm p-6 space-y-5 shadow-xl">
                          ...
                          onSubmit={(e) => {
                            ...
                            if (!commentText.trim()) {
                              dispatchNotification({
                                ...
                                message: isAr ? '⚠️ يرجى مشاركتنا تجربتك قبل إرسال التقييم.' : '⚠️ Please share your experience before submitting.'
                              });
                              return;
                            }
                        ...
                        >
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                              <label className="text-[10px] text-zinc-300 uppercase tracking-widest block font-semibold">
                                {isAr ? 'اختر المنتج' : 'Choose a Product'}
                              </label>
                              <select
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[10px] text-zinc-300 uppercase tracking-widest block font-semibold">
                                {isAr ? 'درجة التقييم' : 'Rating Score'}
                              </label>
                              <select
                                name="reviewRating"
                                className="w-full bg-black border border-white/10 rounded-xs p-2.5 text-xs text-white focus:outline-none focus:border-[#D4AF37]/50"
                              >
                                <option value="5">{isAr ? '★★★★★ (٥ - ممتاز)' : '★★★★★ (5 - Excellent)'}</option>
                                <option value="4">{isAr ? '★★★★☆ (٤ - جيد جداً)' : '★★★★☆ (4 - Good)'}</option>
                                <option value="3">{isAr ? '★★★☆☆ (٣ - مقبول)' : '★★★☆☆ (3 - Average)'}</option>
                                <option value="2">{isAr ? '★★☆☆☆ (٢ - ضعيف)' : '★★☆☆☆ (2 - Poor)'}</option>
                                <option value="1">{isAr ? '★☆☆☆☆ (١ - سيء جداً)' : '★☆☆☆☆ (1 - Terrible)'}</option>
                              </select>
                            </div>
                          </div>
                          ...
                          <div className="space-y-1.5">
                            <label className="text-[10px] text-zinc-300 uppercase tracking-widest block font-semibold">
                              {isAr ? 'تعليق وملاحظات التقييم' : 'Review Comment'}
                            </label>
                            <textarea
                              name="reviewComment"
                              rows={4}
                              placeholder={isAr 
                                ? 'شاركنا رأيك وتجربتك حول جودة المنتج، المقاس والملائمة، راحة الارتداء، التوصيل، أو أي تفاصيل أخرى ترغب في مشاركتها مع مجتمع زول.' 
                                : 'Share your experience with this product. Tell us about the quality, fit, comfort, delivery, or anything other customers should know.'}
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                              <button
                                ...
                                onClick={() => dispatchNotification({
                                  ...
                                  variant: 'toast',
                                  title: 'System',
                                  message: isAr ? 'بوابة تحميل الصور جاهزة.' : 'Photo upload prompt ready.'
                                })}
                              ...
                              <button
                                ...
                                onClick={() => dispatchNotification({
                                  ...
                                  variant: 'toast',
                                  title: 'System',
                                  message: isAr ? 'بوابة تحميل الفيديو جاهزة.' : 'Video upload prompt ready.'
                                })}
                                className="px-3.5 py-2 bg-black border border-white/10 hover:border-[#D4AF37]/50 text-zinc-300 hover:text-white rounded-xs text-[10px] uppercase font-semibold transition-all cursor-pointer"
                              >
                                {isAr ? 'إضافة فيديو (اختياري)' : 'Add Video (Optional)'}
                              </button>
                            </div>
                            <p className="text-[10px] text-zinc-500 font-sans">
                              {isAr ? 'قم بتحميل صور حقيقية أو مقاطع فيديو للمنتج لمساعدة العملاء الآخرين.' : 'Upload real photos or videos of the product to help other customers.'}
                            </p>
                          </div>
                          ...
                          <div className="p-3.5 bg-black/60 border border-white/5 rounded-xs space-y-1.5">
                            <span className="text-[10px] uppercase tracking-wider text-[#D4AF37] font-semibold block">
                              {isAr ? 'نصائح لكتابة التقييم' : 'Review Tips'}
                            </span>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[10px] text-zinc-400 font-sans">
                              <span>{isAr ? '✔ جودة وصناعة المنتج' : '✔ Product quality'}</span>
                              <span>{isAr ? '✔ دقة المقاس والملاءمة' : '✔ Size & fitting'}</span>
                              <span>{isAr ? '✔ راحة الارتداء اليومي' : '✔ Comfort'}</span>
                              <span>{isAr ? '✔ سرعة واحترافية التوصيل' : '✔ Delivery experience'}</span>
                              <span>{isAr ? '✔ فخامة وجمال التغليف' : '✔ Packaging'}</span>
                              <span>{isAr ? '✔ القيمة الرائعة مقابل السعر' : '✔ Value for money'}</span>
                            </div>
                          </div>
                          ...
                          <div className="pt-2 flex items-center gap-3">
                            ...
                            >
                              {editingReviewId ? (isAr ? 'حفظ التعديلات' : 'Save Changes') : (isAr ? 'إرسال التقييم' : 'Submit Review')}
                            </button>
                            {editingReviewId && (
                              <button
                                type="button"
                      ...
                      <div className="bg-[#060606]/80 border border-white/10 rounded-sm p-6 space-y-4 shadow-xl">
                        <div className="border-b border-white/5 pb-3">
                          <h4 className="text-white text-xs uppercase font-display font-bold tracking-wider">
                            {isAr ? 'تقييماتي السابقة' : 'My Reviews'}
                          </h4>
                        </div>
                        ...
                        {customerReviews.length === 0 ? (
                          <div className="text-center py-10 space-y-3">
                            <p className="text-white text-sm font-semibold uppercase tracking-wider">
                              {isAr ? 'لم تقم بتقييم أي منتجات حتى الآن.' : "You haven't reviewed any products yet."}
                            </p>
                            <p className="text-zinc-500 text-xs font-sans">
                              {isAr 
                                ? 'شاركنا رأيك وتجربتك لمساعدة الآخرين في اختيار روائعهم المفضلة.' 
                                : 'Share your experience to help other customers make better shopping decisions.'}
                            </p>
                            <div className="pt-2">
                              <button
                              ...
                              >
                                {isAr ? 'تقييم المنتجات المشتراة' : 'Review Purchased Products'}
                              </button>
                            </div>
                          </div>
                        ) : (
                          <div className="space-y-4">
                            {customerReviews.map((rev) => (
                              <div key={rev.id} className="p-4 bg-black border border-white/10 rounded-xs space-y-3 relative text-left ltr:text-left rtl:text-right">
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <div className="flex items-center gap-2 mt-0.5">
                                        ...
                                        <span className="text-[9px] bg-emerald-950/80 text-emerald-400 border border-emerald-500/30 px-1.5 py-0.5 rounded-xs font-mono font-bold">
                                ...
                                <p className="text-zinc-300 text-xs font-sans leading-relaxed rtl:pr-13 ltr:pl-13">{rev.comment}</p>
                                ...
                                <div className="flex items-center justify-end gap-2 pt-2 border-t border-white/5">
                                  ...
                                  <button
                                    onClick={() => {
                                      if (window.confirm(isAr ? 'حذف التقييم؟\n\nهل أنت متأكد من رغبتك في حذف هذا التقييم؟' : 'Delete Review?\n\nAre you sure you want to remove this review?')) {
                                        setCustomerReviews(prev => prev.filter(r => r.id !== rev.id));
                                        dispatchNotification({
                                          type: 'system',
                                          variant: 'toast',
                                          title: isAr ? 'تم الحذف' : 'Deleted',
                                          message: isAr ? 'تم حذف التقييم بنجاح.' : 'Review removed successfully.'
                                        });
                                      }
                                    }}
                    ...
                    >
                      {selectedInvoice ? (
                        ...
                        <div className="bg-[#060606]/80 border border-white/10 rounded-sm p-6 space-y-6 shadow-xl">
                          ...
                          <div className="space-y-1">
                            <h3 className="text-white text-base font-display font-bold uppercase tracking-wider flex items-center gap-2">
                              <FileText className="w-5 h-5 text-[#D4AF37]" /> {isAr ? 'الفواتير والإيصالات الضريبية' : 'Invoices & Receipts'}
                            </h3>
                            <p className="text-zinc-400 text-xs font-sans leading-relaxed">
                              {isAr 
                                ? 'تصفح، حمّل، واطبع فواتير مشترياتك وإيصالات سداد المعاملات المالية المعتمدة.' 
                            ...
                            <div className="p-12 border border-dashed border-white/10 rounded-xs text-center space-y-4 bg-black/40">
                              ...
                              <div className="space-y-1">
                                <h4 className="text-white font-semibold text-sm uppercase font-display tracking-wider">
                                  {isAr ? 'لا توجد فواتير متاحة' : 'No invoices available.'}
                                </h4>
                                <p className="text-zinc-400 text-xs font-sans max-w-sm mx-auto">
                                  {isAr 
                                    ? 'ستظهر فواتيرك الضريبية المعتمدة هنا تلقائياً فور تأكيد أي طلب سداد ناجح.' 
        ...
        {activeDashboardTab === 'admin' && (
          <StaffDashboard
            ...
            setCurrentPage={setCurrentPage}
            notificationEngine={activeNotificationEngine}
                ...
                <div className="space-y-2 pb-2">
                  ...
                  <div className="bg-zinc-950/20 border border-white/5 p-3.5 rounded-xs space-y-2">
                    {selectedDetailedOrder.items.map((it, idx) => (
                      <div key={idx} className="flex justify-between items-center text-xs">
                        <div className="flex flex-col gap-0.5 text-left">
                          <span className="text-white font-medium">{it.name} <strong className="text-zinc-500 font-sans tabular-nums-fix">×{it.quantity}</strong></span>
                          {it.selectedOption && (
                            <span className="text-zinc-500 text-[9px] font-mono tracking-wide">{it.selectedOption}</span>
                          )}
                        </div>
                        <span className="text-zinc-400 font-sans tabular-nums-fix">{formatCurrency(it.price * it.quantity)} SAR</span>
                      </div>
                    ))}
                    <div className="pt-2 border-t border-white/5 flex justify-between items-center text-xs font-semibold">
                      ...
                      <span className="text-[#D4AF37] font-sans tabular-nums-fix">{formatCurrency(selectedDetailedOrder.total)} SAR</span>
                    </div>
                  </div>
                </div>
