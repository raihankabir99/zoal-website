...
const BUCKETS = [
  ...
  { id: 'documents', name: 'Documents', description: 'Catalogs, PDFs & manuals', public: true, accepted: 'PDF, DOCX, JPEG, PNG' },
  { id: 'invoices', name: 'Invoices (Private)', description: 'Customer receipts & orders', public: false, accepted: 'PDF, JPEG, PNG' }
];
...
export default function SupabaseStoragePanel() {
  ...
  const handleFileUpload = async (file: File) => {
    // Client-side quick security check
    const forbiddenExts = ['.php', '.exe', '.bat', '.sh', '.js', '.ts', '.html', '.htm', '.dll', '.cmd'];
    const fileName = file.name.toLowerCase();
    if (forbiddenExts.some(ext => fileName.endsWith(ext))) {
      setError(`Security Violation: Extension is strictly prohibited for security reasons.`);
      return;
    }

    if (file.size > (activeBucket === 'documents' ? 20 : 10) * 1024 * 1024) {
      setError(`File size exceeds the ${activeBucket === 'documents' ? '20MB' : '10MB'} limit for this bucket.`);
      return;
    }
    ...
    <div className="bg-charcoal-dark text-white border border-white/10 rounded-lg p-6 max-w-7xl mx-auto my-8 font-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 border-b border-white/10 pb-5">
        <div>
          ...
          <h2 className="text-2xl font-serif text-white font-semibold">Supabase Store Storage</h2>
          <p className="text-gray-400 text-sm mt-1">
            Manage product images, category banners, lookbooks, blogs, and secure invoices using high-grade CDN storage.
          </p>
        </div>
        <button 
