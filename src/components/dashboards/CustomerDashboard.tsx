...
function mapApiOrder(row: any): Order {
  return {
    ...
    customerName: row.customer_name || '', email: row.email || '', phone: row.phone || '',
    address: row.shipping_address || row.address || '', paymentMethod: row.payment_method || '—', trackingNumber: row.tracking_number || '',
  };
}
...
export default function CustomerDashboard({ customerSubTab, setSidebarOpen, selectedOrder, setSelectedOrder, orders }: CustomerDashboardProps) {
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      ...
      if (!token) {
        if (!cancelled) { setAuthoritativeOrders([]); setOrdersLoading(false); }
        return;
      }
      try {
  ...
  const totals = useMemo(() => {
    ...
    return { orders: customerOrders.length, active: activeOrders.length, completed: customerOrders.filter((order) => order.status === 'Completed').length, spent: activeOrders.reduce((sum, order) => sum + (Number(order.total) || 0), 0) };
  }, [customerOrders]);

  if (selectedOrder) return (
    <section className="space-y-4" aria-label="Customer order details">
      <button type="button" onClick={() => setSelectedOrder(null)} className="inline-flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 text-zinc-300 hover:text-white rounded-sm text-xs uppercase tracking-wider"><ArrowLeft className="w-3.5 h-3.5" /> Back to Orders</button>
      <div className="bg-zinc-950 border border-white/5 rounded-sm p-5 space-y-4">
        <div><p className="text-[9px] uppercase tracking-[0.3em] text-zinc-500">Order</p><h2 className="text-lg font-semibold text-white mt-1">{selectedOrder.id}</h2></div>
  ...
  if (customerSubTab === 'invoices') return (
    <section className="space-y-4" aria-label="Customer invoices">
      <div className="flex items-center justify-between gap-3"><div><p className="text-[9px] uppercase tracking-[0.3em] text-zinc-500">Customer Area</p><h2 className="text-lg font-semibold text-white mt-1">Invoices & Receipts</h2></div><button type="button" onClick={() => setSidebarOpen(true)} className="lg:hidden px-3 py-2 bg-white/5 border border-white/10 text-zinc-300 rounded-sm text-xs">Menu</button></div>
      <div className="bg-zinc-950 border border-white/5 rounded-sm p-8 text-center text-zinc-500 text-sm">Invoices & receipts are not available yet. Your order records remain available under My Orders.</div>
    </section>
  );

  if (customerSubTab === 'orders' || customerSubTab === 'track') return (
    <section className="space-y-4" aria-label="Customer orders">
      <div className="flex items-center justify-between gap-3"><div><p className="text-[9px] uppercase tracking-[0.3em] text-zinc-500">Customer Area</p><h2 className="text-lg font-semibold text-white mt-1">{customerSubTab === 'track' ? 'Track Orders' : 'My Orders'}</h2></div><button type="button" onClick={() => setSidebarOpen(true)} className="lg:hidden px-3 py-2 bg-white/5 border border-white/10 text-zinc-300 rounded-sm text-xs">Menu</button></div>
      {ordersLoading ? <div className="bg-zinc-950 border border-white/5 rounded-sm p-8 text-center text-zinc-500 text-sm">Loading customer orders…</div> : customerOrders.length === 0 ? <div className="bg-zinc-950 border border-white/5 rounded-sm p-8 text-center text-zinc-500 text-sm">No customer orders are available.</div> : <div className="space-y-2">{customerOrders.map((order) => <button key={order.id} type="button" onClick={() => setSelectedOrder(order)} className="w-full text-left bg-zinc-950 border border-white/5 hover:border-[#D4AF37]/30 rounded-sm p-4 transition-colors"><div className="flex flex-col md:flex-row md:items-center justify-between gap-3"><div><p className="text-sm font-semibold text-white">{order.id}</p><p className="text-[10px] text-zinc-500 mt-1">{order.date || 'Date unavailable'}</p></div><div className="flex items-center gap-3"><span className="inline-flex items-center gap-1.5 text-xs text-zinc-300">{getStatusIcon(order.status)}{getStatusLabel(order.status)}</span><span className="text-sm font-semibold text-[#D4AF37]">{Number(order.total || 0).toFixed(2)}</span></div></div></button>)}</div>}
    </section>
  );
