...
import { Mail, Phone, Lock, User, Sparkles, ShieldCheck, ArrowRight, ArrowLeft, Loader2, CheckCircle2, AlertCircle, Eye, EyeOff, Chrome, Facebook, KeyRound, X } from 'lucide-react';
...
export default function AuthPage({
        ...
        focusCheckTimeout = setTimeout(async () => {
          try {
            ...
            if (session) {
              ...
              setSuccessMsg('Account session established successfully!');
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook') => {
    ...
    try {
      const { error } = await supabaseClient.auth.signInWithOAuth({
        provider,
        options: {
          redirectTo: window.location.origin + '/auth/callback' // Ensure this URL is configured in Supabase
        }
      });
  ...
  const triggerOtpSend = async (emailInput: string) => {
    ...
    try {
      ...
      setSuccessMsg(`A direct verification code has been dispatched to ${emailInput}`);
      setView('otp_verify');
      ...
      setError(err.message || 'Communication with authentication service failed.');
    } finally {
      setSocialLoading(null);
    }
  };
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabaseClient.auth.resetPasswordForEmail(emailOrPhone.trim(), {
  ...
  return (
    <div className={isModal ? "w-full relative text-white selection:bg-[#D4AF37]/30 selection:text-white" : "bg-black text-white min-h-screen flex items-center justify-center relative overflow-hidden px-4 py-8 sm:py-12 md:py-16 selection:bg-[#D4AF37]/30 selection:text-white"}>
      ...
      <div className={isModal ? "relative w-full" : "relative w-full max-w-xl bg-[#09090b]/85 border border-white/10 rounded-xl shadow-[0_24px_70px_rgba(0,0,0,0.95),0_0_40px_rgba(212,175,55,0.03)] backdrop-blur-xl z-10 overflow-hidden min-h-[580px] lg:min-h-[640px]"}>
        ...
        {onCancel && !hideClose && (
          <button
            type="button"
            onClick={onCancel}
            className="absolute top-4 right-4 rtl:right-auto rtl:left-4 z-50 text-zinc-400 hover:text-[#D4AF37] active:text-[#D4AF37] p-2.5 rounded-full transition-colors cursor-pointer block sm:hidden focus:outline-none focus:ring-1 focus:ring-[#D4AF37]/30"
            aria-label="Close"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        {/* ====================================================
            ...
            ==================================================== */}
        <div id="auth-portal-card" className={isModal ? "w-full p-2 sm:p-4 flex flex-col justify-between relative z-10" : "w-full p-6 sm:p-10 md:p-12 flex flex-col justify-between relative z-10"}>
          ...
          <div className="my-auto max-w-md w-full mx-auto space-y-6">
            ...
            <div className="text-center">
              ...
              <p className="text-zinc-500 text-[10px] sm:text-xs mt-1 uppercase tracking-widest font-sans">
                ...
                {view === 'register' && 'JOIN THE ZOAL CIRCLE'}
                {view === 'forgot' && 'Enter your email to receive a reset code'}
                {view === 'verify' && 'Enter the 6-digit verification code sent to your email'}
                {view === 'reset' && 'Complete pass alteration keys'}
                {view === 'otp_request' && 'ENTER YOUR EMAIL TO RECEIVE OTP'}
                {view === 'otp_verify' && 'Input the 6-digit verification code sent to your inbox'}
              </p>
            </div>
                  ...
                  <div className="space-y-1.5 text-left">
                    ...
                    <div className="relative">
                      ...
                      <button
                        ...
                        className="absolute right-0 top-0 h-full w-11 flex items-center justify-center text-[#D4AF37]/80 hover:text-[#D4AF37] focus:text-[#D4AF37] transition-colors cursor-pointer focus:outline-none focus:ring-1 focus:ring-[#D4AF37]/40 rounded-r-md"
                ...
                <div className="text-center">
                  {otpCountdown > 0 ? (
                    <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-sans">
                      Resend key in <span className="text-white font-mono font-bold">{Math.floor(otpCountdown / 60)}:{(otpCountdown % 60).toString().padStart(2, '0')}</span>
                    </p>
                  ) : (
                    <button
                      type="button"
            ...
            {view === 'register' && (
              <form onSubmit={handleRegisterSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5 text-left col-span-1 sm:col-span-2 md:col-span-1">
                    <label htmlFor="reg-password" className="text-[9px] text-zinc-400 uppercase tracking-widest block font-bold">Password</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) => setPassword(e.target.value)}
                        onKeyDown={handlePasswordKeyDown}
                        className="w-full bg-black border border-white/10 rounded-md py-3.5 pl-10 pr-11 text-xs text-white focus:outline-none focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37]/30 font-sans"
                      />
                  ...
                  <div className="space-y-1.5 text-left">
                    <label htmlFor="reg-confirmPassword" className="text-[9px] text-zinc-400 uppercase tracking-widest block font-bold">CONFIRM PASSWORD</label>
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                      <input
            ...
            {view === 'verify' && (
              <OTPVerificationView
                ...
                onVerify={(otp) => {
                  ...
                  handleVerifySubmit({ preventDefault: () => {} } as any);
                }}
                onResend={() => {
                ...
                <div className="space-y-1.5 text-left">
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={password}
                      onChange={(e) => setPassword(e.target.value)}
                      className="w-full bg-black border border-white/10 rounded-md py-3 pl-4 pr-11 text-xs text-white focus:outline-none focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37]/45 font-sans"
                    />
                    <button
                      type="button"
          ...
          <div className="mt-8 pt-6 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-3 text-[9px] uppercase tracking-[0.15em] text-zinc-500 font-sans font-medium text-center sm:text-left select-none">
            <div className="flex items-center gap-1.5">
              ...
              <span>Protected by System Security</span>
            </div>
