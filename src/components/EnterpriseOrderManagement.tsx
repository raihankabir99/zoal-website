...
export default function EnterpriseOrderManagement({
  ...
  const getLocalizedStatus = (status: string) => {
    ...
    const statusMap: Record<string, string> = {
      ...
      'Confirmed': 'تم التأكيد والتحقق',
      'Processing': 'تحت التجهيز والتحضير',
      'Preparing': 'تحت التجهيز والتحضير',
      'Packed': 'مغلف ومؤمن',
      'Ready for Shipping': 'جاهز للشحن اللوجستي',
      'Shipped': 'تم الشحن',
      ...
      'Refund Requested': 'طلب استرجاع معلق',
      'Refund Approved': 'تم قبول الاسترجاع',
      'Refund Completed': 'تم إعادة المبلغ بالكامل',
    };
    return statusMap[status] || status;
  };

  const getLocalizedPaymentStatus = (pStatus: string) => {
    if (!isAr) return pStatus;
    const pStatusMap: Record<string, string> = {
      'Paid': 'مدفوع',
      'Unpaid': 'غير مدفوع',
      'Pending': 'معلق السداد',
      'Failed': 'فشلت',
      'Refunded': 'مسترجع بالكامل',
      'Partially Refunded': 'مسترجع جزئياً',
    };
    return pStatusMap[pStatus] || pStatus;
  };
        ...
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="text-[9px] font-mono uppercase tracking-[0.3em] text-gold-pure bg-gold-pure/10 px-2 py-0.5 rounded-xs border border-gold-pure/20">
              {userRole === 'admin' 
                ? (isAr ? 'لوحة التحكم للمشرف' : 'Admin Panel') 
                : userRole === 'staff' 
                ? (isAr ? 'جناح إدارة وتجهيز الطلبات' : 'Staff Fulfillment Suite') 
                : (isAr ? 'الدعم الفني للعملاء' : 'Customer Support')}
            </span>
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
            <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest">{isAr ? 'مركز الإدارة v2.4' : 'Management Center v2.4'}</span>
          </div>
          <h1 className="text-2xl font-bold tracking-widest font-display text-white uppercase">
            {userRole === 'customer' 
              ? (isAr ? 'سجل الطلبات السابقة' : 'Order History') 
              : (isAr ? 'إدارة الطلبات الفاخرة' : 'Order Management')}
          </h1>
          <p className="text-xs text-zinc-400 mt-1 font-sans">
            {userRole === 'customer' 
              ? (isAr 
                  ? 'عرض وتتبع طلباتك السابقة وتفاصيل الفواتير المسجلة.' 
                  : 'View and manage your previous orders and purchase history.') 
              : (isAr 
                  ? 'إدارة طلبات العملاء وتتبع مراحل الشحن والتوصيل ومتابعة تقارير المبيعات الفورية.' 
        ...
        {userRole !== 'customer' && (
          <div className="flex bg-zinc-950 p-1 border border-white/5 rounded-xs shrink-0 self-start md:self-center font-mono text-[10px]">
            <RoleGuard role="admin" userRole={userRole}>
              ...
              >
                <BarChart3 className="w-3.5 h-3.5" /> {isAr ? 'نظرة عامة' : 'Overview'}
              </button>
            </RoleGuard>
            ...
            >
              <ClipboardList className="w-3.5 h-3.5" /> {isAr ? 'جميع الطلبات' : 'All Orders'}
            </button>
            ...
            <RoleGuard role="staff" userRole={userRole}>
              ...
              >
                <PackageCheck className="w-3.5 h-3.5" /> {isAr ? 'مهامي المعينة' : 'My Assigned'}
              </button>
            </RoleGuard>
            ...
            <RoleGuard role="admin" userRole={userRole}>
              ...
              >
                <FileSpreadsheet className="w-3.5 h-3.5" /> {isAr ? 'تقارير المبيعات' : 'Sales Reports'}
              </button>
            </RoleGuard>
          </div>
        )}
            ...
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-white tabular-nums-fix">{stats.totalCount}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-amber-400 tabular-nums-fix">{stats.pending}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-blue-300 tabular-nums-fix">{stats.confirmed}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-purple-300 tabular-nums-fix">{stats.processing}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-pink-300 tabular-nums-fix">{stats.packed}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-cyan-300 tabular-nums-fix">{stats.readyForShipping}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-[#22d3ee] tabular-nums-fix">{stats.shipped}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-emerald-300 tabular-nums-fix">{stats.delivered}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-rose-400 tabular-nums-fix">{stats.cancelled}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-zinc-300 tabular-nums-fix">{stats.refundRequested}</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-gold-pure truncate tabular-nums-fix">{formatCurrency(stats.totalRev)} SAR</p>
                ...
                <div className="bg-zinc-950 border border-white/5 p-3 rounded-xs relative group hover:border-gold-pure/30 transition-all text-left">
                  ...
                  <p className="text-xl font-bold font-sans text-purple-200 tabular-nums-fix">{stats.monthlyOrders}</p>
          ...
          {(activeTab === 'ledger' || (activeTab === 'my-tasks' && hasRole('staff'))) && (
            ...
            >
              ...
              <div className="bg-[#09090b]/80 border border-white/5 rounded-xs p-4 space-y-4 text-left">
                <div className="flex items-center justify-between border-b border-white/5 pb-2">
                  <span className="text-[10px] uppercase tracking-wider font-mono text-white font-bold flex items-center gap-1.5">
                    <Filter className="w-3.5 h-3.5 text-gold-pure" /> {isAr ? 'تصفية قائمة الطلبات المتقدمة' : 'Order Matrix Filter'}
                  </span>
                  <span className="text-[9px] font-mono text-zinc-500">
                    {isAr 
                      ? `تم العثور على ${filteredOrders.length} طلبات تطابق المعايير` 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono uppercase text-zinc-500 block">
                      {!hasRole('staff') 
                        ? (isAr ? 'البحث في الطلبات' : 'Search Orders') 
                        : (isAr ? 'البحث باسم العميل / رقم الطلب' : 'Search Client / Serial')}
                    </label>
                    <div className="flex items-center gap-2 bg-black border border-white/10 px-2.5 py-1.5 rounded-xs">
                      <Search className="w-3.5 h-3.5 text-zinc-600" />
                      <input
                        type="text"
                        placeholder={isAr ? 'المعرّف، الاسم، الهاتف...' : 'ID, Name, Phone...'}
                        value={searchQuery}
                  ...
                  {/* Status Selection */}
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono uppercase text-zinc-500 block">{isAr ? 'مرحلة التوصيل والشحن' : 'Delivery Stage'}</label>
                    <select
                      value={statusFilter}
                      onChange={(e) => setStatusFilter(e.target.value)}
                      className="bg-black w-full border border-white/10 text-zinc-300 text-xs py-1.5 px-2 rounded-xs outline-none focus:border-gold-pure font-sans"
                    >
                      <option value="all">{isAr ? 'جميع المراحل' : 'All Stages'}</option>
                      <option value="Pending">{isAr ? 'قيد المعالجة الأولية' : 'Pending'}</option>
                      <option value="Confirmed">{isAr ? 'تم التأكيد والتحقق' : 'Confirmed'}</option>
                      <option value="Processing">{isAr ? 'تحت التجهيز والتحضير' : 'Processing'}</option>
                      <option value="Packed">{isAr ? 'مغلف ومؤمن' : 'Packed'}</option>
                      <option value="Ready for Shipping">{isAr ? 'جاهز للتسليم اللوجستي' : 'Ready for Shipping'}</option>
                      <option value="Shipped">{isAr ? 'تم شحن الطرد' : 'Shipped'}</option>
                      <option value="Out for Delivery">{isAr ? 'خارج للتوصيل الفوري' : 'Out for Delivery'}</option>
                      <option value="Delivered">{isAr ? 'تم التوصيل للعميل' : 'Delivered'}</option>
                      <option value="Cancelled">{isAr ? 'تم إلغاء المعاملة' : 'Cancelled'}</option>
                      <option value="Returned">{isAr ? 'مرتجع للمستودع' : 'Returned'}</option>
                      <option value="Refund Requested">{isAr ? 'طلب استرجاع مالي معلق' : 'Refund Requested'}</option>
                      <option value="Refund Approved">{isAr ? 'تم قبول الاسترجاع المالي' : 'Refund Approved'}</option>
                      <option value="Refund Completed">{isAr ? 'تم إعادة المبلغ بالكامل' : 'Refund Completed'}</option>
                    </select>
                  </div>

                  {/* Payment Selection */}
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono uppercase text-zinc-500 block">{isAr ? 'حالة السداد / القناة' : 'Payment / Channel'}</label>
                    {hasRole('admin') ? (
                      <select
                        ...
                        className="bg-black w-full border border-white/10 text-zinc-300 text-xs py-1.5 px-2 rounded-xs outline-none focus:border-gold-pure font-sans"
                      >
                        <option value="all">{isAr ? 'جميع المعاملات' : 'All Payments'}</option>
                        <option value="paid">{isAr ? 'مدفوعة ومؤكدة' : 'Paid'}</option>
                        <option value="pending">{isAr ? 'معلقة السداد' : 'Pending'}</option>
                        <option value="failed">{isAr ? 'مرفوضة / فشلت' : 'Failed'}</option>
                        <option value="refunded">{isAr ? 'مسترجعة بالكامل' : 'Refunded'}</option>
                        <option value="cancelled">{isAr ? 'ملغاة نهائياً' : 'Cancelled'}</option>
                        <option value="cod">{isAr ? 'الدفع نقداً عند الاستلام' : 'Cash on Delivery (COD)'}</option>
                        <option value="visa">{isAr ? 'بطاقات فيزا الائتمانية' : 'Visa Card'}</option>
                        <option value="mastercard">{isAr ? 'بطاقات ماستركارد' : 'Mastercard'}</option>
                        <option value="mada">{isAr ? 'شبكة مدى الرقمية' : 'Mada Network'}</option>
                        <option value="applepay">{isAr ? 'أبل باي الآمنة' : 'Apple Pay'}</option>
                      </select>
                    ) : (
                      <select
                        value={paymentFilter}
                      ...
                      >
                        <option value="all">{isAr ? 'جميع حالات السداد' : 'All Payment States'}</option>
                        ...
                        <option value="Partially Refunded">{isAr ? 'مسترجعة جزئياً' : 'Partially Refunded'}</option>
                      </select>
                    )}
                  </div>
                  ...
                  {/* Date Picker */}
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono uppercase text-zinc-500 block">{isAr ? 'تاريخ المعاملة' : 'Placement Date'}</label>
                    <input
                      type="date"
                      value={dateFilter}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-white/5">
                  <div className="flex gap-2 col-span-2">
                    <div className="w-1/2 space-y-1">
                      <label className="text-[9px] font-mono uppercase text-zinc-500 block">{isAr ? 'الحد الأدنى للسعر (ريال)' : 'Min Price (SAR)'}</label>
                      <input
                        type="number"
                        placeholder="e.g. 100"
                        value={minAmount}
                    ...
                    <div className="w-1/2 space-y-1">
                      <label className="text-[9px] font-mono uppercase text-zinc-500 block">{isAr ? 'الحد الأقصى للسعر (ريال)' : 'Max Price (SAR)'}</label>
                      <input
                        type="number"
                  ...
                  <div className="flex items-end justify-end gap-3 text-[10px] font-mono">
                    ...
                    >
                      <FileSpreadsheet className="w-3.5 h-3.5 text-gold-pure" /> {isAr ? 'تصدير ملف Excel / CSV' : 'Export CSV'}
                    </button>
              ...
              <div className="hidden md:block bg-[#050507] border border-white/5 rounded-xs overflow-hidden">
                <table className="w-full text-left text-xs divide-y divide-white/5" dir={isAr ? 'rtl' : 'ltr'}>
                  <thead className="bg-black text-zinc-500 text-[8.5px] uppercase tracking-widest font-mono">
                    <tr>
                      ...
                      <th className="p-4">{isAr ? 'رمز الطلب' : 'Order Code'}</th>
                      <th className="p-4">
                        {!hasRole('staff') 
                          ? (isAr ? 'تفاصيل الطلب' : 'Order Details') 
                          : (isAr ? 'تفاصيل العميل' : 'Client Detail')}
                      </th>
                      <th className="p-4">{isAr ? 'التاريخ' : 'Date'}</th>
                      <th className="p-4">{isAr ? 'تفاصيل الدفع' : 'Payment Info'}</th>
                      <th className="p-4">{isAr ? 'طريقة الشحن' : 'Courier Method'}</th>
                      <th className="p-4">{isAr ? 'مرحلة الشحن والتنفيذ' : 'Fulfillment Stage'}</th>
                      {hasRole('staff') && <th className="p-4">{isAr ? 'فريق العمل المعين' : 'Assigned Crew'}</th>}
                      <th className="p-4 text-right">{isAr ? 'المجموع الإجمالي' : 'Sum Total'}</th>
                    ...
                    {paginatedOrders.length > 0 ? (
                          ...
                          <td className="p-4">
                            <div className="space-y-0.5">
                              ...
                              <span className={`inline-block px-1.5 py-0.5 rounded-sm text-[8px] uppercase tracking-wider font-semibold font-mono ${
                                ...
                                {getLocalizedPaymentStatus(o.paymentStatus || 'Unpaid')}
                              </span>
                            </div>
                          </td>
                          <td className="p-4 text-zinc-400 font-mono text-[10px]">
                            {o.shippingMethod === 'AL ZOAL Premium Ship' 
                              ? (isAr ? 'شحن زُوال الفاخر' : 'AL ZOAL Premium Ship') 
                              : (o.shippingMethod || 'AL ZOAL Premium Ship')}
                          </td>
                          <td className="p-4">
                            <span className={`px-2 py-0.5 rounded-sm text-[8.5px] uppercase tracking-wider font-semibold font-mono ${
                              ...
                              {getLocalizedStatus(o.status)}
                            </span>
                          </td>
                          {hasRole('staff') && (
                            <td className="p-4 font-mono text-[10px] text-zinc-300">
                              {o.assignedStaff || (
                                <span className="text-zinc-600 italic">{isAr ? 'غير معيّن' : 'Unassigned'}</span>
                              )}
                            </td>
                          )}
                          <td className="p-4 text-right font-sans font-bold text-white tabular-nums-fix">
                            {formatCurrency(o.total)} {isAr ? 'ريال' : 'SAR'}
                          </td>
                      ...
                      <tr>
                        <td colSpan={10} className="p-8 text-center text-zinc-500 font-sans italic">
                          {isAr 
                            ? 'لم يتم العثور على أي طلبات تطابق معايير التصفية المحددة.' 
              ...
              {/* MOBILE CARD VIEW */}
              <div className="block md:hidden space-y-3" dir={isAr ? 'rtl' : 'ltr'}>
                {paginatedOrders.length > 0 ? (
                  paginatedOrders.map((o) => (
                    <div
                      key={o.id}
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="space-y-0.5">
                          <span className="text-[8px] font-mono text-zinc-500 uppercase block">
                            {!hasRole('staff') ? (isAr ? 'الطلب' : 'Order') : (isAr ? 'العميل' : 'Client')}
                          </span>
                          <span className="text-zinc-200 font-semibold">{!hasRole('staff') ? o.id : o.customerName}</span>
                        </div>
                        <div className="space-y-0.5">
                          <span className="text-[8px] font-mono text-zinc-500 uppercase block">{isAr ? 'التاريخ' : 'Date'}</span>
                          <span className="text-zinc-400 font-mono">{o.date}</span>
                        </div>
                        <div className="space-y-0.5">
                          <span className="text-[8px] font-mono text-zinc-500 uppercase block">{isAr ? 'المجموع الإجمالي' : 'Sum Value'}</span>
                          <span className="text-white font-sans font-bold tabular-nums-fix">{formatCurrency(o.total)} {isAr ? 'ريال' : 'SAR'}</span>
                        </div>
                        <div className="space-y-0.5">
                          <span className="text-[8px] font-mono text-zinc-500 uppercase block">{isAr ? 'بوابة الدفع' : 'Payment Gate'}</span>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-white/5 text-[10px]">
                        {hasRole('staff') && (
                          <span className="font-mono text-zinc-500">
                            {isAr ? `الموظف المعين: ${o.assignedStaff || 'غير معيّن'}` : `Staff: ${o.assignedStaff || 'Unassigned'}`}
                          </span>
                        )}
                        <button className="text-gold-pure uppercase font-bold tracking-widest font-mono hover:text-white cursor-pointer">
                          {isAr ? 'عرض التفاصيل والخيارات' : 'View details'}
                        </button>
                      </div>
                    </div>
                  ))
                  ...
                  <div className="p-12 text-center border border-dashed border-white/10 bg-zinc-950/40 rounded-xs space-y-4">
                    ...
                    <div className="space-y-1">
                      <h4 className="text-white font-semibold text-sm uppercase font-display tracking-wider">{isAr ? 'لا يوجد طلبات بعد' : 'No Orders Yet'}</h4>
                      <p className="text-zinc-400 text-xs font-sans">{isAr ? 'ابدأ التسوق الآن لتقديم طلبك الأول.' : 'Start shopping to place your first order.'}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-between bg-[#060608] border border-white/5 rounded-xs p-3 font-mono text-[10px]">
                <span className="text-zinc-500">
                  {isAr 
                    ? `عرض الصفحة ${currentPageNum} من أصل ${totalPages}` 
                ...
                <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-4 bg-black border border-white/5 rounded-xs text-left">
                      ...
                      <p className="text-2xl font-bold font-sans text-gold-pure tabular-nums-fix">{formatCurrency(stats.totalRev)} SAR</p>
                    </div>
                    <div className="p-4 bg-black border border-white/5 rounded-xs text-left">
                      <span className="text-[8px] font-mono text-zinc-500 uppercase block mb-1">Average Order Value (AOV)</span>
                      <p className="text-2xl font-bold font-sans text-white tabular-nums-fix">
                  ...
                  <div className="divide-y divide-white/5 font-sans">
                    {orders.slice(0, 3).map((o, index) => (
                      <div key={index} className="py-3 flex items-center justify-between text-xs">
                        ...
                        <span className="font-sans text-zinc-300 font-bold tabular-nums-fix">{formatCurrency(o.items[0]?.price * (o.items[0]?.quantity || 1) || 0)} SAR</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-[10px] font-mono text-zinc-400">
                        ...
                        <div>
                          ...
                          <span className={`inline-block px-1.5 py-0.5 rounded-sm text-[8px] uppercase tracking-wider font-semibold font-mono mt-0.5 ${
                            (selectedOrder.paymentStatus || 'Unpaid') === 'Paid' || (selectedOrder.paymentStatus as string) === 'paid' ? 'bg-emerald-950 text-emerald-400 border border-emerald-500/20' :
                            (selectedOrder.paymentStatus || 'Unpaid') === 'Refunded' ? 'bg-zinc-800 text-zinc-300 border border-white/10' :
                            (selectedOrder.paymentStatus || 'Unpaid') === 'Partially Refunded' || (selectedOrder.paymentStatus as string) === 'partially_refunded' ? 'bg-zinc-800 text-gold-pure border border-gold-pure/20' :
                  ...
                  <div className="p-4 border border-white/5 bg-zinc-950/40 rounded-xs space-y-3">
                        ...
                        <div key={idx} className="pt-2 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-zinc-900 border border-white/10 rounded-xs overflow-hidden shrink-0">
                              <SafeImage
                                product={{ name: item.name, image: item.image, images: item.image ? [item.image] : [] }}
                                alt={item.name}
                            ...
                            <div className="text-left">
                              ...
                              <p className="text-[10px] text-zinc-500 font-sans mt-0.5 tabular-nums-fix">
                                {formatCurrency(item.price)} SAR x {item.quantity}
                              </p>
                            </div>
                          </div>
                          <span className="font-sans font-bold text-zinc-200 tabular-nums-fix">
                    ...
                    <div className="border-t border-white/5 pt-3 space-y-1.5 text-zinc-400 font-mono text-[11px]">
                      <div className="flex justify-between">
                        ...
                        <span className="text-zinc-300 tabular-nums-fix">{formatCurrency(selectedOrder.subtotal)} SAR</span>
                      </div>
                      <div className="flex justify-between">
                        <span>VAT (15%)</span>
                        <span className="text-zinc-300 tabular-nums-fix">{formatCurrency(selectedOrder.tax || selectedOrder.subtotal * 0.15)} SAR</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Shipping Fee</span>
                        <span className="text-zinc-300 tabular-nums-fix">{formatCurrency(selectedOrder.shipping)} SAR</span>
                      </div>
                      {selectedOrder.discount > 0 && (
                        <div className="flex justify-between text-gold-pure font-semibold">
                          <span>Voucher Discount</span>
                          <span className="tabular-nums-fix">-{formatCurrency(selectedOrder.discount)} SAR</span>
                        </div>
                      )}
                      <div className="flex justify-between text-white font-bold text-sm border-t border-white/5 pt-2 font-mono">
                        <span>Grand Total Bill</span>
                        <span className="text-gold-pure tabular-nums-fix">{formatCurrency(selectedOrder.total)} SAR</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 border border-gold-pure/20 bg-zinc-950/60 rounded-xs space-y-3">
                    ...
                    {hasRole('admin') && (selectedOrder.paymentStatus === 'Paid' || (selectedOrder.paymentStatus as string) === 'paid' || selectedOrder.status === 'Refund Requested' || selectedOrder.status === 'Completed' || selectedOrder.status === 'Delivered') && (
                                ...
                                try {
                                  ...
                                  setOrders(prev => prev.map(o => {
                                    if (o.id === selectedOrder.id) {
                                      return {
                                        ...o,
                                        status: (amount < o.total ? 'partially_refunded' : 'refunded') as any,
                                        ...
                                        adminNotes: (o.adminNotes || '') + ` [Refund: ${amount} SAR - ${reason}]`
                                      };
                                    }
                                    return o;
                                  }));
                                  ...
                                  setSelectedOrder(prev => {
                                    ...
                                    return {
                                      ...prev,
                                      status: (amount < prev.total ? 'partially_refunded' : 'refunded') as any,
                                      paymentStatus: amount < prev.total ? 'Partially Refunded' : 'Refunded',
                                      refundHistory: [...updatedHistory, newRefund],
                                      paymentTimeline: newPaymentTimeline,
                                      adminNotes: (prev.adminNotes || '') + ` [Refund: ${amount} SAR - ${reason}]`
                                    };
                                  });
                    ...
                    {!hasRole('staff') && (
                      <div className="space-y-3">
                        ...
                        {((selectedOrder.paymentStatus || 'Unpaid') === 'Unpaid' || (selectedOrder.paymentStatus || 'Unpaid') === 'Failed' || (selectedOrder.paymentStatus as string) === 'pending_payment') && (
                          <button
                            onClick={async () => {
                              try {
