...
export default function SupportCenterDashboard({ currentUser, orders, addLog, onBack }: SupportCenterProps) {
  ...
  const fetchTickets = () => {
    ...
    fetch('/api/support/tickets', { headers })
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}: Secure support access denied or server offline.`);
        return res.json();
      })
      .then(data => {
      ...
      .catch(err => {
        ...
        setApiError(err.message || 'Failed to retrieve support tickets from server.');
        setIsLoading(false);
      });
  };
  ...
  const fetchStaffRoster = () => {
    ...
    fetch('/api/support/staff', { headers })
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
  ...
  const fetchAuditLogs = () => {
    ...
    fetch('/api/support/logs', { headers })
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
  ...
  const [kpiMetrics, setKpiMetrics] = useState({
    ...
    slaCompliance: 'N/A',
    trendData: [{ name: 'Today', compliance: 100 }]
  });
  ...
  const fetchReports = () => {
    ...
    fetch('/api/support/reports', { headers })
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
  ...
  useEffect(() => {
    if (Array.isArray(tickets) && tickets.length > 0) {
      if (!selectedTicketId || !tickets.some(t => t && t.id === selectedTicketId)) {
  ...
  const [providerConfigFields, setProviderConfigFields] = useState({
    apiKey: 'MOCK_ZOAL_DEMO_SUPPORT_KEY_PLACEHOLDER',
  ...
  const recordSupportAction = (actionText: string, type = 'Action') => {
    ...
    fetch('/api/support/logs', {
      method: 'POST',
      headers,
      body: JSON.stringify({ action: actionText, type })
    })
      .then(res => {
        if (res.ok) {
          fetchAuditLogs();
      ...
      .catch(err => {
        console.error('Error recording support action:', err);
      });
  };
  ...
  const activeTicket = useMemo(() => {
    ...
    return list.find(t => t && t.id === selectedTicketId) || list[0] || null;
  ...
  const handleAddKBArticle = (e: React.FormEvent) => {
    ...
    fetch('/api/support/kb', {
      ...
      .catch(err => {
        ...
        alert('Failed to save Knowledge Base article on server.');
      });
  };
  ...
  // Support Analytics metrics loaded securely from zoal_support_tickets and zoal_ticket_messages via backend reports API
    ...
    <div className="space-y-8 animate-fade-in text-left font-sans text-white select-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[720px] select-none animate-fade-in">
          ...
          <div className="lg:col-span-3 bg-zinc-950 border border-white/5 rounded-sm flex flex-col overflow-hidden h-full">
            ...
            <div className="flex-grow overflow-y-auto divide-y divide-white/5 p-2 space-y-1.5">
              {isLoading ? (
                <div className="p-6 text-center text-zinc-500 font-mono text-[9px] uppercase tracking-widest space-y-3">
                  <div className="w-5 h-5 border-t-2 border-gold-pure rounded-full animate-spin mx-auto"></div>
                  <p>Loading authoritative ticket registry...</p>
                </div>
              ) : apiError ? (
                <div className="p-6 text-center text-rose-400 font-mono text-[9px] uppercase tracking-widest space-y-4">
                  <p className="border border-rose-500/20 bg-rose-950/20 p-3 rounded-xs leading-relaxed text-zinc-300">
                    Error: {apiError}
                  </p>
                  <button
                    onClick={fetchTickets}
                    className="w-full py-2 bg-rose-950/30 hover:bg-rose-900/40 border border-rose-500/30 hover:border-rose-500/60 rounded-xs font-mono text-white text-[9px] uppercase tracking-wider transition-all cursor-pointer"
                  >
                    Retry Connection
                  </button>
                </div>
      ...
      {activeSubTab === 'knowledge' && (
        <div className="space-y-6 select-none animate-fade-in text-left">
          ...
          {/* Articles list grid */}
          {kbArticles.length === 0 ? (
            <div className="bg-zinc-950 border border-white/5 p-12 text-center rounded-xs space-y-3 font-mono">
              <BookOpen className="w-8 h-8 text-zinc-600 mx-auto" />
              <p className="text-zinc-400 text-xs uppercase tracking-wider">No Knowledge Base articles found in database.</p>
              <p className="text-zinc-600 text-[10px]">Click "Build Article" above to create and publish enterprise documentation.</p>
            </div>
          ) : (
