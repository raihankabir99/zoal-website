      ...
      <div className="bg-zinc-950 border border-white/5 p-4 rounded-xs min-h-[380px]">
        ...
        {activeFormTab === 'general' && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  <option value="cosmetics">Elite Cosmetics & Apothecary</option>
                </select>
              </div>
              <div className="space-y-1">
            ...
            <div className="p-3 bg-gold-pure/5 border border-gold-pure/15 rounded-xs space-y-2 mt-2">
              ...
              {formState.productType === 'Cosmetics' && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="space-y-1">
                    <label className="text-[8.5px] text-zinc-400 uppercase font-mono block">Fragrance Family</label>
                    <input 
                      type="text"
                      value={formState.cosmeticFragrance}
                      onChange={(e) => setFormState(prev => ({ ...prev, cosmeticFragrance: e.target.value }))}
                      placeholder="e.g. Woody Musk / Oud"
                      className="w-full bg-black border border-white/10 p-1.5 rounded-xs text-white font-sans text-[9px]"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[8.5px] text-zinc-400 uppercase font-mono block">Skin Compatibility</label>
                    <select
                      value={formState.cosmeticSkinType}
                      onChange={(e) => setFormState(prev => ({ ...prev, cosmeticSkinType: e.target.value }))}
                      className="w-full bg-black border border-white/10 p-1.5 rounded-xs text-white font-sans text-[9px]"
                    >
                  ...
                  <div className="space-y-1">
                    <label className="text-[8.5px] text-zinc-400 uppercase font-mono block">Organic Content %</label>
                    <input 
                      type="number"
                      value={formState.cosmeticOrganicPct}
                      onChange={(e) => setFormState(prev => ({ ...prev, cosmeticOrganicPct: e.target.value }))}
                      placeholder="e.g. 100"
        ...
        {activeFormTab === 'shipping' && (
          <div className="space-y-4 animate-fade-in">
            ...
            {/* NEW ADDITION: LOGISTICS & PICKUP MATRIX */}
            <h4 className="text-gold-pure font-mono text-[9.5px] uppercase tracking-widest border-t border-b border-white/5 py-2 mt-6 mb-4 flex items-center gap-1.5">
              <Settings className="w-3.5 h-3.5" /> Logistics & Pickup Matrix
            </h4>

            {/* SECTION A: STORE PICKUP DEFINITION */}
            <div className="p-3 bg-white/5 border border-white/5 rounded-xs space-y-4">
              <span className="text-[9px] font-mono text-gold-pure uppercase tracking-widest block font-bold">
                🏪 Store Pickup Logistics & Business Hours
              </span>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[9px] text-zinc-400 uppercase tracking-wider block">Store Pickup Location</label>
                  <textarea 
                    value={formState.pickup_location || ''}
                    onChange={(e) => setFormState(prev => ({ ...prev, pickup_location: e.target.value }))}
                    placeholder={`ZOAL Flagship Store\nAbu Bakr As Siddiq Rd\nAl Hofuf\nSaudi Arabia`}
                    rows={3}
                    className="w-full bg-black border border-white/5 p-2 rounded-xs text-white focus:border-gold-pure outline-none font-sans"
                  />
                  <div className="flex gap-2">
                    <button 
                      type="button"
                      onClick={() => setFormState(prev => ({ ...prev, pickup_location: `ZOAL Flagship Store\nAbu Bakr As Siddiq Rd\nAl Hofuf\nSaudi Arabia` }))}
                      className="text-[8px] text-gold-pure/80 hover:text-gold-pure font-mono uppercase underline cursor-pointer"
                    >
                      Load Flagship Store Default
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="text-[9px] text-zinc-400 uppercase tracking-wider block">Pickup Ready Time</label>
                    <select
                      value={formState.pickup_ready_time || ''}
                      onChange={(e) => setFormState(prev => ({ ...prev, pickup_ready_time: e.target.value }))}
                      className="w-full bg-black border border-white/5 p-2 rounded-xs text-white focus:border-gold-pure outline-none font-sans cursor-pointer"
                    >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="space-y-1">
                      <label className="text-[8.5px] text-zinc-500 uppercase tracking-wider block">Store Open</label>
                      <input 
                        type="text" 
                        value={formState.pickup_open || ''}
                        onChange={(e) => setFormState(prev => ({ ...prev, pickup_open: e.target.value }))}
                        placeholder="09:00 AM"
                        className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-mono"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[8.5px] text-zinc-500 uppercase tracking-wider block">Store Close</label>
                      <input 
                        type="text" 
                        value={formState.pickup_close || ''}
                        onChange={(e) => setFormState(prev => ({ ...prev, pickup_close: e.target.value }))}
                        placeholder="11:00 PM"
                        className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-mono"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[8.5px] text-zinc-500 uppercase tracking-wider block">Friday Schedule</label>
                      <input 
                        type="text" 
                        value={formState.pickup_friday_schedule || ''}
                        onChange={(e) => setFormState(prev => ({ ...prev, pickup_friday_schedule: e.target.value }))}
                        placeholder="Custom"
                        className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-mono"
                      />
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[9px] text-zinc-400 uppercase tracking-wider block">Pickup Instructions</label>
                  <textarea 
                    value={formState.pickup_instruction || ''}
                    onChange={(e) => setFormState(prev => ({ ...prev, pickup_instruction: e.target.value }))}
                    placeholder={`Bring Order ID.\nBring SMS.\nShow QR Code.\nCollect from Pickup Counter.`}
                    rows={3}
                    className="w-full bg-black border border-white/5 p-2 rounded-xs text-white focus:border-gold-pure outline-none font-sans"
                  />
                  <div className="flex gap-2">
                    <button 
                      type="button"
                      onClick={() => setFormState(prev => ({ ...prev, pickup_instruction: `Bring Order ID.\nBring SMS.\nShow QR Code.\nCollect from Pickup Counter.` }))}
                      className="text-[8px] text-gold-pure/80 hover:text-gold-pure font-mono uppercase underline cursor-pointer"
                    >
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-2">
                    <div className="space-y-1">
                      <label className="text-[8.5px] text-zinc-500 uppercase tracking-wider block">Pickup Phone</label>
                      <input 
                        type="text" 
                        value={formState.pickup_phone || ''}
                        onChange={(e) => setFormState(prev => ({ ...prev, pickup_phone: e.target.value }))}
                        placeholder="+966..."
                        className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-mono"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[8.5px] text-zinc-500 uppercase tracking-wider block">WhatsApp</label>
                      <input 
                        type="text" 
                        value={formState.pickup_whatsapp || ''}
                        onChange={(e) => setFormState(prev => ({ ...prev, pickup_whatsapp: e.target.value }))}
                        placeholder="+966..."
                        className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-mono"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[8.5px] text-zinc-500 uppercase tracking-wider block">Pickup Email</label>
                      <input 
                        type="email" 
                        value={formState.pickup_email || ''}
                        onChange={(e) => setFormState(prev => ({ ...prev, pickup_email: e.target.value }))}
                        placeholder="store@zoal.sa"
                        className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-sans"
                      />
                    </div>
                  </div>

                  <div className="space-y-1">
                    <label className="text-[9px] text-zinc-400 uppercase tracking-wider block">Google Maps Location URL</label>
                    <input 
                      type="url" 
                      value={formState.pickup_map_url || ''}
                      onChange={(e) => setFormState(prev => ({ ...prev, pickup_map_url: e.target.value }))}
            ...
            {/* SECTION B: DELIVERY SCOPE & ZONES */}
            <div className="p-3 bg-white/5 border border-white/5 rounded-xs space-y-4">
              <span className="text-[9px] font-mono text-gold-pure uppercase tracking-widest block font-bold">
                🌐 Delivery Scopes & Geographic Zones
              </span>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[9px] text-zinc-400 uppercase tracking-wider block font-mono">Fulfillment & Shipping Scope</label>
                  <div className="grid grid-cols-2 gap-2">
                    {['Local Delivery', 'Pickup', 'Saudi Shipping', 'International', 'Digital'].map(scope => {
                      const arr = Array.isArray(formState.shipping_scope) ? formState.shipping_scope : [];
                      const isChecked = arr.includes(scope);
                      return (
                        <label key={scope} className="flex items-center gap-2 cursor-pointer select-none">
                          <input 
                            ...
                            checked={isChecked}
                            onChange={(e) => {
                              const nextArr = e.target.checked ? [...arr, scope] : arr.filter((v: string) => v !== scope);
                              setFormState((prev: any) => ({ ...prev, shipping_scope: nextArr }));
                            }}
                            className="accent-gold-pure"
                          />
                          <span className="text-[9px] text-zinc-300 font-mono">{scope}</span>
                        </label>
                      );
                    })}
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {['Al Hofuf', 'Al Ahsa', 'Dammam', 'Riyadh', 'Jeddah', 'Other'].map(zone => {
                      ...
                      return (
                        <label key={zone} className="flex items-center gap-2 cursor-pointer select-none">
                          <input 
                            type="checkbox"
                            checked={isChecked}
                            onChange={(e) => {
                              const nextArr = e.target.checked ? [...arr, zone] : arr.filter((v: string) => v !== zone);
                              setFormState((prev: any) => ({ ...prev, delivery_zones: nextArr }));
                            }}
                            className="accent-gold-pure"
                          />
                          <span className="text-[9px] text-zinc-300 font-mono">{zone}</span>
                        </label>
                      );
                    })}
                  </div>
            ...
            {/* SECTION C: POLICY CONTROLS */}
            <div className="p-3 bg-white/5 border border-white/5 rounded-xs space-y-4">
              <span className="text-[9px] font-mono text-gold-pure uppercase tracking-widest block font-bold">
                💳 Pricing, Packaging & Thermal Controls
              </span>
                ...
                <div className="space-y-2 pt-2">
                  <div className="flex items-center gap-2">
                    <input 
                      ...
                      checked={!!formState.free_shipping}
                      onChange={(e) => setFormState(prev => ({ ...prev, free_shipping: e.target.checked }))}
                      className="accent-gold-pure cursor-pointer"
                    />
                    <label htmlFor="shippingFreeShipping" className="text-[9px] text-zinc-300 font-mono cursor-pointer select-none font-bold">Free Shipping Available</label>
                  </div>
                </div>

                <div className="space-y-1">
                  <label className="text-[9px] text-zinc-400 uppercase tracking-wider block">Minimum Order for Free Shipping (SAR)</label>
                  <input 
                    type="number" 
                    value={formState.free_shipping_minimum !== undefined && formState.free_shipping_minimum !== null ? formState.free_shipping_minimum : ''}
                    onChange={(e) => setFormState(prev => ({ ...prev, free_shipping_minimum: e.target.value === '' ? undefined : Number(e.target.value) }))}
                    disabled={!formState.free_shipping}
                    className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-mono disabled:opacity-30"
                  />
                </div>
                ...
                <div className="space-y-2 pt-2">
                  <div className="flex items-center gap-2">
                    <input 
                      type="checkbox"
                      id="shippingCOD"
                      checked={!!formState.cash_on_delivery}
                      onChange={(e) => setFormState(prev => ({ ...prev, cash_on_delivery: e.target.checked }))}
                      className="accent-gold-pure cursor-pointer"
                    />
                    <label htmlFor="shippingCOD" className="text-[9px] text-zinc-300 font-mono cursor-pointer select-none">Cash On Delivery (COD)</label>
                  </div>
                </div>

                <div className="space-y-1">
                  <label className="text-[9px] text-zinc-400 uppercase tracking-wider block">Packaging Grade</label>
                  <select
                    value={formState.packaging_type || ''}
                    onChange={(e) => setFormState(prev => ({ ...prev, packaging_type: e.target.value }))}
                    className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-sans cursor-pointer"
                  >
                    <option value="">Standard Packaging</option>
                    <option value="Standard">Standard</option>
                    <option value="Luxury">Luxury Gold Seal</option>
                    <option value="Gift Box">Premium Gift Box</option>
                    <option value="Premium Thermal">Premium Thermal Shield</option>
                    <option value="Coffee Sealed">N2 Coffee Sealed Bag</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-white/5 pt-3">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <input 
                      type="checkbox"
                      id="shippingTempControl"
                      checked={!!formState.temperature_control}
                      onChange={(e) => setFormState(prev => ({ ...prev, temperature_control: e.target.checked }))}
                      className="accent-gold-pure cursor-pointer"
                    />
                    <label htmlFor="shippingTempControl" className="text-[9px] text-zinc-300 font-mono cursor-pointer select-none font-bold">Cold Chain/Temperature Control Required</label>
                  </div>

                  <div className="space-y-1">
                    <label className="text-[9px] text-zinc-400 uppercase tracking-wider block font-mono">Special Handling Flags</label>
                    <div className="grid grid-cols-3 gap-2">
                      {['Fragile', 'Glass', 'Liquid', 'Food', 'Premium', 'Luxury', 'Cold Chain', 'Thermal Protection', 'Keep Dry'].map(flag => {
                        const arr = Array.isArray(formState.handling_flags) ? formState.handling_flags : [];
                        const isChecked = arr.includes(flag);
                        return (
                          <label key={flag} className="flex items-center gap-1.5 cursor-pointer select-none">
                            <input 
                              ...
                              checked={isChecked}
                              onChange={(e) => {
                                const nextArr = e.target.checked ? [...arr, flag] : arr.filter((v: string) => v !== flag);
                                setFormState((prev: any) => ({ ...prev, handling_flags: nextArr }));
                              }}
                              className="accent-gold-pure"
                            />
                            <span className="text-[8px] text-zinc-400 font-mono">{flag}</span>
                          </label>
                        );
                      })}
                    </div>
                ...
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="text-[9px] text-zinc-400 uppercase tracking-wider block">Special Logistics & Shipping Notes</label>
                    <textarea 
                      value={formState.shipping_note || ''}
                      onChange={(e) => setFormState(prev => ({ ...prev, shipping_note: e.target.value }))}
                      placeholder="Enter specific logistics/shipping instructions for administrative staff"
                      rows={2}
                      className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-sans text-[10px]"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[9px] text-zinc-400 uppercase tracking-wider block">Customer Facing Delivery Message</label>
                    <textarea 
                      value={formState.customer_delivery_message || ''}
                      onChange={(e) => setFormState(prev => ({ ...prev, customer_delivery_message: e.target.value }))}
                      placeholder="Orders before 5 PM are processed today."
                      rows={2}
                      className="w-full bg-black border border-white/5 p-1.5 rounded-xs text-white focus:border-gold-pure outline-none font-sans text-[10px]"
                    />
                  </div>
