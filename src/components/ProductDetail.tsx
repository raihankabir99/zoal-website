...
Please confirm availability at your nearest flagship boutique. Thank you.`;
  ...
  return (
    <div className="bg-black text-white min-h-screen pt-[72px] sm:pt-[84px] md:pt-[88px] lg:pt-[92px] pb-12 sm:pb-20 pb-[calc(3rem+env(safe-area-inset-bottom,0px))]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Navigation line back */}
        <button
          onClick={onBack}
          className="inline-flex items-center space-x-2 rtl:space-x-reverse text-xs uppercase tracking-widest text-zinc-500 hover:text-gold-pure transition-colors duration-300 mb-4 sm:mb-8 cursor-pointer"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-16 items-start">
          ...
          <div className="lg:col-span-6 space-y-3 sm:space-y-4">
            ...
            <div className="flex space-x-2.5 sm:space-x-3 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-1.5 sm:pb-2">
        ...
        <div className="mt-6 md:mt-20 border-t border-white/5 pt-5 md:pt-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 md:gap-6 max-w-5xl mx-auto">
            {productFeatures.map((feature, idx) => (
              <div key={idx} className="bg-[#050505] border border-white/5 p-3 md:p-6 rounded-xs text-left group hover:border-gold-pure/30 transition-colors space-y-2 md:space-y-5">
                <div className="flex items-center space-x-2.5 rtl:space-x-reverse mb-1 md:mb-3">
                  <div className="w-7 h-7 sm:w-10 sm:h-10 rounded-full bg-zinc-900 flex items-center justify-center shrink-0 group-hover:bg-gold-pure/10 transition-colors">
                    <Star className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gold-pure" />
                  </div>
                  <h4 className="text-white text-xs font-display uppercase tracking-wider line-clamp-1">{getLocalizedFeature(feature)}</h4>
                </div>
        ...
        <div className="mt-6 md:mt-20 border-t border-white/5 pt-5 md:pt-16">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-2 md:mb-12 gap-2 md:gap-6">
            ...
            {!showReviewForm && (
              ...
              >
                {localReviews.length === 0 ? 'Share Your Experience' : t('product_detail.write_a_review')}
              </button>
            )}
          </div>
            ...
            <div className={`lg:col-span-${showReviewForm ? '7' : '12'} space-y-2 md:space-y-4`}>
              {localReviews.length === 0 ? (
                <div className="p-12 border border-dashed border-white/5 bg-zinc-950/20 text-center rounded-sm">
                  ...
                  <p className="text-white text-sm font-display tracking-widest uppercase">No reviews have been shared yet.</p>
                  <p className="text-zinc-500 text-xs mt-2">Be the first to share your experience and help others make confident choices.</p>
                </div>
              ) : (
              ...
              {showReviewForm && (
                ...
                >
                  ...
                  <form onSubmit={handleReviewSubmit} className="space-y-2.5 md:space-y-6">
                    ...
                    <div className="space-y-1.5 md:space-y-3">
                      <label className="text-[10px] text-zinc-100 font-semibold uppercase tracking-widest font-display block">
                        {t('product_detail.add_photos', { defaultValue: 'Add Photos' })} (Max 5)
                      </label>
                      <div className="flex flex-wrap gap-2">
                        ...
                        {reviewPhotos.length < 5 && (
                          ...
                          >
                            ...
                            <span className="text-[8px] text-zinc-600 uppercase font-display">Add</span>
                          </button>
                        )}
                      </div>
                      <input
                    ...
                    <div className="space-y-1.5 md:space-y-3 pt-1 md:pt-2">
                      ...
                      <div className="flex gap-2 md:gap-6 space-x-0">
                        <label className="flex items-center space-x-2 cursor-pointer group">
                          ...
                          <span className={`text-xs uppercase tracking-widest font-display ${recommendProduct === 'yes' ? 'text-white' : 'text-zinc-500'}`}>Yes</span>
                        </label>
                        <label className="flex items-center space-x-2 cursor-pointer group">
                          <input
                            type="radio"
                          ...
                          <span className={`text-xs uppercase tracking-widest font-display ${recommendProduct === 'no' ? 'text-white' : 'text-zinc-500'}`}>No</span>
                        </label>
                      </div>
                    </div>
