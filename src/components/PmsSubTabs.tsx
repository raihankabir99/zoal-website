...
import { SafeImage, resolveProductImage } from '../imageRegistry';
...
export const PmsSubTabs: React.FC<PmsSubTabsProps> = ({
  ...
  useEffect(() => {
    ...
    // SEO sync
    const autoSlug = (selectedPmsProduct.name || 'product').toLowerCase().replace(/[^a-z0-9]+/g, '-');
    ...
    setSeoOpenGraphImage(selectedPmsProduct.seoOpenGraphImage || resolveProductImage(selectedPmsProduct) || '');
  ...
  const handleSaveVariant = (e: React.FormEvent) => {
    ...
    const newVariant: ProductVariant = {
      ...
      image: variantImage.trim() || resolveProductImage(selectedPmsProduct),
      [optName]: optVal
    };
  ...
  const handleCsvImportLocal = (text: string) => {
    try {
      ...
      addLog("Product Created", `Mass imported ${count} new products from CSV list`);
      alert(`List synchronizer: Successfully imported ${count} new custom products from spreadsheet.`);
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs text-xs space-y-4">
              <div className="flex items-center justify-between border-b border-white/5 pb-3">
                ...
                <button
                  onClick={() => {
                    const imgResolved = resolveProductImage(selectedPmsProduct);
                    const samples: ProductVariant[] = [
                      { id: 'v-1', sku: `${selectedPmsProduct.sku || 'ZL'}-SZ-M`, barcode: `628${Date.now()}1`, price: selectedPmsProduct.price, stock: 15, status: 'Active', size: 'Medium', image: imgResolved },
                      { id: 'v-2', sku: `${selectedPmsProduct.sku || 'ZL'}-SZ-L`, barcode: `628${Date.now()}2`, price: selectedPmsProduct.price + 25, stock: 8, status: 'Active', size: 'Large', image: imgResolved },
                      { id: 'v-3', sku: `${selectedPmsProduct.sku || 'ZL'}-SZ-XL`, barcode: `628${Date.now()}3`, price: selectedPmsProduct.price + 45, stock: 3, status: 'Active', size: 'X-Large', image: imgResolved }
                    ];
                    saveProductFields(selectedPmsProduct.id, { variantsList: samples });
                    addLog("Product Updated", `Auto-seeded variants for ${selectedPmsProduct.name}`);
                  }}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-1 space-y-2">
                  ...
                  <div className="aspect-square bg-black border border-white/10 rounded-xs overflow-hidden relative group">
                    <SafeImage product={selectedPmsProduct} className="w-full h-full object-cover" alt="Featured" containerClassName="w-full h-full relative overflow-hidden" />
                    <span className="absolute bottom-2 left-2 bg-gold-pure text-black font-mono text-[8px] font-bold px-1.5 py-0.5 rounded-xs uppercase z-10">FEATURED</span>
                  </div>
                </div>
        ...
        <div className="space-y-4 animate-fade-in text-left">
          {currentUser?.role === 'staff' && (
            <div className="p-3 bg-rose-950/20 border border-rose-900/40 rounded-xs text-rose-400 font-mono text-[9px] uppercase tracking-wider">
              🔒 Security Enforcement: Staff accounts cannot modify Google SERP Metadata or invoke Gemini system AI updates. This screen is in read-only mode.
            </div>
          )}
          <div className="space-y-6">
            <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs">
              <ProductSeoSuite
                product={selectedPmsProduct}
            ...
            <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4 relative overflow-hidden">
                {isGeneratingAi && (
                  <div className="absolute inset-0 bg-black/80 z-20 flex flex-col items-center justify-center space-y-2 font-mono text-xs">
                    ...
                    <span className="text-white uppercase tracking-widest text-[9px] animate-pulse">Consulting Gemini AI...</span>
                  </div>
                )}
              ...
              <button
                onClick={() => {
                  const initialReviews: Review[] = [
                    ...
                    { id: 'rev-2', reviewerName: 'Khalid Al-Dossari', rating: 4, date: new Date(Date.now() - 86400000).toLocaleDateString(), comment: 'Superior quality, very aromatic. The shipping took some time to Hofuf, but worth the wait.' }
                  ];
                  saveProductFields(selectedPmsProduct.id, { reviews: initialReviews });
                  addLog("Product Updated", `Seeded reviews sandbox database on ${selectedPmsProduct.name}`);
                }}
        ...
        <div className="space-y-6 animate-fade-in text-left">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 text-xs">
            <div className="lg:col-span-6 bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4">
              <div>
                <span className="text-[8px] font-mono text-gold-pure uppercase tracking-widest block mb-1">MASS PRICE ACTIONS</span>
            ...
            <div className="lg:col-span-6 bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-6">
                <div>
                  <label className="text-zinc-500 block mb-1 font-mono text-[8px] uppercase">Premium Brand Curator</label>
                  <select
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 text-xs font-sans">
            {/* Bulk Images Assigner */}
            <div className="lg:col-span-6 bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4">
              <div>
                <span className="text-[8px] font-mono text-gold-pure uppercase tracking-widest block mb-1">IMAGE ASSETS CURATOR</span>
                <h3 className="text-base font-bold font-serif text-white">Bulk Media & Image Assigner</h3>
                <p className="text-[10px] text-zinc-500 font-mono">Assign image assets to all {selectedProductIds.length} selected products simultaneously.</p>
              </div>
              <div className="space-y-3 bg-black/40 p-4 border border-white/5 rounded-xs">
                <div className="space-y-1">
                  <label className="text-[8.5px] text-zinc-400 font-mono block">Image URLs (comma-separated or single)</label>
                  <input 
                    type="text" 
                    id="bulk_image_urls_input"
                    placeholder="https://images.unsplash.com/photo-1..., https://images.unsplash.com/photo-2..."
                    className="w-full bg-black border border-white/10 p-2 rounded-xs text-white font-mono text-[9px]"
                  />
                </div>
                <button
                  type="button"
                  onClick={() => {
                    const el = document.getElementById('bulk_image_urls_input') as HTMLInputElement;
                    if (!el || !el.value.trim()) { alert("Please input at least one image URL."); return; }
                    if (selectedProductIds.length === 0) { alert("Please select products from the catalog tab first."); return; }
                    const urls = el.value.split(',').map(url => url.trim()).filter(Boolean);
                    selectedProductIds.forEach(id => {
                      saveProductFields(id, { images: urls });
                    });
                    addLog("Bulk Image Applied", `Bulk assigned ${urls.length} images on ${selectedProductIds.length} products`);
                    alert(`Successfully assigned images to ${selectedProductIds.length} products.`);
                    el.value = '';
                  }}
                  className="w-full py-2 bg-zinc-800 hover:bg-zinc-700 text-white font-mono font-bold uppercase tracking-wider rounded-xs cursor-pointer border border-white/10 text-[9px]"
                >
                  Apply Images to Selections
                </button>
              </div>
            </div>

            {/* Bulk AI Content Copilot Generation */}
            <div className="lg:col-span-6 bg-zinc-950 border border-white/5 p-5 rounded-xs space-y-4">
              <div>
                <span className="text-[8px] font-mono text-gold-pure uppercase tracking-widest block mb-1">AUTOMATED GEMINI ENGINE</span>
                <h3 className="text-base font-bold font-serif text-white">Bulk AI Copilot Content Generator</h3>
                <p className="text-[10px] text-zinc-500 font-mono">Trigger high-fidelity Gemini generation sequentially for all {selectedProductIds.length} selected products.</p>
              </div>
              <div className="space-y-3 bg-black/40 p-4 border border-white/5 rounded-xs">
                <button
                  type="button"
                  disabled={isGeneratingAi || selectedProductIds.length === 0}
                  onClick={async () => {
                    if (selectedProductIds.length === 0) {
                      alert("Please select products from the catalog tab first.");
                      return;
                    }
                    if (!confirm(`Are you sure you want to run Bulk AI Generation on ${selectedProductIds.length} selected products sequentially?`)) {
                      return;
                    }
                    setIsGeneratingAi(true);
                    let successCount = 0;
                    for (const id of selectedProductIds) {
                      ...
                      try {
                        ...
                        console.error(`Bulk AI error on product ${prod.name}:`, err);
                      }
                    }
                    addLog("Bulk AI Generated", `Sequentially executed automated content generator on ${successCount}/${selectedProductIds.length} products`);
                    alert(`Bulk AI copywriting generation complete! Successfully generated intelligence for ${successCount} products.`);
                    setIsGeneratingAi(false);
                  }}
                  className="w-full py-2 bg-gold-pure hover:bg-gold-pure/90 disabled:opacity-50 text-black font-mono font-bold uppercase tracking-wider rounded-xs cursor-pointer text-[9px] flex items-center justify-center gap-1.5"
                >
                  <Sparkles className={`w-3.5 h-3.5 ${isGeneratingAi ? 'animate-spin' : ''}`} />
                  {isGeneratingAi ? 'Running Bulk AI...' : 'Run Sequenced Bulk AI Generation'}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="border border-white/5 bg-black/40 p-4 rounded-xs text-xs space-y-3">
                <h4 className="text-white font-bold font-mono text-[10px] uppercase tracking-wider flex items-center gap-1.5">
                  📥 Import CSV / Microsoft Excel Spreadsheet
                </h4>
              ...
              <div className="border border-white/5 bg-black/40 p-4 rounded-xs text-xs space-y-3 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 font-mono">Download the entire active boutique product catalog, formatted in a clean standard CSV spreadsheet fully compatible with Microsoft Excel, Apple Numbers, and Google Sheets.</p>
                </div>
        ...
        <div className="bg-zinc-950 border border-white/5 p-5 rounded-xs text-xs space-y-4 animate-fade-in text-left">
          <div className="border-b border-white/5 pb-3 flex justify-between items-center">
            <h3 className="text-white font-serif font-bold flex items-center gap-2">
              <Activity className="w-4 h-4 text-gold-pure" /> Product Management Audit log
            </h3>
            ...
            <table className="w-full text-left border-collapse font-mono text-[10px]">
              ...
              <tbody className="divide-y divide-white/5 text-zinc-300">
                {pmsLogs.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-zinc-500 uppercase tracking-widest text-[9px]">
                      No product transactions logged yet.
                    </td>
                  </tr>
                ) : (
