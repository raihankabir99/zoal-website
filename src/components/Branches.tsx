...
export default function Branches() {
  ...
  return (
    <div dir={isAr ? 'rtl' : 'ltr'} className="bg-black text-white min-h-screen pt-[80px] sm:pt-[84px] md:pt-[88px] lg:pt-[92px] pb-20">
          ...
          <div className="lg:col-span-5 space-y-4">
            {BRANCHES.map((branch) => (
              <button
                ...
                className={`w-full text-start p-6 border rounded-sm transition-all duration-300 flex flex-col justify-between cursor-pointer ${
              ...
              >
                <div>
                  ...
                  <p className="text-zinc-500 text-[10.5px] mt-1 flex items-center gap-1.5">
                    <MapPin className="w-3 h-3 text-gold-pure shrink-0" /> <span>{i18n.language === 'ar' ? t(`branches_data.${branch.id}.address`, { defaultValue: branch.address }) : branch.address}</span>
                  </p>
                </div>

                <div className="border-t border-white/5 pt-4 mt-4 space-y-2 text-xs font-sans">
                  <p className="text-zinc-400 flex items-center gap-2">
                    <Phone className="w-3.5 h-3.5 text-gold-pure/60 shrink-0" /> <span style={{ direction: 'ltr' }}>{branch.phone}</span>
                  </p>
                  <p className="text-zinc-400 flex items-center gap-2">
                    <Clock className="w-3.5 h-3.5 text-gold-pure/60 shrink-0" /> <span>{i18n.language === 'ar' ? t(`branches_data.${branch.id}.hours`, { defaultValue: branch.hours }) : branch.hours}</span>
                  </p>
                </div>
            ...
            <div className="relative aspect-video rounded-sm overflow-hidden border border-white/10 bg-[#0c0c0c] flex items-center justify-center p-4">
              ...
              <div className="relative z-10 text-center space-y-3">
                ...
                <p className="text-zinc-500 text-[10.5px] max-w-sm font-sans mx-auto">
                  {t('branches.gps_coordinates', { defaultValue: 'GPS Coordinates' })}<br />
                  {t('branches.latitude', { defaultValue: 'Latitude:' })} {selectedBranch.coordinates.lat}<br />
                  {t('branches.longitude', { defaultValue: 'Longitude:' })} {selectedBranch.coordinates.lng}<br />
              ...
              <div className="absolute bottom-3 start-3 text-[8px] font-mono text-zinc-500 tracking-widest uppercase">
                {t('branches.verified_location', { defaultValue: 'Verified Location' })}
              </div>
