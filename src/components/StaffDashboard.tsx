...
export default function StaffDashboard({
  ...
  // Unified Enterprise Notification Engine
  ...
  const staffNotifications = activeNotificationEngine.notifications;
  const unreadStaffNotifCount = activeNotificationEngine.unreadCount;
  ...
  return (
    <div className="space-y-1 lg:space-y-6 text-left animate-fade-in">
      ...
      <div className="flex md:hidden flex-col mt-0 pt-0 mb-1 space-y-0">
        {/* Row 2: Dashboard Switch (My Dashboard | Staff Dashboard | Owner/Admin Dashboard) */}
        {(userRole === 'admin' || userRole === 'staff' || userRole === 'owner') && (
          <div className="w-full flex justify-center">
            <div className="flex bg-black border border-white/10 rounded-xs p-0.5 w-full justify-center rtl:flex-row-reverse h-8 items-center">
              {activeDashboardTab !== 'customer' && setActiveDashboardTab && (
                <button
                  onClick={() => setActiveDashboardTab('customer')}
                  className="py-1 px-1 text-[8px] sm:text-[9px] tracking-tight font-display uppercase text-zinc-400 hover:text-white flex items-center justify-center gap-0.5 flex-1 cursor-pointer transition-all whitespace-nowrap h-7"
                >
                  <User className="w-2.5 h-2.5" /> MY DASHBOARD
                </button>
              )}
              {(userRole === 'admin' || userRole === 'staff') && setActiveDashboardTab && (
                <button
                  onClick={() => setActiveDashboardTab('admin')}
                  className="bg-gold-pure text-black font-semibold py-1 px-1 text-[8px] sm:text-[9px] tracking-tight font-display uppercase rounded-xs flex items-center justify-center gap-0.5 flex-1 cursor-pointer transition-all whitespace-nowrap h-7"
                >
                  <Shield className="w-2.5 h-2.5" /> STAFF DASHBOARD
                </button>
              )}
              {(userRole === 'admin' || userRole === 'owner') && setCurrentPage && (
                <button
                  onClick={() => setCurrentPage('admin')}
                  className="text-gold-pure hover:bg-gold-pure/10 border border-gold-pure/30 py-1 px-1 text-[8px] sm:text-[9px] tracking-tight font-display uppercase rounded-xs flex items-center justify-center gap-0.5 flex-1 cursor-pointer transition-all whitespace-nowrap h-7"
                >
                  <Shield className="w-2.5 h-2.5 text-gold-pure" /> OWNER/ADMIN
                </button>
              )}
            </div>
          </div>
      ...
      {/* ================= TABLET HEADER (md >= 768px and lg < 1024px) ================= */}
      <div className="hidden md:flex lg:hidden flex-col bg-zinc-950 border border-white/5 rounded-sm p-4 space-y-4 mb-4">
        {/* Row 1: Title Only */}
        <div className="text-center border-b border-white/5 pb-3">
          <h1 className="text-xl font-bold tracking-wider font-display uppercase text-white">
            Staff Dashboard
          </h1>
        </div>
        ...
        {(userRole === 'admin' || userRole === 'staff' || userRole === 'owner') && (
          <div className="w-full flex justify-center">
            <div className="flex bg-black border border-white/5 rounded-sm p-1 w-full max-w-md justify-center rtl:flex-row-reverse">
              {activeDashboardTab !== 'customer' && setActiveDashboardTab && (
                <button
                  onClick={() => setActiveDashboardTab('customer')}
                  className="py-1.5 px-3 text-xs font-display uppercase text-zinc-500 hover:text-white flex items-center justify-center gap-1 flex-1 cursor-pointer transition-all"
                >
              ...
              {(userRole === 'admin' || userRole === 'staff') && setActiveDashboardTab && (
                <button
                  onClick={() => setActiveDashboardTab('admin')}
                  className="bg-gold-pure text-black font-semibold py-1.5 px-3 text-xs font-display uppercase rounded-xs flex items-center justify-center gap-1 flex-1 cursor-pointer transition-all"
                >
              ...
              {(userRole === 'admin' || userRole === 'owner') && setCurrentPage && (
                <button
                  onClick={() => setCurrentPage('admin')}
                  className="text-gold-pure hover:bg-gold-pure/10 border border-gold-pure/30 py-1.5 px-3 text-xs font-display uppercase rounded-xs flex items-center justify-center gap-1 flex-1 cursor-pointer transition-all"
                >
                  <Shield className="w-3.5 h-3.5 text-gold-pure" /> ADMIN
                </button>
              )}
            </div>
          </div>
        ...
        {/* Row 3: Navigation & Status Bar on One Row */}
        <div className="flex flex-row items-center justify-between gap-3 w-full pt-1 border-t border-white/5">
          {/* Breadcrumb / Back / Menu */}
          <div className="flex items-center gap-2">
            <button 
              onClick={() => { setStaffSidebarOpen(!staffSidebarOpen); }}
              className="p-1 text-zinc-400 hover:text-[#D4AF37] transition-colors focus:outline-none shrink-0"
            ...
            <button
              onClick={() => {
                if (selectedStaffOrder) {
                  setSelectedStaffOrder(null);
                } else if (staffSubTab !== 'overview') {
                  setStaffSubTab('overview');
                } else {
                  setCurrentPage?.('home');
                }
              }}
              className="flex items-center gap-1 px-2.5 py-1 bg-white/5 hover:bg-gold-pure/20 border border-white/10 text-zinc-400 hover:text-white rounded-xs transition-all shrink-0 cursor-pointer"
            >
              <ArrowLeft className="w-3 h-3" />
              <span className="text-[9px] uppercase tracking-widest font-bold">Back</span>
            </button>
            <span className="text-[9px] uppercase tracking-widest text-[#D4AF37] font-mono font-semibold truncate">
              {staffSubTab === 'overview' ? 'Overview' : staffSubTab}
            </span>
          </div>

          {/* Right Controls: Duty, Language, Notification, Profile */}
          <div className="flex items-center gap-2 shrink-0">
            <div className="flex items-center gap-1 px-2 py-1 rounded-sm border border-white/5 bg-black text-[9px] uppercase font-mono">
              <span className={`w-1.5 h-1.5 rounded-full ${
                staffDutyStatus === 'active' ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'
              }`} />
              <span className="text-zinc-400">Duty:</span>
              <span className={staffDutyStatus === 'active' ? 'text-emerald-400 font-bold' : 'text-amber-400 font-bold'}>
                {staffDutyStatus.toUpperCase()}
              </span>
            </div>
            <DashboardLanguageSwitcher />
            <button
              onClick={() => { setStaffSubTab('alerts'); setSelectedStaffOrder(null); }}
              className="relative p-1.5 bg-black hover:bg-zinc-900 border border-white/5 text-zinc-400 hover:text-white rounded-xs cursor-pointer"
            >
              <Bell className="w-3.5 h-3.5" />
              {staffNotifications.filter(n => !n.read).length > 0 && (
                <span className="absolute -top-1 -right-1 w-3 h-3 bg-[#D4AF37] text-black font-mono font-bold text-[8px] flex items-center justify-center rounded-full">
                  {staffNotifications.filter(n => !n.read).length}
                </span>
              )}
            </button>
            <div className="flex items-center gap-1 px-2 py-1 bg-black border border-white/5 rounded-xs">
              <span className="text-[9px] font-semibold text-zinc-300 uppercase truncate max-w-[80px]">
      ...
      {/* Top Navigation & Status bar (Mobile & Desktop) */}
      <div className="md:hidden lg:flex bg-zinc-950 border border-white/5 rounded-sm p-1.5 lg:p-4 flex-row items-center justify-between gap-1.5 lg:gap-4 md:-mt-10 lg:-mt-10 md:mb-[1px] lg:mb-[1px]">
        {/* Breadcrumb Navigation */}
        <div className="flex items-center gap-1 lg:gap-2">
          <button 
            onClick={() => { setStaffSidebarOpen(!staffSidebarOpen); }}
            className="lg:hidden p-1 -ml-1 text-zinc-400 hover:text-[#D4AF37] transition-colors focus:outline-none shrink-0"
          ...
          <div className="flex items-center gap-1 lg:gap-2">
            <button
              ...
              className="flex items-center gap-1 px-1.5 py-1 lg:px-3 lg:py-1.5 bg-white/5 hover:bg-gold-pure/20 border border-white/10 hover:border-gold-pure/30 text-zinc-400 hover:text-white rounded-xs transition-all duration-300 group cursor-pointer shrink-0"
              aria-label="Go Back"
            >
              <ArrowLeft className="w-3 h-3 group-hover:-translate-x-0.5 transition-transform" />
              <span className="text-[8px] lg:text-[9px] uppercase tracking-wide lg:tracking-widest font-bold">Back</span>
            </button>
            <div className="flex items-center gap-1 text-[7.5px] lg:text-[9px] uppercase tracking-wide lg:tracking-[0.25em] text-zinc-500 font-mono shrink-0">
        ...
        <div className="flex items-center justify-end gap-1.5 lg:gap-3.5 font-sans shrink-0">
          {/* Duty Status Badge */}
          <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-sm border border-white/5 bg-black text-[7.5px] lg:text-[9px] uppercase tracking-wide lg:tracking-wider font-mono shrink-0">
            ...
            <span className="text-zinc-400 hidden sm:inline">Duty:</span>
            <span className={
              ...
              <span className="inline sm:hidden">
                {staffDutyStatus === 'active' ? 'ACTIVE' :
                 staffDutyStatus === 'break' ? 'BREAK' : 'OFFLINE'}
              </span>
            </span>
          </div>
          ...
          <button
            onClick={() => { setStaffSubTab('alerts'); setSelectedStaffOrder(null); }}
            className="relative p-1.5 lg:p-2 bg-black hover:bg-zinc-900 border border-white/5 hover:border-[#D4AF37]/30 text-zinc-400 hover:text-white rounded-xs transition-all cursor-pointer group shrink-0"
            title="System Alerts Feed"
          >
            <Bell className="w-3 h-3 lg:w-3.5 lg:h-3.5 group-hover:animate-swing" />
            {staffNotifications.filter(n => !n.read).length > 0 && (
              <span className="absolute -top-1 -right-1 w-2.5 lg:w-3.5 h-2.5 lg:h-3.5 bg-[#D4AF37] text-black font-mono font-bold text-[7px] lg:text-[8.5px] flex items-center justify-center rounded-full animate-bounce">
                {staffNotifications.filter(n => !n.read).length}
              </span>
            )}
          </button>
          ...
          <div className="relative group/staff select-none shrink-0">
            <div className="flex items-center gap-1 lg:gap-2 p-0.5 px-1.5 lg:p-1 lg:px-2.5 bg-black hover:bg-zinc-900 border border-white/5 rounded-xs cursor-pointer duration-300">
              <div className="w-4 h-4 lg:w-5 lg:h-5 rounded-full border border-[#D4AF37] bg-zinc-900 flex items-center justify-center text-[8px] lg:text-[10px] text-[#D4AF37] shrink-0">
                💼
              </div>
              <span className="text-[8.5px] lg:text-[9.5px] font-semibold text-zinc-300 uppercase tracking-wider truncate max-w-[90px] hidden sm:block">
                {currentUser ? (currentUser.name || 'Staff').split(' ')[0] : 'Staff'}
              </span>
            </div>
      ...
      <div className="flex flex-col lg:flex-row gap-1.5 lg:gap-6 items-start relative md:mt-[1px] lg:mt-[1px]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-zinc-950 border border-white/5 p-4 rounded-sm relative overflow-hidden flex flex-col justify-between h-28">
                  ...
                  <div>
                    ...
                    <span className="text-2xl font-sans text-white font-bold block leading-none tabular-nums-fix">
                ...
                <div className="bg-zinc-950 border border-white/5 p-4 rounded-sm relative overflow-hidden flex flex-col justify-between h-28">
                  ...
                  <div>
                    ...
                    <span className={`text-2xl font-sans font-bold block leading-none tabular-nums-fix ${
                ...
                <div className="bg-zinc-950 border border-white/5 p-4 rounded-sm relative overflow-hidden flex flex-col justify-between h-28">
                  ...
                  <div>
                    ...
                    <span className="text-2xl font-sans text-white font-bold block leading-none tabular-nums-fix">3 Active</span>
                  </div>
                      ...
                      {orders
                        ...
                        .map((ord) => {
                          ...
                          return (
                            ...
                            >
                              <td className="py-4 pl-2 font-sans font-bold text-[#D4AF37] tabular-nums-fix">{ord.id}</td>
                              <td className="py-4 text-left">
                                <p className="font-semibold text-zinc-200">{ord.customerName}</p>
                                <p className="text-[10px] text-zinc-500 tabular-nums-fix">{ord.phone}</p>
                              </td>
                              ...
                              <td className="py-4 text-left font-sans font-bold text-zinc-300 tabular-nums-fix">{formatCurrency(ord.total)} SAR</td>
                              <td className="py-4 text-left">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="p-4 border border-white/5 bg-zinc-950/30 rounded-xs space-y-2">
                          ...
                          <p className="text-zinc-300 font-medium leading-normal">{(selectedStaffOrder as any).city || 'Branch B Main'}</p>
                          <p className="text-zinc-400 leading-relaxed font-sans">{selectedStaffOrder.address || 'Saudi Arabia VIP Delivery Address'}</p>
                          <p className="text-[10px] text-zinc-500 font-mono">Method: {(selectedStaffOrder as any).method || 'Standard Courier'}</p>
                        </div>
                      </div>
                        ...
                        <div className="p-4 border border-white/5 bg-zinc-950/30 rounded-xs space-y-3">
                          ...
                          <div className="divide-y divide-white/5 space-y-2 max-h-[160px] overflow-y-auto pr-1">
                            {selectedStaffOrder.items && selectedStaffOrder.items.length > 0 ? (
                              selectedStaffOrder.items.map((itm: any, idx) => (
                                <div key={idx} className="pt-2 flex items-center justify-between">
                                  <div className="text-left">
                                    <p className="text-zinc-200 font-medium">{itm.product?.name || itm.name}</p>
                                    <p className="text-zinc-500 text-[10px] font-sans tabular-nums-fix">Qty: {itm.quantity} • {formatCurrency(itm.product?.price || itm.price || 0)} SAR</p>
                                  </div>
                                  <span className="font-mono text-[#D4AF37] font-semibold text-xs tabular-nums-fix">
                                    {formatCurrency((itm.product?.price || itm.price || 0) * itm.quantity)} SAR
                                  </span>
                                </div>
                              ))
                            ) : (
                          ...
                          <div className="border-t border-white/5 pt-2 flex items-center justify-between text-xs font-bold uppercase tracking-wider">
                            ...
                            <span className="text-white font-sans text-base font-bold tabular-nums-fix">{formatCurrency(selectedStaffOrder.total)} SAR</span>
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      {uniqueCustomers
                        ...
                        .map((c, i) => (
                          <tr key={i} className="hover:bg-white/[0.01]">
                            ...
                            <td className="py-4 font-mono text-[11px] text-zinc-400 text-left tabular-nums-fix">{c.phone}</td>
                            <td className="py-4 text-left font-sans tabular-nums-fix">{c.totalOrders} VIP Orders</td>
                            <td className="py-4 text-left font-sans font-bold text-[#D4AF37] tabular-nums-fix">{formatCurrency(c.totalSpent)} SAR</td>
                ...
                <div className="divide-y divide-white/5 space-y-3.5">
                  {staffNotifications.length === 0 ? (
                    <div className="py-8 text-center text-zinc-500 font-mono text-xs">
                      No notifications for your staff account.
                    </div>
                  ) : (
                    staffNotifications.map((notif) => (
                      <div 
                        key={notif.id} 
                        onClick={() => {
                          activeNotificationEngine.markAsRead(notif.id);
                        }}
                    ...
                    <tbody className="divide-y divide-white/5">
                      {orders.map((ord) => (
                        <tr key={ord.id} className="hover:bg-white/[0.02]">
                          <td className="py-4 pl-2 font-mono font-bold text-gold-pure text-left tabular-nums-fix">{ord.id}</td>
                          <td className="py-4 font-sans text-zinc-300 text-left">
                            <p className="font-semibold">{ord.customerName}</p>
                            <p className="text-[10px] text-zinc-500 tabular-nums-fix">{ord.phone}</p>
                          </td>
                          <td className="py-4 font-sans text-zinc-300 text-left tabular-nums-fix">{formatCurrency(ord.total)} SAR</td>
                          <td className="py-4 text-left">
                            <span className={`px-2 py-0.5 rounded-sm text-[8px] uppercase tracking-widest font-semibold ${
                              ord.status === 'Completed' ? 'bg-emerald-950 text-emerald-400' :
                              ord.status === 'Shipped' ? 'bg-blue-950 text-blue-400' :
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                  ...
                  <div className="space-y-4">
                    <div className="p-4 border border-white/5 bg-zinc-950/20 rounded-xs space-y-3">
                      ...
                      <textarea
                        rows={4}
                        value={returnsConfig.nonReturnableEn.join(String.fromCharCode(10))}
                        onChange={(e) => setLocalReturnsConfig({ ...returnsConfig, nonReturnableEn: e.target.value.split(String.fromCharCode(10)) })}
                        className="w-full bg-black border border-white/10 rounded-xs p-2.5 text-xs text-white focus:outline-none font-sans"
                      />
                    </div>
...
')}
                        onChange={(e) => setLocalReturnsConfig({ ...returnsConfig, nonReturnableAr: e.target.value.split(String.fromCharCode(10)) })}
...
')}
                          onChange={(e) => setLocalReturnsConfig({ ...returnsConfig, exchangeOptionsEn: e.target.value.split(String.fromCharCode(10)) })}
                          className="w-full bg-black border border-white/10 rounded-xs p-2 text-xs text-white focus:outline-none font-sans"
...
')}
                          onChange={(e) => setLocalReturnsConfig({ ...returnsConfig, exchangeOptionsAr: e.target.value.split(String.fromCharCode(10)) })}
