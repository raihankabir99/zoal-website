      ...
      <div className="max-w-4xl mx-auto">
        ...
        {wishlistProducts.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-white/5 bg-zinc-950/25 rounded-sm">
            <h2 className="text-white text-md font-display uppercase tracking-widest font-semibold">{t('wishlist.empty', { defaultValue: 'Your Wishlist is Empty' })}</h2>
            <p className="text-zinc-500 text-xs mt-2">{t('wishlist.empty_desc', { defaultValue: 'Save items you love here to easily find or purchase them later.' })}</p>
            <button
            ...
            {wishlistProducts.map((p) => (
              <div key={p.id} className="p-4 border border-white/5 bg-zinc-900/40 rounded-sm flex items-center justify-between gap-4">
                <div className="flex items-center space-x-4 rtl:space-x-reverse cursor-pointer" onClick={() => {
                  ...
                  <SafeImage product={p} alt={p.category === 'thobes' ? "ZOAL THOBES & MEN'S WEAR" : (i18n.language === 'ar' ? t(`products.${p.id}.name`, { defaultValue: p.name }) : p.name)} className="w-16 h-16 rounded-xs object-cover" containerClassName="w-16 h-16 relative rounded-xs overflow-hidden" />
                  <div>
                    ...
                    <span className="text-gold-pure text-[10px] font-sans mt-1 block tabular-nums-fix">{formatCurrency(p.price)} {t('app.sar')}</span>
                  </div>
                </div>
