...
app.get('/api/products', async (req, res) => {
  ...
  try {
    ...
    let products = (rows || []).map(row => row.data);
    ...
    if (products.length === 0 && !search && !category && offset === 0 && process.env.NODE_ENV !== 'production') {
      ...
      if (reSelectRows) {
        products = reSelectRows.map(row => row.data);
      }
    }
...
app.post('/api/products', authenticateRequest, requireRole(['admin', 'staff']), async (req, res) => {
  ...
  try {
    ...
    if (error) {
      console.error('❌ Error in POST /api/products:', error.message || error);
...
app.put('/api/products/:id', authenticateRequest, requireRole(['admin', 'staff']), async (req, res) => {
  const supabase = getServiceSupabaseClient() || getSupabaseClient();
  if (!supabase) return res.status(500).json({ error: 'Database is not configured.' });

  const id = req.params.id;
  const prod = req.body || {};
  const productId = prod.id || id;
  const uuid = friendlyToUUID(productId);
  const fullProd = { ...prod, id: productId };
  ...
  try {
    const { error } = await supabase
      ...
      .upsert({
        ...
        name: fullProd.name || 'Product',
        category: fullProd.category || 'Coffee',
        price: Number(fullProd.price) || 0,
        is_active: fullProd.status !== 'Hidden' && fullProd.status !== 'Archived',
        data: fullProd,
        updated_at: new Date().toISOString()
      }, { onConflict: 'id' });
...
app.patch('/api/products', authenticateRequest, requireRole(['admin', 'staff']), async (req, res) => {
  ...
  try {
    ...
    const mergedData = { ...(existing?.data || {}), ...prod };
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        price: Number(mergedData.price) || 0,
        is_active: mergedData.status !== 'Hidden' && mergedData.status !== 'Archived',
        data: mergedData,
        updated_at: new Date().toISOString()
      })
      .eq('id', uuid);
...
app.patch('/api/products/:id', authenticateRequest, requireRole(['admin', 'staff']), async (req, res) => {
  const supabase = getServiceSupabaseClient() || getSupabaseClient();
  if (!supabase) return res.status(500).json({ error: 'Database is not configured.' });
  ...
  try {
    ...
    const mergedData = { ...(existing?.data || {}), ...prod, id };
