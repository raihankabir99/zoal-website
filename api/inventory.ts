...
function supabaseUrl() {
  return (process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.SUPABASE_URL || DEFAULT_URL).replace(/\/$/, '');
}
function serviceKey() {
...
async function authenticate(req: any, adminClient: any) {
  ...
  const response = await fetch(`${supabaseUrl()}/auth/v1/user`, {
    headers: {
      apikey: process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY || DEFAULT_ANON_KEY,
...
function cleanNonNegativeInteger(value: any, field: string) {
...
async function resolveWarehouseId(adminClient: any, requested: any) {
  if (requested) {
    ...
    const byId = await adminClient.from('zoal_warehouses').select('id').eq('id', value).maybeSingle();
    if (byId.data?.id) return byId.data.id;
    const byCode = await adminClient.from('zoal_warehouses').select('id').eq('warehouse_code', value).maybeSingle();
    if (byCode.data?.id) return byCode.data.id;
    const byName = await adminClient.from('zoal_warehouses').select('id').ilike('warehouse_name', value).maybeSingle();
  ...
  if (error) throw error;
  if (!data?.id) throw Object.assign(new Error('No active warehouse is configured.'), { status: 409 });
  return data.id;
}
...
export default async function handler(req: any, res: any) {
  ...
  if (!key) return res.status(503).json({ error: 'Inventory database is unavailable.' });
  ...
  try {
    if (req.method === 'GET') {
      const { data, error } = await adminClient
        .from('zoal_inventory')
        .select('id,product_id,quantity,reserved_quantity,min_stock,max_stock,low_stock_threshold,warehouse_id,warehouse_location,updated_at')
    ...
    if (!current) return res.status(404).json({ error: 'Inventory row not found for product and warehouse.' });

    const before = { ...current };
    const operation = body.operation || 'set';
    let nextQuantity = Number(current.quantity);

    if (operation === 'adjust') {
      const delta = Number(body.quantityChange ?? body.delta);
      if (!Number.isInteger(delta)) return res.status(400).json({ error: 'quantityChange must be an integer.' });
      nextQuantity += delta;
    } else if (operation === 'set') {
      nextQuantity = cleanNonNegativeInteger(body.quantity, 'quantity');
    } else {
      return res.status(400).json({ error: 'Unsupported inventory operation.' });
    }

    if (nextQuantity < 0) return res.status(409).json({ error: 'Inventory cannot become negative.' });
    if (current.max_stock !== null && nextQuantity > Number(current.max_stock)) {
      return res.status(409).json({ error: `Inventory cannot exceed max_stock (${current.max_stock}).` });
    }
    if (Number(current.reserved_quantity) > nextQuantity) {
      return res.status(409).json({ error: 'Inventory cannot be set below reserved quantity.' });
    }
    ...
    await adminClient.from('zoal_activity_logs').insert({
      id: `inv_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
      user_id: auth.user.id,
      email: auth.email,
      action: `INVENTORY_${operation.toUpperCase()}`,
      timestamp: new Date().toISOString(),
      ip: requestIp(req),
      user_agent: req.headers?.['user-agent'] || 'inventory-api',
      resource_type: 'inventory',
      resource_id: current.id,
      before_state: before,
      after_state: updated,
      changed_fields: Object.keys(updatePayload),
      metadata: { product_id: productId, warehouse_id: warehouseId },
      result: 'success',
      severity: 'info',
      source: 'inventory'
    });
    ...
    return res.status(status).json({ error: status >= 500 ? 'Inventory server error.' : error.message });
  }
}
