...
CREATE TABLE IF NOT EXISTS zoal_customer_crm (
  ...
  status TEXT CHECK (status IS NULL OR status IN ('Active', 'Inactive', 'Blocked', 'Suspended', 'VIP', 'Verified')),
  segment TEXT CHECK (segment IS NULL OR segment IN ('New Customer', 'Returning Customer', 'Regular Customer', 'VIP Customer', 'Inactive Customer', 'High Value Customer', 'Frequent Buyer')),
  ...
  membership_level TEXT CHECK (membership_level IS NULL OR membership_level IN ('Bronze', 'Silver', 'Gold')),
  referral_credits NUMERIC(12,2) CHECK (referral_credits IS NULL OR referral_credits >= 0),
  birthday_reward TEXT CHECK (birthday_reward IS NULL OR birthday_reward IN ('Available', 'Claimed', 'Expired', 'None')),
