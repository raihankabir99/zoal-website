...
async function initializeBrandingDatabase() {
  ...
  try {
    ...
    // Ensure zoal_products.metadata column exists
    try {
      await client.query(`
        ALTER TABLE zoal_products ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}'::jsonb;
      `);
      console.log('✅ Checked and ensured zoal_products.metadata JSONB column exists.');
    } catch (e: any) {
      console.log('⚠️ Could not alter zoal_products table, it might not exist yet:', e.message || e);
    }

    // Ensure zoal_supabase_products is migrated to UUID type
    try {
      const typeCheck = await client.query(`
        SELECT data_type FROM information_schema.columns 
        WHERE table_name = 'zoal_supabase_products' AND column_name = 'id';
      `);
      if (typeCheck.rows.length > 0 && typeCheck.rows[0].data_type !== 'uuid') {
        console.log('🔄 AL ZOAL Database Migration: Upgrading zoal_supabase_products id column to UUID...');
        await client.query('DROP TABLE IF EXISTS zoal_supabase_products CASCADE;');
      }
    } catch (e: any) {
      console.warn('⚠️ Non-blocking warning during database type checking:', e.message || e);
    }

    // Initialize zoal_supabase_products table for dynamic product CRUD with proper UUID type
    try {
      await client.query(`
      ...
      `);
      console.log('✅ Checked and ensured zoal_supabase_products table exists with UUID type.');
    } catch (e: any) {
      console.error('❌ Failed to create zoal_supabase_products table:', e.message || e);
    }
  } catch (err: any) {
...
app.get('/api/products', async (req, res) => {
  ...
  try {
    ...
    // Query rows
    const selectRes = await client.query('SELECT data FROM zoal_supabase_products ORDER BY created_at DESC');
    let products = selectRes.rows.map(row => row.data);

    // Auto-seed table if empty
    if (products.length === 0) {
      console.log('🌱 zoal_supabase_products table is empty. Seeding with default PRODUCTS list...');
      ...
      const reSelectRes = await client.query('SELECT data FROM zoal_supabase_products ORDER BY created_at DESC');
...
app.delete('/api/products/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query('DELETE FROM zoal_supabase_products WHERE id = $1', [uuid]);
