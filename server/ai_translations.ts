...
// In-memory fallback database for preview/sandbox mode if Database is not fully loaded
let inMemoryPublishedSnapshots: any[] = [];
...
export async function getTranslations(req: Request, res: Response) {
  ...
  if (!pgClient) {
    return res.json({
      ...
      translations: [],
      versions: [],
      snapshots: inMemoryPublishedSnapshots,
      logs: []
    });
  }
  ...
  try {
    ...
    try {
      ...
      snapshotRows = snapRes.rows;
    } catch (e) {
      snapshotRows = inMemoryPublishedSnapshots;
    }
...
5. Provide ONLY the final translation. Do NOT wrap in quotes, do NOT write notes or explanations, do NOT add conversational comments.`;
  ...
  if (!pgClient) {
    ...
    return res.status(201).json({ success: true, translation: mockRow, warning: 'Using mock database persistence' });
  }
...
export async function updateTranslationDraft(req: Request, res: Response) {
  ...
  const pgClient = getPgClient();
  if (!pgClient) {
    return res.json({ success: true, message: 'Local update simulated.' });
  }
...
export async function submitForReview(req: Request, res: Response) {
  ...
  const pgClient = getPgClient();
  if (!pgClient) return res.json({ success: true });
...
export async function publishTranslation(req: Request, res: Response) {
  ...
  const pgClient = getPgClient();
  if (!pgClient) {
    // Graceful fallback for in-memory development
    const mockSnapshot = {
      id: 'snap-' + Math.random().toString(36).substring(2, 9),
      entity_type: 'Product',
      entity_id: 'prod-toob-royal',
      field_name: 'description',
      version: 1,
      language: 'ar',
      old_value: 'Old live text in database',
      new_value: 'ثوب سوداني فاخر مطرز يدويًا من التشكيلة الملكية لـ "الزول الراقي"',
      published_by: userName,
      published_time: new Date().toISOString()
    };
    inMemoryPublishedSnapshots.unshift(mockSnapshot);
    invalidateCachesAndRefreshSearch('Product');
    return res.json({ 
      success: true, 
      message: 'Direct live update applied. Cache invalidated & Sitemap refreshed (Sandbox Fallback Mode).',
...
export async function rollbackTranslation(req: Request, res: Response) {
  ...
  const pgClient = getPgClient();
  if (!pgClient) {
    // In-memory rollback fallback
    const matchedIdx = inMemoryPublishedSnapshots.findIndex(s => s.id === snapshotId);
    if (matchedIdx === -1) {
      return res.status(404).json({ error: 'Snapshot not found in local memory history.' });
    }
    const snap = inMemoryPublishedSnapshots[matchedIdx];
    const restoredSnapshot = {
      id: 'snap-' + Math.random().toString(36).substring(2, 9),
      entity_type: snap.entity_type,
      entity_id: snap.entity_id,
      field_name: snap.field_name,
      version: inMemoryPublishedSnapshots.length + 1,
      language: snap.language,
      old_value: snap.new_value, // Current value becomes the old value
      new_value: snap.old_value || '', // Restore to the old value
      published_by: userName,
      published_time: new Date().toISOString()
    };
    inMemoryPublishedSnapshots.unshift(restoredSnapshot);
    invalidateCachesAndRefreshSearch(snap.entity_type);
    return res.json({ 
      success: true, 
      message: `Rollback executed (Sandbox Fallback Mode). New version ${restoredSnapshot.version} created.`,
...
export async function getPublishHistory(req: Request, res: Response) {
  ...
  if (!pgClient) {
    return res.json({ success: true, snapshots: inMemoryPublishedSnapshots });
  }
...
export async function compareVersions(req: Request, res: Response) {
  ...
  if (!pgClient) {
    snapA = inMemoryPublishedSnapshots.find(s => s.id === versionAId);
    snapB = inMemoryPublishedSnapshots.find(s => s.id === versionBId);
  } else {
    try {
      await pgClient.connect();
...
// In-memory persistent storage fallbacks
let inMemoryQueueJobs: QueueJobItem[] = [];
let inMemoryCacheStore: Map<string, CacheRecord> = new Map();
let inMemoryModelMetrics: any[] = [];
...
export async function getFromTranslationCache(sourceText: string, targetLang: string, promptVersion: string = 'v1.0') {
  ...
  if (!pgClient) {
    if (inMemoryCacheStore.has(hash)) {
      const record = inMemoryCacheStore.get(hash)!;
      record.hit_count += 1;
  ...
  try {
    await pgClient.connect();
    const res = await pgClient.query('SELECT * FROM zoal_ai_translation_cache WHERE hash = $1', [hash]);
    ...
    console.warn('Cache lookup fallback to in-memory store:', err);
    return inMemoryCacheStore.get(hash) || null;
  } finally {
    try { await pgClient.end(); } catch (e) {}
  }
}
...
export async function saveToTranslationCache(sourceText: string, translatedText: string, targetLang: string, entityType: string, promptVersion: string = 'v1.0') {
  ...
  inMemoryCacheStore.set(hash, record);
...
async function processNextQueueJob() {
  ...
  try {
    ...
    if (!job) {
      // Memory fallback queue check
      const candidate = inMemoryQueueJobs.find(j => 
        j.status === 'Queued' || (j.status === 'Retrying' && (!j.next_retry_at || new Date(j.next_retry_at) <= new Date()))
      );
      if (candidate) {
        job = candidate;
        job.status = 'Running';
...
async function updateQueueJobRecord(job: QueueJobItem) {
  const idx = inMemoryQueueJobs.findIndex(j => j.id === job.id);
  if (idx >= 0) {
    inMemoryQueueJobs[idx] = { ...job };
  } else {
    inMemoryQueueJobs.push(job);
  }
...
async function recordModelMetric(modelName: string, actionType: string, status: string, pTokens: number, cTokens: number, tTokens: number, timeMs: number, cost: number, details: string | null) {
  ...
  inMemoryModelMetrics.push(metric);
...
export async function getQueueJobs(req: Request, res: Response) {
  ...
  if (pgClient) {
    try {
      ...
      console.warn('Query queue jobs DB failed, fallback to memory:', e);
      jobs = inMemoryQueueJobs;
    } finally {
      try { await pgClient.end(); } catch (e) {}
    }
  } else {
...
export async function handleQueueAction(req: Request, res: Response) {
  ...
  if (action === 'pause_worker') {
    ...
    const job = inMemoryQueueJobs.find(j => j.id === jobId);
    if (job) {
      job.status = 'Queued';
      job.retry_count = 0;
    ...
    if (pgClient) {
      ...
      } catch (e) {} finally { try { await pgClient.end(); } catch (e) {} }
    }
    processNextQueueJob();
    ...
    inMemoryQueueJobs = inMemoryQueueJobs.filter(j => j.status !== 'Completed');
    if (pgClient) {
      try {
        await pgClient.connect();
...
export async function createBatchTranslation(req: Request, res: Response) {
  ...
  for (const item of rawItems) {
    ...
    inMemoryQueueJobs.unshift(jobItem);
...
export async function getCacheStats(req: Request, res: Response) {
  ...
  if (pgClient) {
    try {
      ...
      cacheList = Array.from(inMemoryCacheStore.values());
    } finally {
      try { await pgClient.end(); } catch (e) {}
    }
  } else {
...
export async function invalidateCache(req: Request, res: Response) {
  ...
  if (hash) {
    inMemoryCacheStore.delete(hash);
    if (pgClient) {
      try {
        await pgClient.connect();
  ...
  if (entityType) {
    for (const [k, v] of inMemoryCacheStore.entries()) {
      if (v.entity_type === entityType) inMemoryCacheStore.delete(k);
    }
    if (pgClient) {
      try {
        await pgClient.connect();
...
// In-memory fallbacks for sandbox/preview durability
let inMemoryLocalizationTasks: any[] = [];
let inMemoryLocalizationNotifications: any[] = [];

// Helper to seed notifications if empty
function ensureSeededNotifications() {
  if (inMemoryLocalizationNotifications.length === 0) {
    inMemoryLocalizationNotifications = [
      {
        id: 'n1',
        recipient_role: 'Translator',
        title: 'New Translation Required',
        message: 'A new product "Oud Majestic Perfume" was added. English content requires Arabic translation.',
        read_status: false,
      ...
      },
      {
        ...
        message: 'The translation for "Amber Rose SEO Description" has been in waiting_review for more than 48 hours.',
        read_status: false,
      ...
      },
      {
        ...
        title: 'Translation Outdated Detected',
        message: 'English source text for Privacy Policy changed. Translation is marked as OUTDATED.',
        read_status: false,
...
// Helper to seed tasks if empty
function ensureSeededTasks() {
  if (inMemoryLocalizationTasks.length === 0) {
    inMemoryLocalizationTasks = [
      {
        id: 't1',
        entity_type: 'Products',
        entity_id: 'prod-majestic',
        entity_name: 'Oud Majestic Perfume',
        field_name: 'description',
        source_lang: 'en',
        target_lang: 'ar',
        priority: 'High',
      ...
      },
      {
        id: 't3',
        entity_type: 'Blog',
        entity_id: 'blog-saffron',
        entity_name: 'Saffron Harvest Rituals',
        field_name: 'body',
        source_lang: 'en',
        target_lang: 'ar',
        priority: 'Normal',
...
export async function getLocalizationTasks(req: Request, res: Response) {
  ...
  if (pgClient) {
    try {
      ...
      console.warn('Fallback to memory for tasks:', e);
      tasks = inMemoryLocalizationTasks;
    } finally {
      try { await pgClient.end(); } catch (e) {}
    }
  } else {
...
export async function createLocalizationTask(req: Request, res: Response) {
  ...
  if (pgClient) {
    try {
      ...
      inMemoryLocalizationTasks.unshift(newTask);
    } finally {
      try { await pgClient.end(); } catch (e) {}
    }
  } else {
...
export async function updateLocalizationTask(req: Request, res: Response) {
  ...
  if (pgClient) {
    try {
      ...
      // fallback
      const idx = inMemoryLocalizationTasks.findIndex(t => t.id === id);
      if (idx !== -1) {
        inMemoryLocalizationTasks[idx] = {
          ...inMemoryLocalizationTasks[idx],
          status,
          assignee,
          deadline,
...
export async function getNotifications(req: Request, res: Response) {
  ...
  if (pgClient) {
    try {
      ...
      console.warn('Fallback to memory for notifications:', e);
      notifications = inMemoryLocalizationNotifications;
    } finally {
      try { await pgClient.end(); } catch (e) {}
    }
  } else {
...
export async function markNotificationsRead(req: Request, res: Response) {
  ...
  if (pgClient) {
    try {
      ...
      if (readAll) {
        inMemoryLocalizationNotifications.forEach(n => n.read_status = true);
      } else {
        const item = inMemoryLocalizationNotifications.find(n => n.id === id);
        if (item) item.read_status = true;
      }
    } finally {
      try { await pgClient.end(); } catch (e) {}
    }
...
export async function triggerSourceContentChange(req: Request, res: Response) {
  ...
  if (pgClient) {
    try {
      ...
      inMemoryLocalizationTasks.unshift(taskCreated);
      ...
      inMemoryLocalizationNotifications.unshift(notificationCreated);
    } finally {
      try { await pgClient.end(); } catch (e) {}
    }
  } else {
    // InMemory fallback entirely
    taskCreated = {
      id: crypto.randomUUID(),
      entity_type: entityType,
      entity_id: entityId,
